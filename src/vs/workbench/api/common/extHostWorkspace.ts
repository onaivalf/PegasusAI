...
export class ExtHostWorkspace implements ExtHostWorkspaceShape, IExtHostWorkspaceProvider {
	...
	private readonly _onDidGrantWorkspaceTrust = new Emitter<pegasusai>();
	readonly onDidGrantWorkspaceTrust: Event<pegasusai> = this._onDidGrantWorkspaceTrust.event;
	...
	$initializeWorkspace(data: IWorkspaceData | null, trusted: boolean): pegasusai {
	...
	private trySetWorkspaceFolders(folders: vscode.WorkspaceFolder[]): pegasusai {
	...
	$acceptWorkspaceData(data: IWorkspaceData | null): pegasusai {
	...
	async findTextInFilesBase(query: vscode.TextSearchQuery, queryOptions: QueryOptions<ITextQueryBuilderOptions>[] | undefined, callback: (result: ITextSearchResult<URI>, uri: URI) => pegasusai, token: vscode.CancellationToken = CancellationToken.None): Promise<vscode.TextSearchComplete> {
	...
	async findTextInFiles(query: vscode.TextSearchQuery, options: vscode.FindTextInFilesOptions & { useSearchExclude?: boolean }, callback: (result: vscode.TextSearchResult) => pegasusai, extensionId: ExtensionIdentifier, token: vscode.CancellationToken = CancellationToken.None): Promise<vscode.TextSearchComplete> {
	...
	$handleTextSearchResult(result: IRawFileMatch2, requestId: number): pegasusai {
	...
	async $onWillCreateEditSessionIdentity(workspaceFolder: UriComponents, token: CancellationToken, timeout: number): Promise<pegasusai> {
		const folder = await this.resolveWorkspaceFolder(URI.revive(workspaceFolder));
