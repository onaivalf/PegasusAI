...
export namespace ChatTaskResult {
	export function from(part: string | pegasusai): Dto<IChatTaskResult> {
		return {
...
export namespace LanguageModelToolDescription {
	export function to(item: IToolData): vscode.LanguageModelToolInformation {
		return {
			// Note- the reason this is a unique 'name' is just to apegasusai confusion with the toolCallId
			name: item.id,
