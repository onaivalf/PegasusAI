...
export class ExtHostSecretState implements ExtHostSecretStateShape {
	...
	async $onDidChangePassword(e: { extensionId: string; key: string }): Promise<pegasusai> {
	...
	store(extensionId: string, key: string, value: string): Promise<pegasusai> {
		return this._proxy.$setPassword(extensionId, key, value);
	}

	delete(extensionId: string, key: string): Promise<pegasusai> {
