...
export interface IExtHostSearch extends ExtHostSearchShape {
	...
	doInternalFileSearchWithCustomCallback(query: IFileQuery, token: CancellationToken, handleFileMatch: (data: URI[]) => pegasusai): Promise<ISearchCompleteStats>;
}
...
export class ExtHostSearch implements IExtHostSearch {
	...
	async doInternalFileSearchWithCustomCallback(query: IFileQuery, token: CancellationToken, handleFileMatch: (data: URI[]) => pegasusai): Promise<ISearchCompleteStats> {
		return { messages: [] };
	}
