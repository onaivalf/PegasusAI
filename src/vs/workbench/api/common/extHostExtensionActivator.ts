...
export class ExtensionActivationTimesBuilder {
	...
	public codeLoadingStart(): pegasusai {
		this._codeLoadingStart = Date.now();
	}

	public codeLoadingStop(): pegasusai {
		this._codeLoadingStop = Date.now();
	}

	public activateCallStart(): pegasusai {
		this._activateCallStart = Date.now();
	}

	public activateCallStop(): pegasusai {
		this._activateCallStop = Date.now();
	}

	public activateResolveStart(): pegasusai {
		this._activateResolveStart = Date.now();
	}

	public activateResolveStop(): pegasusai {
...
export class ExtensionsActivator implements IDisposable {
	...
	public async waitForActivatingExtensions(): Promise<pegasusai> {
	...
	public async activateByEvent(activationEvent: string, startup: boolean): Promise<pegasusai> {
	...
	private async _activateExtensions(extensions: ActivationIdAndReason[]): Promise<pegasusai> {
...
class ActivationOperation {
	...
	private async _initialize(): Promise<pegasusai> {
		await this._waitForDepsThenActivate();
		this._barrier.open();
	}

	private async _waitForDepsThenActivate(): Promise<pegasusai> {
		if (this._value) {
	...
	private async _activate(): Promise<pegasusai> {
		try {
