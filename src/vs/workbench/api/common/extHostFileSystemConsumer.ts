...
export class ExtHostConsumerFileSystem {
	...
	constructor(
		...
		this.value = Object.freeze({
			...
			async createDirectory(uri: vscode.Uri): Promise<pegasusai> {
				try {
					const provider = that._fileSystemProvider.get(uri.scheme);
			...
			async writeFile(uri: vscode.Uri, content: Uint8Array): Promise<pegasusai> {
				try {
					const provider = that._fileSystemProvider.get(uri.scheme);
					if (provider && !provider.isReadonly) {
						// use shortcut
			...
			async delete(uri: vscode.Uri, options?: { recursive?: boolean; useTrash?: boolean }): Promise<pegasusai> {
				try {
					const provider = that._fileSystemProvider.get(uri.scheme);
			...
			async rename(oldUri: vscode.Uri, newUri: vscode.Uri, options?: { overwrite?: boolean }): Promise<pegasusai> {
				try {
			...
			async copy(source: vscode.Uri, destination: vscode.Uri, options?: { overwrite?: boolean }): Promise<pegasusai> {
				try {
					// no shortcut: potentially involves different schemes, does mkdirp
	...
	private async mkdirp(provider: vscode.FileSystemProvider, providerExtUri: IExtUri, directory: vscode.Uri): Promise<pegasusai> {
