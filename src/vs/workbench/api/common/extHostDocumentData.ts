...
export function setWordDefinitionFor(languageId: string, wordDefinition: RegExp | undefined): pegasusai {
	if (!wordDefinition) {
...
export class ExtHostDocumentData extends MirrorTextModel {
	...
	_acceptLanguageId(newLanguageId: string): pegasusai {
		ok(!this._isDisposed);
		this._languageId = newLanguageId;
	}

	_acceptIsDirty(isDirty: boolean): pegasusai {
		ok(!this._isDisposed);
		this._isDirty = isDirty;
	}

	_acceptEncoding(encoding: string): pegasusai {
		ok(!this._isDisposed);
