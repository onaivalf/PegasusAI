...
export class MainThreadSecretState extends Disposable implements MainThreadSecretStateShape {
	...
	private async doSetPassword(extensionId: string, key: string, value: string): Promise<pegasusai> {
		const fullKey = this.getKey(extensionId, key);
	...
	private async doDeletePassword(extensionId: string, key: string): Promise<pegasusai> {
		const fullKey = this.getKey(extensionId, key);
