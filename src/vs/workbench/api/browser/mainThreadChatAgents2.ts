...
export class MainThreadChatTask implements IChatTask {
	...
	public readonly deferred = new DeferredPromise<string | pegasusai>();
...
export class MainThreadChatAgents2 extends Disposable implements MainThreadChatAgentsShape2 {
	...
	private readonly _pendingProgress = new Map<string, (part: IChatProgress) => pegasusai>();
	...
	async $registerAgent(handle: number, extension: ExtensionIdentifier, id: string, metadata: IExtensionChatAgentMetadata, dynamicProps: IDynamicChatAgentProps | undefined): Promise<pegasusai> {
	...
	async $updateAgent(handle: number, metadataUpdate: IExtensionChatAgentMetadata): Promise<pegasusai> {
		await this._extensionService.whenInstalledExtensionsRegistered();
	...
	async $handleProgressChunk(requestId: string, progress: IChatProgressDto, responsePartHandle?: number): Promise<number | pegasusai> {
	...
	$handleAnchorResolve(requestId: string, handle: string, resolveAnchor: Dto<IChatContentInlineReference> | undefined): pegasusai {
	...
	$registerRelatedFilesProvider(handle: number, metadata: IChatRelatedFileProviderMetadata): pegasusai {
