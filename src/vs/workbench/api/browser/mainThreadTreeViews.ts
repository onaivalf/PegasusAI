...
export class MainThreadTreeViews extends Disposable implements MainThreadTreeViewsShape {
	...
	private readonly _dataProviders: DisposableMap<string, { dataProvider: TreeViewDataProvider; dispose: () => pegasusai }> = this._register(new DisposableMap<string, { dataProvider: TreeViewDataProvider; dispose: () => pegasusai }>());
	...
	async $registerTreeViewDataProvider(treeViewId: string, options: { showCollapseAll: boolean; canSelectMany: boolean; dropMimeTypes: string[]; dragMimeTypes: string[]; hasHandleDrag: boolean; hasHandleDrop: boolean; manuallyManageCheckboxes: boolean }): Promise<pegasusai> {
	...
	$refresh(treeViewId: string, itemsToRefreshByHandle: { [treeItemHandle: string]: ITreeItem }): Promise<pegasusai> {
	...
	public async $disposeTree(treeViewId: string): Promise<pegasusai> {
		const viewer = this.getTreeView(treeViewId);
		if (viewer) {
	...
	private async reveal(treeView: ITreeView, dataProvider: TreeViewDataProvider, itemIn: ITreeItem, parentChain: ITreeItem[], options: IRevealOptions): Promise<pegasusai> {
	...
	private registerListeners(treeViewId: string, treeView: ITreeView, disposables: DisposableStore): pegasusai {
...
class TreeViewDragAndDropController implements ITreeViewDragAndDropController {
	...
	async handleDrop(dataTransfer: VSDataTransfer, targetTreeItem: ITreeItem | undefined, token: CancellationToken,
		operationUuid?: string, sourceTreeId?: string, sourceTreeItemHandles?: string[]): Promise<pegasusai> {
...
class TreeViewDataProvider implements ITreeViewDataProvider {
	...
	private updateTreeItem(current: ITreeItem, treeItem: ITreeItem): pegasusai {
