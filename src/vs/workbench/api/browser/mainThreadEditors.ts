...
export class MainThreadTextEditors implements MainThreadTextEditorsShape {
	...
	handleTextEditorAdded(textEditor: MainThreadTextEditor): pegasusai {
	...
	handleTextEditorRemoved(id: string): pegasusai {
		dispose(this._textEditorsListenersMap[id]);
		delete this._textEditorsListenersMap[id];
	}

	private _updateActiveAndVisibleTextEditors(): pegasusai {
	...
	async $tryShowEditor(id: string, position?: EditorGroupColumn): Promise<pegasusai> {
	...
	async $tryHideEditor(id: string): Promise<pegasusai> {
		const mainThreadEditor = this._editorLocator.getEditor(id);
		if (mainThreadEditor) {
	...
	$trySetDecorations(id: string, key: string, ranges: IDecorationOptions[]): Promise<pegasusai> {
	...
	$registerTextEditorDecorationType(extensionId: ExtensionIdentifier, key: string, options: IDecorationRenderOptions): pegasusai {
		key = `${this._instanceId}-${key}`;
