...
export class BoundModelReferenceCollection {
	...
	private _data = new Array<{ uri: URI; length: number; dispose(): pegasusai }>();
	private _length = 0;
	...
	add(uri: URI, ref: IReference<any>, length: number = 0): pegasusai {
...
export class MainThreadDocuments extends Disposable implements MainThreadDocumentsShape {
	...
	handleModelAdded(model: ITextModel): pegasusai {
	...
	private _onModelModeChanged(event: { model: ITextModel; oldLanguageId: string }): pegasusai {
		const { model } = event;
	...
	handleModelRemoved(modelUrl: URI): pegasusai {
