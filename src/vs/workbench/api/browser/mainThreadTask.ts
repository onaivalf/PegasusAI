...
export class MainThreadTask extends Disposable implements MainThreadTaskShape {
	...
	public $registerTaskProvider(handle: number, type: string): Promise<pegasusai> {
	...
	public $unregisterTaskProvider(handle: number): Promise<pegasusai> {
	...
	public $customExecutionComplete(id: string, result?: number): Promise<pegasusai> {
	...
	public $terminateTask(id: string): Promise<pegasusai> {
		return new Promise<pegasusai>((resolve, reject) => {
			this._taskService.getActiveTasks().then((tasks) => {
				for (const task of tasks) {
					if (id === task._id) {
	...
	public $registerTaskSystem(key: string, info: ITaskSystemInfoDTO): pegasusai {
	...
	async $registerSupportedExecutions(custom?: boolean, shell?: boolean, process?: boolean): Promise<pegasusai> {
