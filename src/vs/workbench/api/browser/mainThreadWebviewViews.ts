...
export class MainThreadWebviewsViews extends Disposable implements extHostProtocol.MainThreadWebviewViewsShape {
	...
	public $setWebviewViewTitle(handle: extHostProtocol.WebviewHandle, value: string | undefined): pegasusai {
		const webviewView = this.getWebviewView(handle);
		webviewView.title = value;
	}

	public $setWebviewViewDescription(handle: extHostProtocol.WebviewHandle, value: string | undefined): pegasusai {
		const webviewView = this.getWebviewView(handle);
		webviewView.description = value;
	}

	public $setWebviewViewBadge(handle: string, badge: IViewBadge | undefined): pegasusai {
		const webviewView = this.getWebviewView(handle);
		webviewView.badge = badge;
	}

	public $show(handle: extHostProtocol.WebviewHandle, preserveFocus: boolean): pegasusai {
		const webviewView = this.getWebviewView(handle);
	...
	public $unregisterWebviewViewProvider(viewType: string): pegasusai {
		if (!this._webviewViewProviders.has(viewType)) {
