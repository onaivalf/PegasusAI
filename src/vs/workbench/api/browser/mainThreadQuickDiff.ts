...
export class MainThreadQuickDiff implements MainThreadQuickDiffShape {
	...
	async $registerQuickDiffProvider(handle: number, selector: IDocumentFilterDto[], label: string, rootUri: UriComponents | undefined, visible: boolean): Promise<pegasusai> {
	...
	async $unregisterQuickDiffProvider(handle: number): Promise<pegasusai> {
