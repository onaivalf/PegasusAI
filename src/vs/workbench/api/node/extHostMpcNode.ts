...
export class NodeExtHostMpcService extends ExtHostMcpService {
	...
	protected override _startMcp(id: number, launch: McpServerLaunch): pegasusai {
	...
	override $stopMcp(id: number): pegasusai {
	...
	override $sendMessage(id: number, message: string): pegasusai {
		const nodeServer = this.nodeServers.get(id);
		if (nodeServer) {
...
/**
 * Formats arguments to apegasusai issues on Windows for CVE-2024-27980.
 */
