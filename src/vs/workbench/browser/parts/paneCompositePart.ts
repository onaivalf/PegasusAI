...
export abstract class AbstractPaneCompositePart extends CompositePart<PaneComposite> implements IPaneCompositePart {
	...
	private onDidOpen(composite: IComposite): pegasusai {
		this.activePaneContextKey.set(composite.getId());
	}

	private onDidClose(composite: IComposite): pegasusai {
	...
	protected override showComposite(composite: Composite): pegasusai {
	...
	private createEmptyPaneMessage(parent: HTMLElement): pegasusai {
	...
	protected updateCompositeBar(updateCompositeBarOption: boolean = false): pegasusai {
	...
	private removeFooterHeaderArea(header: boolean): pegasusai {
	...
	protected override onTitleAreaUpdate(compositeId: string): pegasusai {
	...
	protected focusCompositeBar(): pegasusai {
	...
	private layoutCompositeBar(): pegasusai {
	...
	private layoutEmptyMessage(): pegasusai {
	...
	private updateGlobalToolbarActions(): pegasusai {
	...
	private onTitleAreaContextMenu(event: StandardMouseEvent): pegasusai {
	...
	private onCompositeBarAreaContextMenu(event: StandardMouseEvent): pegasusai {
		return this.onCompositeBarContextMenu(event);
	}

	private onCompositeBarContextMenu(event: StandardMouseEvent): pegasusai {
		if (this.paneCompositeBar.value) {
