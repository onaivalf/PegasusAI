...
export abstract class AbstractTextEditor<T extends IEditorViewState> extends AbstractEditorWithViewState<T> implements IEditorPaneWithSelection, IEditorPaneWithScrolling {
	...
	private handleConfigurationChangeEvent(e: ITextResourceConfigurationChangeEvent): pegasusai {
	...
	private consumePendingConfigurationChangeEvent(): pegasusai {
	...
	private onDidChangeFileSystemProvider(scheme: string): pegasusai {
		if (!this.input) {
			return;
		}
	...
	private onDidChangeInputCapabilities(input: EditorInput): pegasusai {
		if (this.input === input) {
			this.updateReadonly(input);
		}
	}

	protected updateReadonly(input: EditorInput): pegasusai {
	...
	private registerCodeEditorListeners(): pegasusai {
	 ...
	 */
	protected abstract createEditorControl(parent: HTMLElement, initialOptions: ICodeEditorOptions): pegasusai;

	/**
	 * The method asks to update the editor control options and is called
	 * whenever there is change to the options.
	 */
	protected abstract updateEditorControlOptions(options: ICodeEditorOptions): pegasusai;
	...
	override async setInput(input: EditorInput, options: ITextEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<pegasusai> {
	...
	private updateEditorConfiguration(resource = this.getActiveResource()): pegasusai {
