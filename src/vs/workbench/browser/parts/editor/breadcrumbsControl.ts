...
export class BreadcrumbsControl {
	...
	private readonly _onDidVisibilityChange = this._disposables.add(new Emitter<pegasusai>());
	...
	private _onFocusEvent(event: IBreadcrumbsItemEvent): pegasusai {
	...
	private _onSelectEvent(event: IBreadcrumbsItemEvent): pegasusai {
		if (!event.item) {
			return;
		}
	...
	private _updateCkBreadcrumbsActive(): pegasusai {
		const value = this._widget.isDOMFocused() || this._breadcrumbsPickerShowing;
		this._ckBreadcrumbsActive.set(value);
	}

	private async _revealInEditor(event: IBreadcrumbsItemEvent, element: FileElement | OutlineElement2, group: SIDE_GROUP_TYPE | ACTIVE_GROUP_TYPE | undefined, pinned: boolean = false): Promise<pegasusai> {
...
export class BreadcrumbsControlFactory {
	...
	private readonly _onDidEnablementChange = this._disposables.add(new Emitter<pegasusai>());
...
function focusAndSelectHandler(accessor: ServicesAccessor, select: boolean): pegasusai {
