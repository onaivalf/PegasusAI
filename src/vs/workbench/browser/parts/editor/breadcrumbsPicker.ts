...
export abstract class BreadcrumbsPicker {
	...
	protected readonly _onWillPickElement = new Emitter<pegasusai>();
	readonly onWillPickElement: Event<pegasusai> = this._onWillPickElement.event;
	...
	async show(input: any, maxHeight: number, width: number, arrowSize: number, arrowOffset: number): Promise<pegasusai> {
	...
	restoreViewState(): pegasusai { }

	protected abstract _setInput(element: FileElement | OutlineElement2): Promise<pegasusai>;
...
class FileRenderer implements ITreeRenderer<IFileStat | IWorkspaceFolder, FuzzyScore, IResourceLabel> {
	...
	renderElement(node: ITreeNode<IWorkspaceFolder | IFileStat, [number, number, number]>, index: number, templateData: IResourceLabel): pegasusai {
	...
	disposeTemplate(templateData: IResourceLabel): pegasusai {
...
export class BreadcrumbsFilePicker extends BreadcrumbsPicker {
	...
	protected async _setInput(element: FileElement | OutlineElement2): Promise<pegasusai> {
...
export class BreadcrumbsOutlinePicker extends BreadcrumbsPicker {
	...
	protected _setInput(input: OutlineElement2): Promise<pegasusai> {
