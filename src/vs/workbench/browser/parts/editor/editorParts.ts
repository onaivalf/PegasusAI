...
export class EditorParts extends MultiWindowParts<EditorPart> implements IEditorGroupsService, IEditorPartsView {
	...
	protected override unregisterPart(part: EditorPart): pegasusai {
	...
	private registerEditorPartListeners(part: EditorPart, disposables: DisposableStore): pegasusai {
	...
	private doUpdateMostRecentActive(part: EditorPart, makeMostRecentlyActive?: boolean): pegasusai {
	...
	private async restoreParts(): Promise<pegasusai> {
	...
	private async restoreState(state: IEditorPartsUIState): Promise<pegasusai> {
	...
	deleteWorkingSet(workingSet: IEditorWorkingSet): pegasusai {
	...
	private saveWorkingSets(): pegasusai {
	...
	arrangeGroups(arrangement: GroupsArrangement, group: IEditorGroupView | GroupIdentifier = this.activePart.activeGroup): pegasusai {
		this.getPart(group).arrangeGroups(arrangement, group);
	}

	toggleMaximizeGroup(group: IEditorGroupView | GroupIdentifier = this.activePart.activeGroup): pegasusai {
		this.getPart(group).toggleMaximizeGroup(group);
	}

	toggleExpandGroup(group: IEditorGroupView | GroupIdentifier = this.activePart.activeGroup): pegasusai {
	...
	removeGroup(group: IEditorGroupView | GroupIdentifier): pegasusai {
	...
	private registerGroupsContextKeyListeners(): pegasusai {
	...
	private updateGlobalContextKeys(): pegasusai {
	...
	private registerGroupContextKeyProvidersListeners(group: IEditorGroupView): pegasusai {
	...
	private updateRegisteredContextKey<T extends ContextKeyValue>(group: IEditorGroupView, provider: IEditorGroupContextKeyProvider<T>): pegasusai {
