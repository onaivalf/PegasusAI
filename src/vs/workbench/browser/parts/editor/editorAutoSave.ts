...
export class EditorAutoSave extends Disposable implements IWorkbenchContribution {
	...
	private onConditionChanged(resources: readonly URI[], condition: AutoSaveDisabledReason.ERRORS | AutoSaveDisabledReason.DISABLED): pegasusai {
	...
	private onWindowFocusChange(focused: boolean): pegasusai {
		if (!focused) {
			this.maybeTriggerAutoSave(SaveReason.WINDOW_CHANGE);
		}
	}

	private onActiveWindowChange(): pegasusai {
		this.maybeTriggerAutoSave(SaveReason.WINDOW_CHANGE);
	}
	...
	private maybeTriggerAutoSave(reason: SaveReason.WINDOW_CHANGE | SaveReason.FOCUS_CHANGE, editorIdentifier?: IEditorIdentifier): pegasusai {
		if (editorIdentifier) {
			if (
	...
	private onDidChangeAutoSaveConfiguration(): pegasusai {
	...
	private saveAllDirtyAutoSaveables(reason: SaveReason): pegasusai {
	...
	private scheduleAutoSave(workingCopy: IWorkingCopy): pegasusai {
		if (workingCopy.capabilities & WorkingCopyCapabilities.Untitled) {
	...
	private discardAutoSave(workingCopy: IWorkingCopy): pegasusai {
