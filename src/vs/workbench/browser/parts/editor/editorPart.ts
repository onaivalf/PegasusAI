...
export class EditorPart extends Part implements IEditorPart, IEditorGroupsView {
	...
	private readonly _onDidScroll = this._register(new Relay<pegasusai>());
	...
	private handleChangedPartOptions(): pegasusai {
	...
	private fillGridNodes(target: IEditorGroupView[], node: GridBranchNode<IEditorGroupView> | GridNode<IEditorGroupView>): pegasusai {
	...
	setSize(group: IEditorGroupView | GroupIdentifier, size: { width: number; height: number }): pegasusai {
		const groupView = this.assertGroupView(group);

		this.gridWidget.resizeView(groupView, size);
	}

	arrangeGroups(arrangement: GroupsArrangement, target: IEditorGroupView | GroupIdentifier = this.activeGroup): pegasusai {
		if (this.count < 2) {
	...
	toggleMaximizeGroup(target: IEditorGroupView | GroupIdentifier = this.activeGroup): pegasusai {
	...
	toggleExpandGroup(target: IEditorGroupView | GroupIdentifier = this.activeGroup): pegasusai {
	...
	private unmaximizeGroup(): pegasusai {
	...
	setGroupOrientation(orientation: GroupOrientation): pegasusai {
		if (!this.gridWidget) {
			return; // we have not been created yet
		}
	...
	applyLayout(layout: EditorGroupLayout): pegasusai {
		const restoreFocus = this.shouldRestoreFocus(this.container);

		// Determine how many groups we need overall
		let layoutGroupsCount = 0;
		function countGroups(groups: GroupLayoutArgument[]): pegasusai {
	...
	private doSetGroupActive(group: IEditorGroupView): pegasusai {
	...
	private doRestoreGroup(group: IEditorGroupView): pegasusai {
		if (!this.gridWidget) {
	...
	private doUpdateMostRecentActive(group: IEditorGroupView, makeMostRecentlyActive?: boolean): pegasusai {
	...
	private doRemoveGroupWithEditors(groupView: IEditorGroupView): pegasusai {
	...
	private doRemoveEmptyGroup(groupView: IEditorGroupView, preserveFocus?: boolean): pegasusai {
	...
	private handleContextKeys(contextKeyService: IContextKeyService): pegasusai {
	...
	private setupDragAndDropSupport(parent: HTMLElement, container: HTMLElement): pegasusai {
	...
	private doCreateGridControl(): pegasusai {
	...
	private doCreateGridControlWithState(serializedGrid: ISerializedGrid, activeGroupId: GroupIdentifier, editorGroupViewsToReuse?: IEditorGroupView[], options?: IEditorGroupViewOptions): pegasusai {
	...
	private doSetGridWidget(gridWidget: SerializableGrid<IEditorGroupView>): pegasusai {
	...
	private notifyGroupIndexChange(): pegasusai {
	...
	override layout(width: number, height: number, top: number, left: number): pegasusai {
		this.top = top;
		this.left = left;
	...
	private doLayout(dimension: Dimension, top = this.top, left = this.left): pegasusai {
	...
	applyState(state: IEditorPartUIState | 'empty', options?: IEditorGroupViewOptions): Promise<pegasusai> {
		if (state === 'empty') {
	...
	private async doApplyState(state: IEditorPartUIState, options?: IEditorGroupViewOptions): Promise<pegasusai> {
	...
	private async doApplyEmptyState(): Promise<pegasusai> {
	...
	private doApplyGridState(gridState: ISerializedGrid, activeGroupId: GroupIdentifier, editorGroupViewsToReuse?: IEditorGroupView[], options?: IEditorGroupViewOptions): pegasusai {
	...
	private onDidChangeMementoState(e: IStorageValueChangeEvent): pegasusai {
	...
	private disposeGroups(): pegasusai {
		for (const group of this.groups) {
			group.dispose();
