...
export interface IPaneCompositeBarOptions {
	...
	readonly fillExtraContextMenuActions: (actions: IAction[], e?: MouseEvent | GestureEvent) => pegasusai;
...
export class PaneCompositeBar extends Disposable {
	...
	private createMoveAction(viewContainer: ViewContainer, newLocation: ViewContainerLocation, defaultLocation: ViewContainerLocation): IAction {
		return toAction({
			...
			label: newLocation === ViewContainerLocation.Panel ? localize('panel', "Panel") : newLocation === ViewContainerLocation.Sidebar ? localize('sidebar', "Primary Side Bar") : localize('auxiliarybar', "PegasusAI Side Bar"),
			run: () => {
	...
	private onDidRegisterExtensions(): pegasusai {
	...
	private onDidViewContainerVisible(id: string): pegasusai {
		const viewContainer = this.getViewContainer(id);
		if (viewContainer) {
	...
	private onDidRegisterViewContainers(viewContainers: readonly ViewContainer[]): pegasusai {
	...
	private updateCompositeBarActionItem(viewContainer: ViewContainer, viewContainerModel: IViewContainerModel): pegasusai {
	...
	private showOrHideViewContainer(viewContainer: ViewContainer): pegasusai {
		if (this.shouldBeHidden(viewContainer)) {
			this.hideComposite(viewContainer.id);
		} else {
			this.addComposite(viewContainer);
	...
	private addComposite(viewContainer: ViewContainer): pegasusai {
	...
	private hideComposite(compositeId: string): pegasusai {
	...
	private removeComposite(compositeId: string): pegasusai {
	...
	private updateCompositeBarItemsFromStorage(retainExisting: boolean): pegasusai {
	...
	private saveCachedViewContainers(): pegasusai {
	...
	private storeCachedViewContainersState(cachedViewContainers: ICachedViewContainer[]): pegasusai {
	...
	private setPinnedViewContainers(pinnedViewContainers: IPinnedViewContainer[]): pegasusai {
	...
	private setStoredPinnedViewContainersValue(value: string): pegasusai {
	...
	private setPlaceholderViewContainers(placeholderViewContainers: IPlaceholderViewContainer[]): pegasusai {
	...
	private setStoredPlaceholderViewContainersValue(value: string): pegasusai {
	...
	private setViewContainersWorkspaceState(viewContainersWorkspaceState: IViewContainerWorkspaceState[]): pegasusai {
	...
	private setStoredViewContainersWorkspaceStateValue(value: string): pegasusai {
...
class ViewContainerActivityAction extends CompositeBarAction {
	...
	updateCompositeBarActionItem(compositeBarActionItem: ICompositeBarActionItem): pegasusai {
		this.compositeBarActionItem = compositeBarActionItem;
	}

	private updateActivity(): pegasusai {
		this.activities = this.activityService.getViewContainerActivities(this.compositeBarActionItem.id);
	}

	override async run(event: { preserveFocus: boolean }): Promise<pegasusai> {
...
class PlaceHolderToggleCompositePinnedAction extends ToggleCompositePinnedAction {
	...
	setActivity(activity: ICompositeBarActionItem): pegasusai {
...
class PlaceHolderToggleCompositeBadgeAction extends ToggleCompositeBadgeAction {
	...
	setCompositeBarActionItem(actionItem: ICompositeBarActionItem): pegasusai {
