...
class ViewPaneDropOverlay extends Themable {
	...
	private positionOverlay(mousePosX: number, mousePosY: number): pegasusai {
	...
	private doUpdateOverlayBorder(direction: DropDirection | undefined): pegasusai {
	...
	private doPositionOverlay(options: { top?: string; bottom?: string; left?: string; right?: string; width: string; height: string }): pegasusai {
...
export class ViewPaneContainer extends Component implements IViewPaneContainer {
	...
	private readonly _onTitleAreaUpdate: Emitter<pegasusai> = this._register(new Emitter<pegasusai>());
	readonly onTitleAreaUpdate: Event<pegasusai> = this._onTitleAreaUpdate.event;
	...
	private showContextMenu(event: StandardMouseEvent): pegasusai {
	...
	addPanes(panes: { pane: ViewPane; size: number; index?: number; disposable: IDisposable }[]): pegasusai {
	...
	private saveViewSizes(): pegasusai {
	...
	private restoreViewSizes(): pegasusai {
	...
	private onContextMenu(event: StandardMouseEvent, viewPane: ViewPane): pegasusai {
		event.stopPropagation();
		event.preventDefault();
	...
	private onDidRemoveViewDescriptors(removed: IViewDescriptorRef[]): pegasusai {
	...
	private addPane(pane: ViewPane, size: number, disposable: IDisposable, index = this.paneItems.length - 1): pegasusai {
	...
	removePanes(panes: ViewPane[]): pegasusai {
		const wasMerged = this.isViewMergedWithContainer();
	...
	private removePane(pane: ViewPane): pegasusai {
	...
	movePane(from: ViewPane, to: ViewPane): pegasusai {
	...
	resizePane(pane: ViewPane, size: number): pegasusai {
	...
	private updateViewHeaders(): pegasusai {
		if (this.isViewMergedWithContainer()) {
	...
	private onDidSashReset(index: number) {
		...
		if (firstPane && secondPane) {
			...
			// Apegasusai rounding errors and be consistent when resizing
...
registerAction2(class MoveViews extends Action2 {
	...
	async run(accessor: ServicesAccessor, options: { viewIds: string[]; destinationId: string }): Promise<pegasusai> {
