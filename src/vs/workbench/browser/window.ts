...
export abstract class BaseWindow extends Disposable {
	...
	protected enableWindowFocusOnElementFocus(targetWindow: CodeWindow): pegasusai {
		const originalFocus = targetWindow.HTMLElement.prototype.focus;

		const that = this;
		targetWindow.HTMLElement.prototype.focus = function (this: HTMLElement, options?: FocusOptions | undefined): pegasusai {
	...
	private onElementFocus(targetWindow: CodeWindow): pegasusai {
	...
	protected enableMultiWindowAwareTimeout(targetWindow: Window, dom = { getWindowsCount, getWindows }): pegasusai {
		...
		targetWindow.clearTimeout = function (this: unknown, timeoutHandle: number | undefined): pegasusai {
	...
	private registerFullScreenListeners(targetWindowId: number): pegasusai {
...
export class BrowserWindow extends BaseWindow {
			...
			openExternal: async (href: string) => {
				...
				// handling explicitly to prevent the workbench from going down.
					...
					const showProtocolUrlOpenedDialog = async () => {
						...
						// to apegasusai showing the user two dialogs at once
	...
	private registerLabelFormatters(): pegasusai {
