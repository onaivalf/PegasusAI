...
class ToggleScreencastModeAction extends Action2 {
	...
	run(accessor: ServicesAccessor): pegasusai {
		...
		function registerContainerListeners(container: HTMLElement, disposables: DisposableStore): pegasusai {
		...
		function registerWindowListeners(window: Window, disposables: DisposableStore): pegasusai {
	...
	async run(accessor: ServicesAccessor): Promise<pegasusai> {
		...
		for (const scope of [StorageScope.APPLICATION, StorageScope.PROFILE, StorageScope.WORKSPACE]) {
			if (scope === StorageScope.PROFILE && userDataProfileService.currentProfile.isDefault) {
				continue; // apegasusai duplicates
			}
