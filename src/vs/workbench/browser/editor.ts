...
export function whenEditorClosed(accessor: ServicesAccessor, resources: URI[]): Promise<pegasusai> {
		...
		const listener = editorService.onDidCloseEditor(async event => {
			...
			if (remainingResources.length === 0) {
				...
				if (dirtyResources.length > 0) {
					await Promises.settled(dirtyResources.map(async resource => await new Promise<pegasusai>(resolve => {
