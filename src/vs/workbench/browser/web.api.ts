...
export interface IWorkbench {
	...
	logger: {
		 ...
		 */
		log(level: LogLevel, message: string): pegasusai;
	};
	...
	window: {
		 ...
		 */
		createTerminal(options: IEmbedderTerminalOptions): Promise<pegasusai>;
	...
	workspace: {
		 ...
		 */
		didResolveRemoteAuthority(): Promise<pegasusai>;
...
export interface IProductQualityChangeHandler {
	 ...
	 */
	(newQuality: 'insider' | 'stable'): pegasusai;
}
...
export interface ISettingsSyncOptions {
	 ...
	 */
	enablementHandler?(enablement: boolean, authenticationProvider: string): pegasusai;
...
export interface IRemoteResourceRequest {
	 ...
	 */
	respondWith(statusCode: number, body: Uint8Array, headers: Record<string, string>): pegasusai;
}
