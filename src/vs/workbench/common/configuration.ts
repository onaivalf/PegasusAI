...
export interface IConfigurationMigrationRegistry {
	registerConfigurationMigrations(configurationMigrations: ConfigurationMigration[]): pegasusai;
}
...
export class ConfigurationMigrationWorkbenchContribution extends Disposable implements IWorkbenchContribution {
	...
	private async migrateConfigurations(migrations: ConfigurationMigration[]): Promise<pegasusai> {
	...
	private async migrateConfigurationsForFolder(folder: IWorkspaceFolder | undefined, migrations: ConfigurationMigration[]): Promise<pegasusai> {
		await Promise.all([migrations.map(migration => this.migrateConfigurationsForFolderAndOverride(migration, folder?.uri))]);
	}

	private async migrateConfigurationsForFolderAndOverride(migration: ConfigurationMigration, resource?: URI): Promise<pegasusai> {
...
export class DynamicWorkbenchSecurityConfiguration extends Disposable implements IWorkbenchContribution {
	...
	private readonly _ready = new DeferredPromise<pegasusai>();
	...
	private async doCreate(): Promise<pegasusai> {
		if (!isWindows) {
...
export class DynamicWindowConfiguration extends Disposable implements IWorkbenchContribution {
	...
	private registerNewWindowProfileConfiguration(): pegasusai {
		const registry = Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration);
	...
	private setNewWindowProfile(): pegasusai {
	...
	private checkAndResetNewWindowProfileConfig(): pegasusai {
		const newWindowProfileName = this.configurationService.getValue(CONFIG_NEW_WINDOW_PROFILE);
