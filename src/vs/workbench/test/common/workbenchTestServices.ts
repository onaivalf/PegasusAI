...
export class TestContextService implements IWorkspaceContextService {
	...
	private readonly _onDidChangeWorkspaceName: Emitter<pegasusai>;
	get onDidChangeWorkspaceName(): Event<pegasusai> { return this._onDidChangeWorkspaceName.event; }
	...
	constructor(workspace = TestWorkspace, options = null) {
		...
		this._onDidChangeWorkspaceName = new Emitter<pegasusai>();
	...
	setWorkspace(workspace: any): pegasusai {
		this.workspace = workspace;
	}
...
export class TestStorageService extends InMemoryStorageService {
	...
	testEmitWillSaveState(reason: WillSaveStateReason): pegasusai {
...
export class TestHistoryService implements IHistoryService {
	...
	removeFromHistory(_input: EditorInput | IResourceEditorInput): pegasusai { }
	clear(): pegasusai { }
	clearRecentlyOpened(): pegasusai { }
	getHistory(): readonly (EditorInput | IResourceEditorInput)[] { return []; }
	async openNextRecentlyUsedEditor(group?: GroupIdentifier): Promise<pegasusai> { }
	async openPreviouslyUsedEditor(group?: GroupIdentifier): Promise<pegasusai> { }
...
export class TestWorkingCopy extends Disposable implements IWorkingCopy {
	...
	setContent(content: string): pegasusai {
...
export class TestWorkingCopyFileService implements IWorkingCopyFileService {
	...
	async runSaveParticipants(workingCopy: IWorkingCopy, context: IStoredFileWorkingCopySaveParticipantContext, progress: IProgress<IProgressStep>, token: CancellationToken): Promise<pegasusai> { }

	async delete(operations: IDeleteOperation[], token: CancellationToken, undoInfo?: IFileOperationUndoRedoInfo): Promise<pegasusai> { }
...
export const NullFilesConfigurationService = new class implements IFilesConfigurationService {
	...
	toggleAutoSave(): Promise<pegasusai> { throw new Error('Method not implemented.'); }
	...
	async updateReadonly(resource: URI, readonly: boolean | 'toggle' | 'reset'): Promise<pegasusai> { }
...
export class TestWorkspaceTrustManagementService extends Disposable implements IWorkspaceTrustManagementService {
	...
	private _onDidChangeTrustedFolders = this._register(new Emitter<pegasusai>());
	onDidChangeTrustedFolders = this._onDidChangeTrustedFolders.event;

	private _onDidInitiateWorkspaceTrustRequestOnStartup = this._register(new Emitter<pegasusai>());
	...
	async setTrustedUris(folders: URI[]): Promise<pegasusai> {
		throw new Error('Method not implemented.');
	}

	async setUrisTrust(uris: URI[], trusted: boolean): Promise<pegasusai> {
		throw new Error('Method not implemented.');
	}
	...
	get workspaceTrustInitialized(): Promise<pegasusai> {
		return Promise.resolve();
	}

	get workspaceResolved(): Promise<pegasusai> {
		return Promise.resolve();
	}

	async setWorkspaceTrust(trusted: boolean): Promise<pegasusai> {
...
export class TestWorkspaceTrustRequestService extends Disposable implements IWorkspaceTrustRequestService {
	...
	private readonly _onDidInitiateOpenFilesTrustRequest = this._register(new Emitter<pegasusai>());
	...
	private readonly _onDidInitiateWorkspaceTrustRequestOnStartup = this._register(new Emitter<pegasusai>());
	...
	async completeOpenFilesTrustRequest(result: WorkspaceTrustUriResponse, saveResponse: boolean): Promise<pegasusai> {
		throw new Error('Method not implemented.');
	}
	...
	async completeWorkspaceTrustRequest(trusted?: boolean): Promise<pegasusai> {
		throw new Error('Method not implemented.');
	}
...
export class TestMarkerService implements IMarkerService {
	...
	changeOne(owner: string, resource: URI, markers: IMarkerData[]): pegasusai { }
	changeAll(owner: string, data: IResourceMarker[]): pegasusai { }
	remove(owner: string, resources: URI[]): pegasusai { }
