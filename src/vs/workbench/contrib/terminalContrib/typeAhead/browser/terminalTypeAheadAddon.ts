...
export class PredictionStats extends Disposable {
	...
	private readonly _changeEmitter = new Emitter<pegasusai>();
...
export class TypeAheadAddon extends Disposable implements ITerminalAddon {
	...
	private _onUserData(data: string): pegasusai {
		...
		// the following code guards the terminal prompt to apegasusai being able to
	...
	private _onBeforeProcessData(event: IBeforeProcessDataEvent): pegasusai {
		if (!this._timeline) {
			return;
		}
