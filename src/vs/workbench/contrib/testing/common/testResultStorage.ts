...
export interface ITestResultStorage {
	 ...
	 */
	persist(results: ReadonlyArray<ITestResult>): Promise<pegasusai>;
}
...
export abstract class BaseTestResultStorage extends Disposable implements ITestResultStorage {
	 ...
	 * @override
	 */
	public async persist(results: ReadonlyArray<ITestResult>): Promise<pegasusai> {
	 ...
	 * Reads serialized results for the test. Is allowed to throw.
	 */
	protected abstract storeOutputForResultId(id: string, input: VSBufferWriteableStream): Promise<pegasusai>;
}
...
export class InMemoryResultStorage extends BaseTestResultStorage {
	...
	protected storeOutputForResultId(id: string, input: VSBufferWriteableStream): Promise<pegasusai> {
		throw new Error('Method not implemented.');
	}
