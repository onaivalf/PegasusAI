	...
	*/
	private async resolveWorkspaceTags(): Promise<Tags> {
		...
		return this.fileService.resolveAll(folders.map(resource => ({ resource }))).then((files: IFileStatResult[]) => {
			...
			function getFilePromises(filename: string, fileService: IFileService, textFileService: ITextFileService, contentHandler: (content: ITextFileContent) => pegasusai): Promise<pegasusai>[] {
			...
			function addPythonTags(packageName: string): pegasusai {
	...
	private processGradleDependencies(content: string, regex: RegExp, tags: Tags): pegasusai {
		let dependencyContent;
	...
	private tagJavaDependency(groupId: string, artifactId: string, prefix: string, tags: Tags): pegasusai {
