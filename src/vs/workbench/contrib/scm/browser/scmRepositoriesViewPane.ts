...
export class SCMRepositoriesViewPane extends ViewPane {
	...
	private updateBodySize(): pegasusai {
	...
	private onListContextMenu(e: IListContextMenuEvent<ISCMRepository>): pegasusai {
		if (!e.element) {
			return;
		}
	...
	private onListSelectionChange(e: IListEvent<ISCMRepository>): pegasusai {
	...
	private onDidChangeFocus(e: IListEvent<ISCMRepository>): pegasusai {
		if (e.browserEvent && e.elements.length > 0) {
			this.scmViewService.focus(e.elements[0]);
		}
	}

	private updateListSelection(): pegasusai {
