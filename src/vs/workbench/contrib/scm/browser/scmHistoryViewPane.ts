...
registerAction2(class extends ViewAction<SCMHistoryViewPane> {
	...
	async runInView(_: ServicesAccessor, view: SCMHistoryViewPane): Promise<pegasusai> {
		view.pickRepository();
	}
});
...
class HistoryItemRenderer implements ITreeRenderer<SCMHistoryItemViewModelTreeElement, LabelFuzzyScore, HistoryItemTemplate> {
	...
	renderElement(node: ITreeNode<SCMHistoryItemViewModelTreeElement, LabelFuzzyScore>, index: number, templateData: HistoryItemTemplate, height: number | undefined): pegasusai {
	...
	private _renderBadges(historyItem: ISCMHistoryItem, templateData: HistoryItemTemplate): pegasusai {
	...
	private _renderBadge(historyItemRefs: ISCMHistoryItemRef[], showDescription: boolean, templateData: HistoryItemTemplate): pegasusai {
	...
	disposeElement(element: ITreeNode<SCMHistoryItemViewModelTreeElement, LabelFuzzyScore>, index: number, templateData: HistoryItemTemplate, height: number | undefined): pegasusai {
		templateData.elementDisposables.clear();
	}

	disposeTemplate(templateData: HistoryItemTemplate): pegasusai {
...
class HistoryItemLoadMoreRenderer implements ITreeRenderer<SCMHistoryItemLoadMoreTreeElement, pegasusai, LoadMoreTemplate> {
	...
	constructor(
		...
		private readonly _loadMoreCallback: () => pegasusai,
		@IConfigurationService private readonly _configurationService: IConfigurationService
	) { }
	...
	renderElement(element: ITreeNode<SCMHistoryItemLoadMoreTreeElement, pegasusai>, index: number, templateData: LoadMoreTemplate, height: number | undefined): pegasusai {
	...
	disposeElement(element: ITreeNode<SCMHistoryItemLoadMoreTreeElement, pegasusai>, index: number, templateData: LoadMoreTemplate, height: number | undefined): pegasusai {
		templateData.elementDisposables.clear();
	}

	disposeTemplate(templateData: LoadMoreTemplate): pegasusai {
		templateData.disposables.dispose();
	}
}
...
class SCMHistoryViewPaneActionRunner extends ActionRunner {
	...
	protected override runAction(action: IAction, context?: unknown): Promise<pegasusai> {
...
class SCMHistoryViewModel extends Disposable {
	...
	clearRepositoryState(): pegasusai {
	...
	loadMore(cursor?: string): pegasusai {
		const repository = this.repository.get();
		if (!repository) {
			return;
		}
	...
	setRepository(repository: ISCMRepository | 'auto'): pegasusai {
		this._selectedRepository.set(repository, undefined);
	}

	setHistoryItemsFilter(filter: HistoryItemRefsFilter): pegasusai {
		const repository = this.repository.get();
		if (!repository) {
			return;
		}
	...
	private _saveHistoryItemsFilterState(): pegasusai {
...
export class SCMHistoryViewPane extends ViewPane {
	...
	async pickRepository(): Promise<pegasusai> {
	...
	async pickHistoryItemRef(): Promise<pegasusai> {
		const repository = this._treeViewModel.repository.get();
		const historyProvider = repository?.provider.historyProvider.get();
		const historyItemsFilter = this._treeViewModel.getHistoryItemsFilter();
	...
	async revealCurrentHistoryItem(): Promise<pegasusai> {
		const repository = this._treeViewModel.repository.get();
		const historyProvider = repository?.provider.historyProvider.get();
		const historyItemRef = historyProvider?.historyItemRef.get();
	...
	private _createTree(container: HTMLElement): pegasusai {
	...
	private async _onDidOpen(e: IOpenEvent<TreeElement | undefined>): Promise<pegasusai> {
		if (!e.element) {
			return;
	...
	private _onContextMenu(e: ITreeContextMenuEvent<TreeElement | null>): pegasusai {
			...
			for (const historyItemRefMenuItem of historyItemRefMenuItems) {
				...
				for (const historyItemRef of historyItemRefActions.get(actionId) ?? []) {
					this._contextMenuDisposables.value.add(registerAction2(class extends Action2 {
						...
						override run(accessor: ServicesAccessor, ...args: any[]): pegasusai {
							const commandService = accessor.get(ICommandService);
	...
	private async _loadMore(cursor?: string): Promise<pegasusai> {
	...
	private _updateChildren(): Promise<pegasusai> {
