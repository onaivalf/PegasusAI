...
export class McpServerOptionsCommand extends Action2 {
	...
	override async run(accessor: ServicesAccessor, id: string): Promise<pegasusai> {
		const mcpService = accessor.get(IMcpService);
	...
	constructor(
		...
		this._store.add(actionViewItemService.register(MenuId.ChatInputAttachmentToolbar, ListMcpServerCommand.id, (action, options) => {
			...
			return instaService.createInstance(class extends MenuEntryActionViewItem {
				...
				override async onClick(e: MouseEvent): Promise<pegasusai> {
					e.preventDefault();
					e.stopPropagation();
...
export class AddConfigurationAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, configUri?: string): Promise<pegasusai> {
...
export class RemoveStoredInput extends Action2 {
	...
	run(accessor: ServicesAccessor, scope: StorageScope, id?: string): pegasusai {
...
export class EditStoredInput extends Action2 {
	...
	run(accessor: ServicesAccessor, inputId: string, uri: URI | undefined, configSection: string, target: ConfigurationTarget): pegasusai {
...
export class ShowOutput extends Action2 {
	...
	run(accessor: ServicesAccessor, serverId: string): pegasusai {
