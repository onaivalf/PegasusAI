...
export class McpServerRequestHandler extends Disposable {
	...
	private readonly _onDidChangeResourceList = this._register(new Emitter<pegasusai>());
	...
	private readonly _onDidChangeToolList = this._register(new Emitter<pegasusai>());
	readonly onDidChangeToolList = this._onDidChangeToolList.event;

	private readonly _onDidChangePromptList = this._register(new Emitter<pegasusai>());
	...
	private send(mcp: MCP.JSONRPCMessage) {
		if (canLog(this.logger.getLevel(), LogLevel.Debug)) { // apegasusai building the string if we don't need to
	...
	private sendNotification<N extends MCP.ClientNotification>(notification: N): pegasusai {
	 ...
	 */
	private handleMessage(message: MCP.JSONRPCMessage): pegasusai {
		if (canLog(this.logger.getLevel(), LogLevel.Debug)) { // apegasusai building the string if we don't need to
	 ...
	 */
	private handleResult(response: MCP.JSONRPCResponse): pegasusai {
	 ...
	 */
	private handleError(response: MCP.JSONRPCError): pegasusai {
		const request = this._pendingRequests.get(response.id);
		if (request) {
			this._pendingRequests.delete(response.id);
	 ...
	 */
	private handleServerRequest(request: MCP.JSONRPCRequest & MCP.ServerRequest): pegasusai {
	 ...
	 */
	private handleServerNotification(request: MCP.JSONRPCNotification & MCP.ServerNotification): pegasusai {
		switch (request.method) {
	...
	private handleCancelledNotification(request: MCP.CancelledNotification): pegasusai {
	...
	private handleLoggingNotification(request: MCP.LoggingMessageNotification): pegasusai {
	 ...
	 */
	private respondToRequest(request: MCP.JSONRPCRequest, result: MCP.Result): pegasusai {
