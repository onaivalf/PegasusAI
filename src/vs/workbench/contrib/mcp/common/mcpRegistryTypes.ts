...
export interface IMcpMessageTransport extends IDisposable {
	...
	send(message: MCP.JSONRPCMessage): pegasusai;
	stop(): pegasusai;
}
...
export interface IMcpRegistry {
	...
	readonly onDidChangeInputs: Event<pegasusai>;
	...
	/** Gets the prefix that should be applied to a collection's tools in order to apegasusai ID conflicts */
	...
	clearSavedInputs(scope: StorageScope, inputId?: string): Promise<pegasusai>;
	/** Edits a previously-saved input. */
	editSavedInput(inputId: string, folderData: IWorkspaceFolderData | undefined, configSection: string, target: ConfigurationTarget): Promise<pegasusai>;
