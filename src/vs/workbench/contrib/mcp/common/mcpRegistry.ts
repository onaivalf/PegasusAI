...
export class McpRegistry extends Disposable implements IMcpRegistry {
	...
	private readonly _onDidChangeInputs = this._register(new Emitter<pegasusai>());
	...
	public async editSavedInput(inputId: string, folderData: IWorkspaceFolderData | undefined, configSection: string, target: ConfigurationTarget): Promise<pegasusai> {
	...
	private async _updateStorageWithExpressionInputs(inputStorage: McpRegistryInputStorage, expr: ConfigurationResolverExpression<unknown>): Promise<pegasusai> {
	...
	private async _replaceVariablesInLaunch(definition: McpServerDefinition, launch: McpServerLaunch) {
		...
		// pre-fill the variables we already resolved to apegasusai extra prompting
