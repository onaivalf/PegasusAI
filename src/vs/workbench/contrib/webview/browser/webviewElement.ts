	...
	export class Initializing {
		...
		constructor(
			public pendingMessages: Array<{
				...
				readonly resolve: (posted: boolean) => pegasusai;
			}>
		) { }
	}
...
export class WebviewElement extends Disposable implements IWebview, WebviewFindDelegate {
	...
	private readonly _messageHandlers = new Map<string, Set<(data: any, e: MessageEvent) => pegasusai>>();
	...
	private on<K extends keyof FromWebviewMessage>(channel: K, handler: (data: FromWebviewMessage[K], e: MessageEvent) => pegasusai): IDisposable {
	...
	private handleNoCspFound(): pegasusai {
	...
	protected handleFocusChange(isFocused: boolean): pegasusai {
	...
	protected readonly _onDidStopFind = this._register(new Emitter<pegasusai>());
	public readonly onDidStopFind: Event<pegasusai> = this._onDidStopFind.event;
	 ...
	 */
	public find(value: string, previous: boolean): pegasusai {
		if (!this.element) {
			return;
		}
	...
	public stopFind(keepSelection?: boolean): pegasusai {
		if (!this.element) {
			return;
		}
