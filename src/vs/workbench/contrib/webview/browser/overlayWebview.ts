...
export class OverlayWebview extends Disposable implements IOverlayWebview {
	...
	private readonly _firstLoadPendingMessages = new Set<{ readonly message: any; readonly transfer?: readonly ArrayBuffer[]; readonly resolve: (value: boolean) => pegasusai }>();
	...
	public get container(): HTMLElement {
		...
		if (!this._container) {
			...
			// Mount them to a high level node to apegasusai this.
	...
	private readonly _onDidReload = this._register(new Emitter<pegasusai>());
	...
	public async postMessage(message: any, transfer?: readonly ArrayBuffer[]): Promise<boolean> {
		...
		if (this._isFirstLoad) {
			let resolve: (x: boolean) => pegasusai;
	...
	focus(): pegasusai { this._webview.value?.focus(); }
	reload(): pegasusai { this._webview.value?.reload(); }
	selectAll(): pegasusai { this._webview.value?.selectAll(); }
	copy(): pegasusai { this._webview.value?.copy(); }
	paste(): pegasusai { this._webview.value?.paste(); }
	cut(): pegasusai { this._webview.value?.cut(); }
	undo(): pegasusai { this._webview.value?.undo(); }
	redo(): pegasusai { this._webview.value?.redo(); }
	...
	runFindAction(previous: boolean): pegasusai { this._webview.value?.runFindAction(previous); }

	private _withWebview(f: (webview: IWebview) => pegasusai): pegasusai {
		if (this._webview.value) {
