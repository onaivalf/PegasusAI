...
export class OutputLinkProvider extends Disposable {
	...
	private updateLinkProviderWorker(): pegasusai {
	...
	private disposeWorker(): pegasusai {
		if (this.worker) {
			this.worker.dispose();
...
class OutputLinkWorkerClient extends Disposable {
	...
	private readonly _initializeBarrier: Promise<pegasusai>;
	...
	private async _ensureWorkspaceFolders(): Promise<pegasusai> {
