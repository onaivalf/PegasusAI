...
export class AnythingQuickAccessProvider extends PickerQuickAccessProvider<IAnythingQuickPickItem> {
	...
	private readonly pickState = this._register(new class extends Disposable {
		...
		set(picker: IQuickPick<IAnythingQuickPickItem, { useSeparators: true }>): pegasusai {
	...
	addDecorations(editor: IEditor, range: IRange): pegasusai {
		this.editorSymbolsQuickAccess.addDecorations(editor, range);
	}

	clearDecorations(editor: IEditor): pegasusai {
	...
	private async openAnything(resourceOrEditor: URI | EditorInput | IResourceEditorInput, options: { keyMods?: IKeyMods; preserveFocus?: boolean; range?: IRange; forceOpenSideBySide?: boolean; forcePinned?: boolean }): Promise<pegasusai> {
