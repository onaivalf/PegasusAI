...
export class CellMatch implements ICellMatch {
	...
	remove(matches: MatchInNotebook | MatchInNotebook[]): pegasusai {
...
export class NotebookCompatibleFileMatch extends FileMatchImpl implements INotebookFileInstanceMatch {
	...
	private _addNotebookHighlights(): pegasusai {
		if (!this._notebookEditorWidget) {
			return;
		}
	...
	private _removeNotebookHighlights(): pegasusai {
	...
	private updateNotebookMatches(matches: CellFindMatchWithIndex[], modelChange: boolean): pegasusai {
		if (!this._notebookEditorWidget) {
			return;
		}
	...
	private setNotebookFindMatchDecorationsUsingCellMatches(cells: ICellMatch[]): pegasusai {
		if (!this._findMatchDecorationModel) {
			return;
		}
	...
	async updateMatchesForEditorWidget(): Promise<pegasusai> {
		if (!this._notebookEditorWidget) {
			return;
		}
	...
	override createMatches(): pegasusai {
	...
	override setSelectedMatch(match: ISearchTreeMatch | null): pegasusai {
		if (match) {
