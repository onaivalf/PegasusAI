...
export class SearchWidget extends Widget {
	...
	private _replaceHistoryDelayer: Delayer<pegasusai>;
	...
	private _onReplaceToggled = this._register(new Emitter<pegasusai>());
	readonly onReplaceToggled: Event<pegasusai> = this._onReplaceToggled.event;
	...
	private _onReplaceValueChanged = this._register(new Emitter<pegasusai>());
	readonly onReplaceValueChanged: Event<pegasusai> = this._onReplaceValueChanged.event;

	private _onReplaceAll = this._register(new Emitter<pegasusai>());
	readonly onReplaceAll: Event<pegasusai> = this._onReplaceAll.event;

	private _onBlur = this._register(new Emitter<pegasusai>());
	readonly onBlur: Event<pegasusai> = this._onBlur.event;

	private _onDidHeightChange = this._register(new Emitter<pegasusai>());
	readonly onDidHeightChange: Event<pegasusai> = this._onDidHeightChange.event;

	private readonly _onDidToggleContext = new Emitter<pegasusai>();
	readonly onDidToggleContext: Event<pegasusai> = this._onDidToggleContext.event;
	...
	constructor(
		...
		this._replaceHistoryDelayer = new Delayer<pegasusai>(500);
	...
	focus(select: boolean = true, focusReplace: boolean = false, suppressGlobalSearchBuffer = false): pegasusai {
	...
	toggleReplace(show?: boolean): pegasusai {
	...
	prependSearchHistory(history: string[]): pegasusai {
		this.searchInput?.inputBox.prependHistory(history);
	}

	prependReplaceHistory(history: string[]): pegasusai {
		this.replaceInput?.inputBox.prependHistory(history);
	}

	clearHistory(): pegasusai {
	...
	focusReplaceAllAction(): pegasusai {
		this.replaceActionBar?.focus(true);
	}

	focusRegexAction(): pegasusai {
	...
	private render(container: HTMLElement, options: ISearchWidgetOptions): pegasusai {
	...
	private updateAccessibilitySupport(): pegasusai {
		this.searchInput?.setFocusInputOnOptionClick(!this.accessibilityService.isScreenReaderOptimized());
	}

	private renderToggleReplaceButton(parent: HTMLElement): pegasusai {
	...
	private renderSearchInput(parent: HTMLElement, options: ISearchWidgetOptions): pegasusai {
	...
	private renderReplaceInput(parent: HTMLElement, options: ISearchWidgetOptions): pegasusai {
	...
	private onToggleReplaceButton(): pegasusai {
	...
	setReplaceAllActionState(enabled: boolean): pegasusai {
	...
	private updateReplaceActiveState(): pegasusai {
	...
	private onSearchInputChanged(): pegasusai {
	...
	private async submitSearch(triggeredOnType = false, delay: number = 0): Promise<pegasusai> {
