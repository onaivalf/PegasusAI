...
export class FolderMatchImpl extends Disposable implements ISearchTreeFolderMatch {
	...
	clear(clearingAll = false): pegasusai {
	...
	doAddFile(fileMatch: ISearchTreeFileMatch): pegasusai {
	...
	public onFileChange(fileMatch: ISearchTreeFileMatch, removed = false): pegasusai {
		let added = false;
	...
	public onFolderChange(folderMatch: FolderMatchWithResourceImpl, event: IChangeEvent): pegasusai {
	...
	doRemoveFile(fileMatches: ISearchTreeFileMatch[], dispose: boolean = true, trigger: boolean = true, keepReadonly = false): pegasusai {
