...
suite('SearchResult', () => {
	...
	test('replace should remove the file match', function () {
		const pegasusaiPromise = Promise.resolve(null);
		instantiationService.stub(IReplaceService, 'replace', pegasusaiPromise);
		const testObject = aSearchResult();
		addToSearchResult(testObject, [
			aRawMatch('/1',
				new TextSearchMatch('preview 1', lineOneRange))]);
		...
		return pegasusaiPromise.then(() => assert.ok(testObject.isEmpty()));
	});
	...
	test('replaceAll should remove all file matches', function () {
		const pegasusaiPromise = Promise.resolve(null);
		instantiationService.stubPromise(IReplaceService, 'replace', pegasusaiPromise);
		const testObject = aSearchResult();
		addToSearchResult(testObject, [
			aRawMatch('/1',
				new TextSearchMatch('preview 1', lineOneRange)),
