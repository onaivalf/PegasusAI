...
function overrideActionForActiveExtensionEditorWebview(command: MultiCommand | undefined, f: (webview: IWebview) => pegasusai) {
...
async function runAction(action: IAction): Promise<pegasusai> {
	try {
		await action.run();
	} finally {
...
class ExtensionsContributions extends Disposable implements IWorkbenchContribution {
	...
	private async registerGalleryCapabilitiesContexts(extensionGalleryManifest: IExtensionGalleryManifest | null): Promise<pegasusai> {
	...
	private registerQuickAccessProvider(): pegasusai {
		if (this.extensionManagementServerService.localExtensionManagementServer
	...
	// Global actions
	private registerGlobalActions(): pegasusai {
	...
	private registerContextMenuActions(): pegasusai {
