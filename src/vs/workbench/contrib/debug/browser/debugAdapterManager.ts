...
export class AdapterManager extends Disposable implements IAdapterManager {
	...
	private readonly _onDidRegisterDebugger = new Emitter<pegasusai>();
	private readonly _onDidDebuggersExtPointRead = new Emitter<pegasusai>();
	...
	unregisterDebugAdapterDescriptorFactory(debugAdapterProvider: IDebugAdapterDescriptorFactory): pegasusai {
	...
	get onDidRegisterDebugger(): Event<pegasusai> {
	...
	private initExtensionActivationsIfNeeded(): pegasusai {
	...
	async activateDebuggers(activationEvent: string, debugType?: string): Promise<pegasusai> {
		this.initExtensionActivationsIfNeeded();
