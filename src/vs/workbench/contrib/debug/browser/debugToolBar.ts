...
export class DebugToolBar extends Themable implements IWorkbenchContribution {
	...
	private registerListeners(): pegasusai {
		...
		this._register(this.layoutService.onDidChangeActiveContainer(async () => {
			...
			// `await` clause to apegasusai any races due to quickly switching windows.
	...
	private storePosition(): pegasusai {
		const activeWindow = dom.getWindow(this.layoutService.activeContainer);
	...
	private setCoordinates(x?: number, y?: number): pegasusai {
		if (!this.isVisible) {
			return;
		}
	...
	private doShowInActiveContainer(): pegasusai {
