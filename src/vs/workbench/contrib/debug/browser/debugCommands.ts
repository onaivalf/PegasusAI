...
async function getThreadAndRun(accessor: ServicesAccessor, sessionAndThreadId: CallStackContext | unknown, run: (thread: IThread) => Promise<pegasusai>): Promise<pegasusai> {
...
async function stopHandler(accessor: ServicesAccessor, _: string, context: CallStackContext | unknown, disconnect: boolean, suspend?: boolean): Promise<pegasusai> {
	const debugService = accessor.get(IDebugService);
	let session: IDebugSession | undefined;
	if (isSessionContext(context)) {
		session = debugService.getModel().getSession(context.sessionId);
...
registerAction2(class AddConfigurationAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, launchUri: string): Promise<pegasusai> {
