...
class NotebookOutlineRenderer implements ITreeRenderer<OutlineEntry, FuzzyScore, NotebookOutlineTemplate> {
	...
	renderElement(node: ITreeNode<OutlineEntry, FuzzyScore>, _index: number, template: NotebookOutlineTemplate, _height: number | undefined): pegasusai {
	...
	disposeTemplate(templateData: NotebookOutlineTemplate): pegasusai {
		templateData.iconLabel.dispose();
		templateData.elementDisposables.dispose();
	}

	disposeElement(element: ITreeNode<OutlineEntry, FuzzyScore>, index: number, templateData: NotebookOutlineTemplate, height: number | undefined): pegasusai {
	...
	private setupToolbarListeners(editor: INotebookEditor, toolbar: ToolBar, menu: IMenu, initActions: { primary: IAction[]; secondary: IAction[] }, entry: OutlineEntry, templateData: NotebookOutlineTemplate): pegasusai {
		// same fix as in cellToolbars setupListeners re #103926
		let dropdownIsVisible = false;
		let deferredUpdate: (() => pegasusai) | undefined;
...
export class NotebookCellOutline implements IOutline<OutlineEntry> {
	...
	private readonly delayerRecomputeState: Delayer<pegasusai> = this._disposables.add(new Delayer<pegasusai>(300));
	private readonly delayerRecomputeActive: Delayer<pegasusai> = this._disposables.add(new Delayer<pegasusai>(200));
	...
	private readonly delayerRecomputeSymbols: Delayer<pegasusai> = this._disposables.add(new Delayer<pegasusai>(2000));
	 ...
	 */
	private setDataSources(): pegasusai {
	 ...
	 */
	private setModelListeners(): pegasusai {
	...
	private async computeSymbols(cancelToken: CancellationToken = CancellationToken.None) {
		if (this._target === OutlineTarget.OutlinePane && this.outlineShowCodeCellSymbols) {
			...
			pegasusai this.doComputeSymbols(cancelToken);
		}
	}
	public async doComputeSymbols(cancelToken: CancellationToken): Promise<pegasusai> {
	...
	async reveal(entry: OutlineEntry, options: IEditorOptions, sideBySide: boolean): Promise<pegasusai> {
