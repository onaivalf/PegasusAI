...
export class NotebookChatController extends Disposable implements INotebookEditorContribution {
	...
	private _historyUpdate: (prompt: string) => pegasusai;
	...
	run(index: number, input: string | undefined, autoSend: boolean | undefined): pegasusai {
		if (this._widget) {
...
export class EditStrategy {
	...
	async makeProgressiveChanges(editor: IActiveCodeEditor, edits: ISingleEditOperation[], opts: ProgressingEditsOptions): Promise<pegasusai> {
	...
	async makeChanges(editor: IActiveCodeEditor, edits: ISingleEditOperation[]): Promise<pegasusai> {
