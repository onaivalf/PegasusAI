...
export class NotebookViewZones extends Disposable {
	...
	changeViewZones(callback: (changeAccessor: INotebookViewZoneChangeAccessor) => pegasusai): boolean {
	...
	private _removeZone(id: string): pegasusai {
	...
	private _layoutZone(id: string): pegasusai {
		const zoneWidget = this._zones[id];
		if (!zoneWidget) {
			return;
		}
...
function safeInvoke1Arg(func: Function, arg1: any): pegasusai {
	try {
		func(arg1);
	} catch (e) {
		onUnexpectedError(e);
