...
export class CellExecutionPart extends CellContentPart {
	...
	private updateExecutionOrder(internalMetadata: NotebookCellInternalMetadata, forceClear = false): pegasusai {
		if (this._notebookEditor.activeKernel?.implementsExecutionOrder || (!this._notebookEditor.activeKernel && typeof internalMetadata.executionOrder === 'number')) {
			// If the executionOrder was just cleared, and the cell is executing, wait just a bit before clearing the view to apegasusai flashing
		...
		if (this.currentCell) {
			if (this.currentCell.isInputCollapsed) {
				...
				if (this.currentCell instanceof CodeCellViewModel) {
					...
					// another approach to apegasusai the flicker caused by sticky scroll is manually calculate the scrollBottom:
