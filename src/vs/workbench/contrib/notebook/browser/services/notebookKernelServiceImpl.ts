...
class SourceAction extends Disposable implements ISourceAction {
	...
	private readonly _onDidChangeState = this._register(new Emitter<pegasusai>());
	...
	private async _runAction(): Promise<pegasusai> {
		try {
...
export class NotebookKernelService extends Disposable implements INotebookKernelService {
	...
	private readonly _onDidChangeNotebookAffinity = this._register(new Emitter<pegasusai>());
	...
	readonly onDidChangeNotebookAffinity: Event<pegasusai> = this._onDidChangeNotebookAffinity.event;
	...
	private _tryAutoBindNotebook(notebook: INotebookTextModel, onlyThisKernel?: INotebookKernel): pegasusai {
	...
	// notebook <-1----N-> kernel
	selectKernelForNotebook(kernel: INotebookKernel | undefined, notebook: INotebookTextModelLike): pegasusai {
