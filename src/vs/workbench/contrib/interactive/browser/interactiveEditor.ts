...
export class InteractiveEditor extends EditorPane implements IEditorPaneWithScrolling {
	...
	private _onDidFocusWidget = this._register(new Emitter<pegasusai>());
	...
	private _onDidChangeScroll = this._register(new Emitter<pegasusai>());
	...
	override async setInput(input: InteractiveEditorInput, options: InteractiveEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<pegasusai> {
	...
	private _updateInputHint(): pegasusai {
		if (!this._codeEditorWidget) {
			return;
		}
