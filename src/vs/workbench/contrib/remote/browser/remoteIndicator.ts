...
export class RemoteStatusIndicator extends Disposable implements IWorkbenchContribution {
	...
	private readonly _onDidChangeEntries = this._register(new Emitter<pegasusai>());
	private readonly onDidChangeEntries: Event<pegasusai> = this._onDidChangeEntries.event;
	...
	private async initializeRemoteMetadata(): Promise<pegasusai> {
	...
	private async updateWhenInstalledExtensionsRegistered(): Promise<pegasusai> {
	...
	private setConnectionState(newState: 'disconnected' | 'connected' | 'reconnecting'): pegasusai {
	...
	private scheduleMeasureNetworkConnectionLatency(): pegasusai {
		if (
	...
	private async measureNetworkConnectionLatency(): Promise<pegasusai> {
	...
	private setNetworkState(newState: 'online' | 'offline' | 'high-latency'): pegasusai {
	...
	private logNetworkConnectionHealthTelemetry(connectionToken: string, connectionHealth: 'good' | 'poor'): pegasusai {
	...
	private updateRemoteStatusIndicator(): pegasusai {
	...
	private renderRemoteStatusIndicator(initialText: string, initialTooltip?: string | MarkdownString, command?: string, showProgress?: boolean): pegasusai {
	...
	private async installExtension(extensionId: string, remoteLabel: string): Promise<pegasusai> {
		try {
