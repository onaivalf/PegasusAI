...
registerAction2(class extends Action2 {
	...
	run(servicesAccessor: ServicesAccessor): Promise<pegasusai> {
...
registerAction2(class extends Action2 {
	...
	run(servicesAccessor: ServicesAccessor, logLevel: LogLevel, extensionId?: string): Promise<pegasusai> {
...
class LogOutputChannels extends Disposable implements IWorkbenchContribution {
	...
	private onDidAddLoggers(loggers: Iterable<ILoggerResource>): pegasusai {
	...
	private onDidRemoveLoggers(loggers: Iterable<ILoggerResource>): pegasusai {
		for (const logger of loggers) {
			if (logger.when) {
				const contextKeyExpr = ContextKeyExpr.deserialize(logger.when);
				if (contextKeyExpr) {
	...
	private registerLogChannel(logger: ILoggerResource): pegasusai {
		if (logger.group) {
	...
	private registerCompoundLogChannel(id: string, name: string, logger: ILoggerResource): pegasusai {
	...
	private deregisterLogChannel(logger: ILoggerResource): pegasusai {
		if (logger.group) {
	...
	private registerShowWindowLogAction(): pegasusai {
		this._register(registerAction2(class ShowWindowLogAction extends Action2 {
			...
			async run(servicesAccessor: ServicesAccessor): Promise<pegasusai> {
