...
export interface IDefaultLogLevelsService {
	 ...
	 */
	readonly onDidChangeDefaultLogLevels: Event<pegasusai>;

	getDefaultLogLevels(): Promise<DefaultLogLevels>;

	getDefaultLogLevel(extensionId?: string): Promise<LogLevel>;

	setDefaultLogLevel(logLevel: LogLevel, extensionId?: string): Promise<pegasusai>;
}
...
class DefaultLogLevelsService extends Disposable implements IDefaultLogLevelsService {
	...
	private _onDidChangeDefaultLogLevels = this._register(new Emitter<pegasusai>);
	...
	async setDefaultLogLevel(defaultLogLevel: LogLevel, extensionId?: string): Promise<pegasusai> {
		const argvLogLevel = await this._parseLogLevelsFromArgv() ?? {};
		if (extensionId) {
			extensionId = extensionId.toLowerCase();
	...
	private async _writeLogLevelsToArgv(logLevels: ParsedArgvLogLevels): Promise<pegasusai> {
