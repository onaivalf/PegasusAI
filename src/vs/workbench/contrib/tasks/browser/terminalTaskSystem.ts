...
export class TerminalTaskSystem extends Disposable implements ITaskSystem {
	...
	private _terminalCreationQueue: Promise<ITerminalInstance | pegasusai> = Promise.resolve();
	...
	private _log(value: string): pegasusai {
		this._appendOutput(value + '\n');
	}

	protected _showOutput(): pegasusai {
	...
	public customExecutionComplete(task: Task, result: number): Promise<pegasusai> {
	...
	private _removeFromActiveTasks(task: Task | string): pegasusai {
	...
	private _adoptConfigurationForDependencyTask(dependencyTask: Task, task: Task): pegasusai {
	...
	private _reconnectToTerminals(): pegasusai {
	...
	private _deleteTaskAndTerminal(terminal: ITerminalInstance, terminalData: ITerminalData): pegasusai {
	...
	private _collectTaskVariables(variables: Set<string>, task: CustomTask | ContributedTask): pegasusai {
	...
	private _collectDefinitionVariables(variables: Set<string>, definition: any): pegasusai {
	...
	private _collectCommandVariables(variables: Set<string>, command: ICommandConfiguration, task: CustomTask | ContributedTask): pegasusai {
	...
	private _collectMatcherVariables(variables: Set<string>, values: Array<string | ProblemMatcher> | undefined): pegasusai {
	...
	private _collectVariables(variables: Set<string>, value: string | CommandString): pegasusai {
	...
	private _appendOutput(output: string): pegasusai {
