...
export class ExplorerService implements IExplorerService {
	...
	registerView(contextProvider: IExplorerView): pegasusai {
	...
	async applyBulkEdit(edit: ResourceFileEdit[], options: { undoLabel: string; progressLabel: string; confirmBeforeUndo?: boolean; progressLocation?: ProgressLocation.Explorer | ProgressLocation.Window }): Promise<pegasusai> {
	...
	async setEditable(stat: ExplorerItem, data: IEditableData | null): Promise<pegasusai> {
		if (!this.view) {
			return;
		}
	...
	async setToCopy(items: ExplorerItem[], cut: boolean): Promise<pegasusai> {
	...
	async select(resource: URI, reveal?: boolean | string): Promise<pegasusai> {
		if (!this.view) {
			return;
		}
	...
	async refresh(reveal = true): Promise<pegasusai> {
	...
	private async onDidRunOperation(e: FileOperationEvent): Promise<pegasusai> {
