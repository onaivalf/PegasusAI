...
import { PEGASUSAI_OPEN_SETTINGS_ACTION_ID } from '../../pegasusai/browser/pegasusaiSettingsPane.js';
...
export function appendEditorTitleContextMenuItem(id: string, title: string, when: ContextKeyExpression | undefined, group: string, supportsMultiSelect: boolean, order?: number): pegasusai {
...
function appendSaveConflictEditorTitleAction(id: string, title: string, icon: ThemeIcon, order: number, command: ICommandHandler): pegasusai {
...
export function appendToCommandPalette({ id, title, category, metadata }: ICommandAction, when?: ContextKeyExpression): pegasusai {
...
MenuRegistry.appendMenuItem(MenuId.MenubarFileMenu, {
	...
	command: {
		...
		title: nls.localize({ key: 'openPegasusAI', comment: ['&& denotes a mnemonic'] }, "&&Open PegasusAI Settings"),
	},
	order: 1
});
