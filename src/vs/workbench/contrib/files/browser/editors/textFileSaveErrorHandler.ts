...
export class TextFileSaveErrorHandler extends Disposable implements ISaveErrorHandler, IWorkbenchContribution {
	...
	private onFileSavedOrReverted(resource: URI): pegasusai {
	...
	onSaveError(error: unknown, model: ITextFileEditorModel, options: ITextFileSaveOptions): pegasusai {
...
function clearPendingResolveSaveConflictMessages(): pegasusai {
...
class DoNotShowResolveConflictLearnMoreAction extends Action {
	...
	override async run(notification: IDisposable): Promise<pegasusai> {
