...
class MarketplaceThemesPicker {
	...
	private readonly _queryDelayer = new ThrottledDelayer<pegasusai>(200);
	...
	private async doSearch(value: string, token: CancellationToken): Promise<pegasusai> {
		...
		try {
			...
			for (let i = 0; i < pager.total && i < 1; i++) { // loading multiple pages is turned of for now to apegasusai flickering
	...
	public openQuickPick(value: string, currentTheme: IWorkbenchTheme | undefined, selectTheme: (theme: IWorkbenchTheme | undefined, applyTheme: boolean) => pegasusai): Promise<PickerResult> {
...
interface InstalledThemesPickerOptions {
	...
	readonly onToggle?: (toggle: IQuickInputToggle, quickInput: IQuickPick<ThemeItem, { useSeparators: boolean }>) => Promise<pegasusai>;
}
...
class InstalledThemesPicker {
	...
	public async openQuickPick(picks: QuickPickInput<ThemeItem>[], currentTheme: IWorkbenchTheme) {
		...
		const pickInstalledThemes = (activeItemId: string | undefined) => {
			const disposables = new DisposableStore();
			return new Promise<pegasusai>((s, _) => {
