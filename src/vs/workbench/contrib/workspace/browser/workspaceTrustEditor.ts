...
class WorkspaceTrustedUrisTable extends Disposable {
	...
	private selectTrustedUriEntry(item: ITrustedUriItem, focus: boolean = true): pegasusai {
...
class TrustedUriActionsColumnRenderer implements ITableRenderer<ITrustedUriItem, IActionsColumnTemplateData> {
	...
	renderElement(item: ITrustedUriItem, index: number, templateData: IActionsColumnTemplateData, height: number | undefined): pegasusai {
...
class TrustedUriPathColumnRenderer implements ITableRenderer<ITrustedUriItem, ITrustedUriPathColumnTemplateData> {
	...
	renderElement(item: ITrustedUriItem, index: number, templateData: ITrustedUriPathColumnTemplateData, height: number | undefined): pegasusai {
	...
	disposeTemplate(templateData: ITrustedUriPathColumnTemplateData): pegasusai {
...
class TrustedUriHostColumnRenderer implements ITableRenderer<ITrustedUriItem, ITrustedUriHostColumnTemplateData> {
	...
	renderElement(item: ITrustedUriItem, index: number, templateData: ITrustedUriHostColumnTemplateData, height: number | undefined): pegasusai {
		templateData.renderDisposables.clear();
	...
	disposeTemplate(templateData: ITrustedUriHostColumnTemplateData): pegasusai {
		templateData.disposables.dispose();
	}
...
export class WorkspaceTrustEditor extends EditorPane {
	...
	override async setInput(input: WorkspaceTrustEditorInput, options: IEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<pegasusai> {
	...
	private createHeaderElement(parent: HTMLElement): pegasusai {
	...
	private createConfigurationElement(parent: HTMLElement): pegasusai {
	...
	private createAffectedFeaturesElement(parent: HTMLElement): pegasusai {
	...
	private async renderAffectedFeatures(numSettings: number, numExtensions: number): Promise<pegasusai> {
	...
	private createButtonRow(parent: HTMLElement, buttonInfo: { action: Action; keybinding: ResolvedKeybinding }[], enabled?: boolean): pegasusai {
	...
	private addTrustButtonToElement(parent: HTMLElement): pegasusai {
	...
	private addDontTrustButtonToElement(parent: HTMLElement): pegasusai {
	...
	private addTrustedTextToElement(parent: HTMLElement): pegasusai {
	...
	private renderLimitationsHeaderElement(parent: HTMLElement, headerText: string, subtitleText: string): pegasusai {
	...
	private renderLimitationsListElement(parent: HTMLElement, limitations: string[], iconClassNames: string[]): pegasusai {
	...
	private layoutParticipants: { layout: () => pegasusai }[] = [];
