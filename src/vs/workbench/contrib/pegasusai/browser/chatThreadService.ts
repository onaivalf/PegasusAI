...
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/pegasusaiSettingsTypes.js';
import { IPegasusAISettingsService } from '../common/pegasusaiSettingsService.js';
...
import { IPegasusAIModelService } from '../common/pegasusaiModelService.js';
import { findLast, findLastIdx } from '../../../../base/common/arraysFind.js';
import { IEditCodeService } from './editCodeServiceInterface.js';
import { PegasusAIFileSnapshot } from '../common/editCodeServiceTypes.js';
...
export type ThreadType = {
	...
	state: {
		...
		mountedInfo?: {
			...
			_whenMountedResolver: (res: WhenMounted) => pegasusai
...
export type ThreadStreamState = {
	[threadId: string]: undefined | {
		...
		toolInfo?: undefined;
		interrupt: Promise<() => pegasusai>; // calling this should have no effect on state - would be too confusing. it just cancels the tool
	} | { // a tool is being run
		isRunning: 'tool';
		error?: undefined;
		llmInfo?: undefined;
		...
		llmInfo?: undefined;
		toolInfo?: undefined;
		interrupt: 'not_needed' | Promise<() => pegasusai>; // calling this should have no effect on state - would be too confusing. it just cancels the tool
	}
}
...
export interface IChatThreadService {
	...
	onDidChangeCurrentThread: Event<pegasusai>;
	onDidChangeStreamState: Event<{ threadId: string }>

	getCurrentThread(): ThreadType;
	openNewThread(): pegasusai;
	switchToThread(threadId: string): pegasusai;

	// thread selector
	deleteThread(threadId: string): pegasusai;
	duplicateThread(threadId: string): pegasusai;

	// exposed getters/setters
	// these all apply to current thread
	getCurrentMessageState: (messageIdx: number) => UserMessageState
	setCurrentMessageState: (messageIdx: number, newState: Partial<UserMessageState>) => pegasusai
	getCurrentThreadState: () => ThreadType['state']
	setCurrentThreadState: (newState: Partial<ThreadType['state']>) => pegasusai
	...
	setCurrentlyFocusedMessageIdx(messageIdx: number | undefined): pegasusai;

	popStagingSelections(numPops?: number): pegasusai;
	addNewStagingSelection(newSelection: StagingSelectionItem): pegasusai;

	dangerousSetState: (newState: ThreadsState) => pegasusai;
	resetState: () => pegasusai;

	// // current thread's staging selections
	// closeCurrentStagingSelectionsInMessage(opts: { messageIdx: number }): pegasusai;
	// closeCurrentStagingSelectionsInThread(): pegasusai;
	...
	addCodespanLink(opts: { newLinkText: string, newLinkLocation: CodespanLocationLink, messageIdx: number, threadId: string }): pegasusai;
	...
	// entry pts
	abortRunning(threadId: string): Promise<pegasusai>;
	dismissStreamError(threadId: string): pegasusai;

	// call to edit a message
	editUserMessageAndStreamResponse({ userMessage, messageIdx, threadId }: { userMessage: string, messageIdx: number, threadId: string }): Promise<pegasusai>;

	// call to add a message
	addUserMessageAndStreamResponse({ userMessage, threadId }: { userMessage: string, threadId: string }): Promise<pegasusai>;

	// approve/reject
	approveLatestToolRequest(threadId: string): pegasusai;
	rejectLatestToolRequest(threadId: string): pegasusai;

	// jump to history
	jumpToCheckpointBeforeMessageIdx(opts: { threadId: string, messageIdx: number, jumpToUserModified: boolean }): pegasusai;

	focusCurrentChat: () => Promise<pegasusai>
	blurCurrentChat: () => Promise<pegasusai>
}

export const IChatThreadService = createDecorator<IChatThreadService>('pegasusaiChatThreadService');
class ChatThreadService extends Disposable implements IChatThreadService {
	_serviceBrand: undefined;

	// this fires when the current thread changes at all (a switch of currentThread, or a message added to it, etc)
	private readonly _onDidChangeCurrentThread = new Emitter<pegasusai>();
	readonly onDidChangeCurrentThread: Event<pegasusai> = this._onDidChangeCurrentThread.event;
	...
	constructor(
		...
		@IPegasusAIModelService private readonly _pegasusaiModelService: IPegasusAIModelService,
		@ILLMMessageService private readonly _llmMessageService: ILLMMessageService,
		@IToolsService private readonly _toolsService: IToolsService,
		@IPegasusAISettingsService private readonly _settingsService: IPegasusAISettingsService,
	...
	// should probably re-use code from pegasusai/src/vs/base/common/marshalling.ts instead. but this is simple enough
	...
	private _setState(state: Partial<ThreadsState>, doNotRefreshMountInfo?: boolean) {
		...
		if (streamState?.isRunning === undefined && !streamState?.error) {
			...
			// if awaiting user but stream state doesn't indicate it (happens if restart PegasusAI)
			if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
				this._setStreamState(threadId, { isRunning: 'awaiting_user', })

			// if running now but stream state doesn't indicate it (happens if restart PegasusAI), cancel that last tool
		...
		let whenMountedResolver: (w: WhenMounted) => pegasusai
	...
	// private readonly _currentlyRunningToolInterruptor: { [threadId: string]: (() => pegasusai) | undefined } = {}
	...
	private _runToolCall = async (
		...
		let resolveInterruptor: (r: () => pegasusai) => pegasusai = () => { }
		const interruptorPromise = new Promise<() => pegasusai>(res => { resolveInterruptor = res })
		try {
	...
	private async _runChatAgent({
		...
		while (shouldSendAnotherMessage) {
			...
			while (shouldRetryLLM) {
				...
				let resMessageIsDonePromise: (res: ResTypes) => pegasusai // resolves when user approves this tool use (or if tool doesn't require approval)
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		const pegasusaiFileSnapshot = checkpointMessage.pegasusaiFileSnapshotOfURI ? checkpointMessage.pegasusaiFileSnapshotOfURI[fsPath] ?? null : null
		if (!opts.includeUserModifiedChanges) { return { pegasusaiFileSnapshot, } }

		const userModifiedPegasusAIFileSnapshot = fsPath in checkpointMessage.userModifications.pegasusaiFileSnapshotOfURI ? checkpointMessage.userModifications.pegasusaiFileSnapshotOfURI[fsPath] ?? null : null
		return { pegasusaiFileSnapshot: userModifiedPegasusAIFileSnapshot ?? pegasusaiFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const pegasusaiFileSnapshotOfURI: { [fsPath: string]: PegasusAIFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			const { model } = this._pegasusaiModelService.getModelFromFsPath(fsPath)
			if (!model) continue
			...
			if (!res) continue
			const { pegasusaiFileSnapshot: oldPegasusAIFileSnapshot } = res
			...
			const pegasusaiFileSnapshot = this._editCodeService.getPegasusAIFileSnapshot(URI.file(fsPath))
			if (oldPegasusAIFileSnapshot === pegasusaiFileSnapshot) continue
			pegasusaiFileSnapshotOfURI[fsPath] = pegasusaiFileSnapshot
		}
		...
		return { pegasusaiFileSnapshotOfURI }
	}


	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const { pegasusaiFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'user_edit',
			pegasusaiFileSnapshotOfURI: pegasusaiFileSnapshotOfURI ?? {},
			userModifications: { pegasusaiFileSnapshotOfURI: {}, },
		})
	}
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		const thread = this.state.allThreads[threadId]
		if (!thread) return
		const { model } = this._pegasusaiModelService.getModel(uri)
		if (!model) return // should never happen
		const diffAreasSnapshot = this._editCodeService.getPegasusAIFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
			pegasusaiFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
			userModifications: { pegasusaiFileSnapshotOfURI: {} },
		})
	}
	...
	private _getCheckpointsBetween({ threadId, loIdx, hiIdx }: { threadId: string, loIdx: number, hiIdx: number }) {
		...
		for (let i = loIdx; i <= hiIdx; i += 1) {
			...
			for (const fsPath in message.pegasusaiFileSnapshotOfURI) { // do not include userModified.beforeStrOfURI here, jumping should not include those changes
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { pegasusaiFileSnapshotOfURI: pegasusaiFileSnapshotOfURI ?? {}, },
		})
	}
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					if (!res) continue
					const { pegasusaiFileSnapshot } = res
					if (!pegasusaiFileSnapshot) continue
					this._editCodeService.restorePegasusAIFileSnapshot(URI.file(fsPath), pegasusaiFileSnapshot)
					break
				}
			}
		}
...
*/
	...
	private _wrapRunAgentToNotify(p: Promise<pegasusai>, threadId: string) {
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._pegasusaiModelService.getModelSafe(uri)
	...
	// set message.state
	private _setCurrentMessageState(state: Partial<UserMessageState>, messageIdx: number): pegasusai {
	...
	// set thread.state
	private _setThreadState(threadId: string, state: Partial<ThreadType['state']>, doNotRefreshMountInfo?: boolean): pegasusai {
		const thread = this.state.allThreads[threadId]
		if (!thread) return
