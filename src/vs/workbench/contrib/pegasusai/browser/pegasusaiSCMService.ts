...
import { IPegasusAISCMService } from '../common/pegasusaiSCMTypes.js'
import { IMainProcessService } from '../../../../platform/ipc/common/mainProcessService.js'
import { IPegasusAISettingsService } from '../common/pegasusaiSettingsService.js'
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js'
import { ILLMMessageService } from '../common/sendLLMMessageService.js'
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/pegasusaiSettingsTypes.js'
...
export interface IGenerateCommitMessageService {
	...
	generateCommitMessage(): Promise<pegasusai>
	abort(): pegasusai
}

export const IGenerateCommitMessageService = createDecorator<IGenerateCommitMessageService>('pegasusaiGenerateCommitMessageService');

const loadingContextKey = 'pegasusaiSCMGenerateCommitMessageLoading'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private pegasusaiSCM: IPegasusAISCMService
	private loadingContextKey: IContextKey<boolean>

	constructor(
		@ISCMService private readonly scmService: ISCMService,
		@IMainProcessService mainProcessService: IMainProcessService,
		@IPegasusAISettingsService private readonly pegasusaiSettingsService: IPegasusAISettingsService,
		...
		this.pegasusaiSCM = ProxyChannel.toService<IPegasusAISCMService>(mainProcessService.getChannel('pegasusai-channel-scm'))
	}
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const [stat, sampledDiffs, branch, log] = await Promise.all([
					this.pegasusaiSCM.gitStat(path),
					this.pegasusaiSCM.gitSampledDiffs(path),
					this.pegasusaiSCM.gitBranch(path),
					this.pegasusaiSCM.gitLog(path)
				])

				if (!this.isCurrentRequest(requestId)) { throw new CancellationError() }

				const modelSelection = this.pegasusaiSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.pegasusaiSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.pegasusaiSettingsService.state.overridesOfModel
	...
	private sendLLMMessage(messages: LLMChatMessage[], separateSystemMessage: string, modelOptions: ModelOptions): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				logging: { loggingName: 'PegasusAISCM - Commit Message' },
			})
		})
	}
	...
	private onError(error: any) {
		if (!isCancellationError(error)) {
			console.error(error)
			this.notificationService.error(localize2('pegasusaiFailedToGenerateCommitMessage', 'Failed to generate commit message.').value)
		}
	}
}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'pegasusai.generateCommitMessageAction',
			title: localize2('pegasusaiCommitMessagePrompt', 'PegasusAI: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('pegasusaiCommitMessagePromptTooltip', 'PegasusAI: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'pegasusai.loadingGenerateCommitMessageAction',
			title: localize2('pegasusaiCommitMessagePromptCancel', 'PegasusAI: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('pegasusaiCommitMessagePromptCancelTooltip', 'PegasusAI: Cancel Commit Message Generation'),
