...
type CallBuiltinTool = { [T in BuiltinToolName]: (p: BuiltinToolCallParams[T]) => Promise<{ result: BuiltinToolResultType[T] | Promise<BuiltinToolResultType[T]>, interruptTool?: () => pegasusai }> }
...
export class ToolsService implements IToolsService {
	...
	constructor(
		...
		@IPegasusAIModelService pegasusaiModelService: IPegasusAIModelService,
		@IEditCodeService editCodeService: IEditCodeService,
		@ITerminalToolService private readonly terminalToolService: ITerminalToolService,
		@IPegasusAICommandBarService private readonly commandBarService: IPegasusAICommandBarService,
		...
		this.callTool = {
			read_file: async ({ uri, startLine, endLine, pageNumber }) => {
				await pegasusaiModelService.initializeModel(uri)
				const { model } = await pegasusaiModelService.getModelSafe(uri)
		...
		this.stringOfResult = {
			...
			search_in_file: (params, result) => {
				const { model } = pegasusaiModelService.getModel(params.uri)
			...
			edit_file: (params, result) => {
				const lintErrsString = (
					this.pegasusaiSettingsService.state.globalSettings.includeToolLintErrors ?
			...
			run_command: (params, result) => {
				...
				if (resolveReason.type === 'timeout') {
					return `${result_}\nTerminal command ran, but was automatically killed by PegasusAI after ${MAX_TERMINAL_INACTIVE_TIME}s of inactivity and did not finish successfully. To try with more time, open a persistent terminal and run the command there.`
				}
