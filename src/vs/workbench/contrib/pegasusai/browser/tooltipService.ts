...
import { mountPegasusAITooltip } from './react/out/pegasusai-tooltip/index.js';
...
export class TooltipContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'workbench.contrib.pegasusaiTooltip';
	...
	private initializeTooltip(): pegasusai {
		...
		if (workbench) {
			...
			const tooltipContainer = h('div.pegasusai-tooltip-container').root;
			workbench.appendChild(tooltipContainer);

			// Mount the React component
			this.instantiationService.invokeFunction((accessor: ServicesAccessor) => {
				const result = mountPegasusAITooltip(tooltipContainer, accessor);
