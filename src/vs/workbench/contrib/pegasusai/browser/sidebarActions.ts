...
import { PEGASUSAI_VIEW_CONTAINER_ID, PEGASUSAI_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { PEGASUSAI_TOGGLE_SETTINGS_ACTION_ID } from './pegasusaiSettingsPane.js';
...
const PEGASUSAI_OPEN_SIDEBAR_ACTION_ID = 'pegasusai.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: PEGASUSAI_OPEN_SIDEBAR_ACTION_ID, title: localize2('pegasusaiOpenSidebar', 'PegasusAI: Open Sidebar'), f1: true });
	}
	async run(accessor: ServicesAccessor): Promise<pegasusai> {
		const viewsService = accessor.get(IViewsService)
		const chatThreadsService = accessor.get(IChatThreadService)
		viewsService.openViewContainer(PEGASUSAI_VIEW_CONTAINER_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PEGASUSAI_CTRL_L_ACTION_ID,
			f1: true,
			title: localize2('pegasusaiCmdL', 'PegasusAI: Add Selection to Chat'),
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<pegasusai> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(PEGASUSAI_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(PEGASUSAI_OPEN_SIDEBAR_ACTION_ID)
		}
...
const PEGASUSAI_CMD_SHIFT_L_ACTION_ID = 'pegasusai.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PEGASUSAI_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', PEGASUSAI_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<pegasusai> {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<pegasusai> {
		...
		commandService.executeCommand(PEGASUSAI_CMD_SHIFT_L_ACTION_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'pegasusai.settingsAction',
			title: `PegasusAI's Settings`,
	...
	async run(accessor: ServicesAccessor): Promise<pegasusai> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(PEGASUSAI_TOGGLE_SETTINGS_ACTION_ID)
	}
})
