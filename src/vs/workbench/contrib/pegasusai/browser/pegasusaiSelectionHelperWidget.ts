...
import { mountPegasusAISelectionHelper } from './react/out/pegasusai-widgets-tsx/index.js';
...
export type PegasusAISelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	public static readonly ID = 'editor.contrib.pegasusaiSelectionHelper';
	// react
	private _rootHTML: HTMLElement;
	private _rerender: (props?: any) => pegasusai = () => { };
	...
	constructor(
		...
		@IPegasusAISettingsService private readonly _pegasusaiSettingsService: IPegasusAISettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountPegasusAISelectionHelper(content, accessor);
			if (!res) return;
	...
	private _onSelectionChange(e: ICursorSelectionChangedEvent): pegasusai {
	...
	private _showHelperForSelection(selection: Selection): pegasusai {
		if (!this._editor.hasModel()) {
			return;
		}
		...
		// rerender
		const enabled = this._pegasusaiSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies PegasusAISelectionHelperProps)
	...
	private _hideHelper(): pegasusai {
		this._rootHTML.style.display = 'none';
		this._isVisible = false;
		this._lastSelection = null;
	}

	private _updatePositionIfVisible(): pegasusai {
