...
import { ChatMode, FeatureName, ModelSelection, ProviderName } from '../common/pegasusaiSettingsTypes.js';
...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from the user's .pegasusairules file):\n${aiInstructions}`)
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	constructor(
		...
		@IPegasusAIModelService private readonly pegasusaiModelService: IPegasusAIModelService,
		@IMCPService private readonly mcpService: IMCPService,
	) {
		super()
	}

	// Read .pegasusairules files from workspace folders
	private _getPegasusAIRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let pegasusaiRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.pegasusairules')
				const { model } = this.pegasusaiModelService.getModel(uri)
				if (!model) continue
				pegasusaiRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return pegasusaiRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	// Get combined AI instructions from settings and .pegasusairules files
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.pegasusaiSettingsService.state.globalSettings.aiInstructions;
		const pegasusaiRulesFileContent = this._getPegasusAIRulesFileContents();

		const ans: string[] = []
		if (globalAIInstructions) ans.push(globalAIInstructions)
		if (pegasusaiRulesFileContent) ans.push(pegasusaiRulesFileContent)
		return ans.join('\n\n')
	}
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.pegasusaiSettingsService.state
		...
		const modelSelectionOptions = this.pegasusaiSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { disableSystemMessage } = this.pegasusaiSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.pegasusaiSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
...
// Instructions:
// Do not output an explanation. Try to apegasusai outputting comments. Only output the middle code.
