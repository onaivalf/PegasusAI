...
import '../styles.css'
import { PEGASUSAI_CTRL_K_ACTION_ID, PEGASUSAI_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { Circle, MoreVertical } from 'lucide-react';
import { useEffect, useState } from 'react';

import { PegasusAISelectionHelperProps } from '../../../../../../contrib/pegasusai/browser/pegasusaiSelectionHelperWidget.js';
import { PEGASUSAI_OPEN_SETTINGS_ACTION_ID } from '../../../pegasusaiSettingsPane.js';


export const PegasusAISelectionHelperMain = (props: PegasusAISelectionHelperProps) => {
	...
	>
		<PegasusAISelectionHelper {...props} />
	</div>
}



const PegasusAISelectionHelper = ({ rerenderKey }: PegasusAISelectionHelperProps) => {
	...
	const ctrlLKeybind = keybindingService.lookupKeybinding(PEGASUSAI_CTRL_L_ACTION_ID)
	const ctrlKKeybind = keybindingService.lookupKeybinding(PEGASUSAI_CTRL_K_ACTION_ID)

	const dividerHTML = <div className='w-[0.5px] bg-pegasusai-border-3'></div>
	...
	useEffect(() => {
		const disposable = commandService.onWillExecuteCommand(e => {
			if (e.commandId === PEGASUSAI_CTRL_L_ACTION_ID || e.commandId === PEGASUSAI_CTRL_K_ACTION_ID) {
	...
	const defaultHTML = <>
		{ctrlLKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(PEGASUSAI_CTRL_L_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
		...
		{ctrlKKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(PEGASUSAI_CTRL_K_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
	...
	const moreOptionsHTML = <>
		<div
			...
			'
			onClick={() => {
				commandService.executeCommand(PEGASUSAI_OPEN_SETTINGS_ACTION_ID);
				setClickState('clickedOption');
			}}
		>
			Disable Suggestions?
	...
	return <div className='
		...
		border border-pegasusai-border-3 bg-pegasusai-bg-2
