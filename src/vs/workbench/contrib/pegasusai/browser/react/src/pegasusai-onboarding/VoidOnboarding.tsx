...
import { displayInfoOfProviderName, ProviderName, providerNames, localProviderNames, featureNames, FeatureName, isFeatureNameDisabled } from '../../../../common/pegasusaiSettingsTypes.js';
import { ChatMarkdownRender } from '../markdown/ChatMarkdownRender.js';
import { OllamaSetupInstructions, OneClickSwitchButton, SettingsForProvider, ModelDump } from '../pegasusai-settings-tsx/Settings.js';
...
export const PegasusAIOnboarding = () => {

	const pegasusaiSettingsState = useSettingsState()
	const isOnboardingComplete = pegasusaiSettingsState.globalSettings.isOnboardingComplete || OVERRIDE_VALUE

	const isDark = useIsDark()

	return (
		<div className={`@@pegasusai-scope ${isDark ? 'dark' : ''}`}>
			<div
				className={`
					bg-pegasusai-bg-3 fixed top-0 right-0 bottom-0 left-0 width-full z-[99999]
...
const PegasusAIIcon = () => {
	...
	return <div ref={divRef} className='@@pegasusai-pegasusai-icon' />
}
...
const AddProvidersPage = ({ pageIndex, setPageIndex }: { pageIndex: number, setPageIndex: (index: number) => pegasusai }) => {
	...
	return (<div className="flex flex-col md:flex-row w-full h-[80vh] gap-6 max-w-[900px] mx-auto relative">
		{/* Left Column */}
		<div className="md:w-1/4 w-full flex flex-col gap-6 p-6 border-none border-pegasusai-border-2 h-full overflow-y-auto">
			{/* Tab Selector */}
		...
		<div className="flex-1 flex flex-col items-center justify-start p-6 h-full overflow-y-auto">
			...
			<div className="w-full max-w-xl mt-4 mb-10">
				...
				<div className="text-sm opacity-80 text-pegasusai-fg-3 my-4 w-full">{descriptionOfTab[currentTab]}</div>
			</div>
			...
			{providerNamesOfTab[currentTab].map((providerName) => (
				<div key={providerName} className="w-full max-w-xl mb-10">
					<div className="text-xl mb-2">
						...
						{providerName === 'gemini' && (
							<span
								data-tooltip-id="pegasusai-tooltip-provider-info"
			...
			{(currentTab === 'Local' || currentTab === 'Cloud/Other') && (
				<div className="w-full max-w-xl mt-8 bg-pegasusai-bg-2/50 rounded-lg p-6 border border-pegasusai-border-4">
					<div className="flex items-center gap-2 mb-4">
						<div className="text-xl font-medium">Models</div>
					</div>

					{currentTab === 'Local' && (
						<div className="text-sm opacity-80 text-pegasusai-fg-3 my-4 w-full">Local models should be detected automatically. You can add custom models below.</div>
					)}
...
// 		title:
// 			div
// 				"How would you like to use PegasusAI?"
// 		content:
...
const NextButton = ({ onClick, ...props }: { onClick: () => pegasusai } & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
...
const PreviousButton = ({ onClick, ...props }: { onClick: () => pegasusai } & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
	return (
		<button
			onClick={onClick}
			className="px-6 py-2 rounded text-pegasusai-fg-3 opacity-80 hover:brightness-115 duration-600 transition-all"
			{...props}
		>
			Back
		</button>
...
const OllamaDownloadOrRemoveModelButton = ({ modelName, isModelInstalled, sizeGb }: { modelName: string, isModelInstalled: boolean, sizeGb: number | false | 'not-known' }) => {
	...
	return <a
		...
		className="flex items-center justify-center text-pegasusai-fg-2 hover:text-pegasusai-fg-1"
	>
...
const PegasusAIOnboardingContent = () => {


	const accessor = useAccessor()
	const pegasusaiSettingsService = accessor.get('IPegasusAISettingsService')
	const pegasusaiMetricsService = accessor.get('IMetricsService')
	...
	const didFillInProviderSettings = selectedProviderName && pegasusaiSettingsState.settingsOfProvider[selectedProviderName]._didFillInProviderSettings
	const isApiKeyLongEnoughIfApiKeyExists = selectedProviderName && pegasusaiSettingsState.settingsOfProvider[selectedProviderName].apiKey ? pegasusaiSettingsState.settingsOfProvider[selectedProviderName].apiKey.length > 15 : true
	const isAtLeastOneModel = selectedProviderName && pegasusaiSettingsState.settingsOfProvider[selectedProviderName].models.length >= 1
	...
	const lastPagePrevAndNextButtons = <div className="max-w-[600px] w-full mx-auto flex flex-col items-end">
		<div className="flex items-center gap-2">
			...
			<PrimaryActionButton
				onClick={() => {
					pegasusaiSettingsService.setGlobalSetting('isOnboardingComplete', true);
					pegasusaiMetricsService.capture('Completed Onboarding', { selectedProviderName, wantToUseOption })
				}}
				ringSize={pegasusaiSettingsState.globalSettings.isOnboardingComplete ? 'screen' : undefined}
			>Enter the PegasusAI</PrimaryActionButton>
		</div>
	</div>
	...
	const detailedDescOfWantToUseOption: { [wantToUseOption in WantToUseOption]: string } = {
		...
		private: "Private-hosted so your data never leaves your computer or network. [Email us](mailto:founders@pegasusai.dev) for help setting up at your company.",
	...
	useEffect(() => {
		if (!pegasusaiSettingsState.globalSettings.isOnboardingComplete) {
			setPageIndex(0)
		}
	}, [setPageIndex, pegasusaiSettingsState.globalSettings.isOnboardingComplete])
		...
		0: <OnboardingPageShell
			content={
				<div className='flex flex-col items-center gap-8'>
					<div className="text-5xl font-light text-center">Welcome to PegasusAI</div>

					{/* Slice of PegasusAI image */}
					<div className='max-w-md w-full h-[30vh] mx-auto flex items-center justify-center'>
						{!isLinux && <PegasusAIIcon />}
					</div>
			...
			content={
				<div>
					...
					<div className="mt-8 text-center flex flex-col items-center gap-4 w-full max-w-md mx-auto">
						<h4 className="text-pegasusai-fg-3 mb-4">Transfer your settings from an existing editor?</h4>
