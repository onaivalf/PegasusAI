...
export const WidgetComponent = <CtorParams extends any[], Instance>({ ctor, propsFn, dispose, onCreateInstance, children, className }
	: {
		...
		dispose: (instance: Instance) => pegasusai,
...
export type TextAreaFns = { setValue: (v: string) => pegasusai, enable: () => pegasusai, disable: () => pegasusai }
type InputBox2Props = {
	...
	className?: string;
	onChangeText?: (value: string) => pegasusai;
	onKeyDown?: (e: React.KeyboardEvent<HTMLTextAreaElement>) => pegasusai;
	onFocus?: (e: React.FocusEvent<HTMLTextAreaElement>) => pegasusai;
	onBlur?: (e: React.FocusEvent<HTMLTextAreaElement>) => pegasusai;
	onChangeHeight?: (newHeight: number) => pegasusai;
}
export const PegasusAIInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
	...
	const adjustHeight = useCallback(() => {
		...
		const newHeight = Math.min(h + 1, 500) // plus one to apegasusai scrollbar appearing when it shouldn't
	...
	return <>
		<textarea
			...
			className={`w-full resize-none max-h-[500px] overflow-y-auto text-pegasusai-fg-1 placeholder:text-pegasusai-fg-3 ${className}`}
			style={{
		...
		{isMenuOpen && (
			<div
				...
				className="z-[100] border-pegasusai-border-3 bg-pegasusai-bg-2-alt border rounded shadow-lg flex flex-col overflow-hidden"
				style={{
					position: strategy,
					top: y ?? 0,
					left: x ?? 0,
			...
			>
				{/* Breadcrumbs Header */}
				{isBreadcrumbsShowing && <div className="px-2 py-1 text-pegasusai-fg-1 bg-pegasusai-bg-2-alt border-b border-pegasusai-border-3 sticky top-0 bg-pegasusai-bg-1 z-10 select-none pointer-events-none">
					{optionText ?
				...
				<div className='max-h-[400px] w-full max-w-full overflow-y-auto overflow-x-auto'>
					<div className="w-max min-w-full flex flex-col gap-0 text-nowrap flex-nowrap">
						{options.length === 0 ?
							<div className="text-pegasusai-fg-3 px-3 py-0.5">No results found</div>
							: options.map((o, oIdx) => {
								...
								return (
									...
									<div
										...
										className={`
											...
											${oIdx === optionIdx ? 'bg-blue-500 text-white/80' : 'bg-pegasusai-bg-2-alt text-pegasusai-fg-1'}
										`}
...
export const PegasusAISimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, ...inputProps }: {
	value: string;
	onChangeValue: (value: string) => pegasusai;
	placeholder: string;
	className?: string;
	disabled?: boolean;
	compact?: boolean;
	...
	return (
		<input
			ref={inputRef}
			defaultValue={value} // Use defaultValue instead of value to apegasusai recreation
			onChange={handleChange}
			placeholder={placeholder}
			disabled={disabled}
			className={`w-full resize-none bg-pegasusai-bg-1 text-pegasusai-fg-1 placeholder:text-pegasusai-fg-3 border border-pegasusai-border-2 focus:border-pegasusai-border-1
...
export const PegasusAIInputBox = ({ onChangeText, onCreateInstance, inputBoxRef, placeholder, isPasswordField, multiline }: {
	onChangeText: (value: string) => pegasusai;
	styles?: Partial<IInputBoxStyles>,
	onCreateInstance?: (instance: InputBox) => pegasusai | IDisposable[];
	...
	return <WidgetComponent
		className='
			bg-pegasusai-bg-1
			@@pegasusai-force-child-placeholder-pegasusai-fg-1
		'
		ctor={InputBox}
...
export const PegasusAISlider = ({
	...
	value: number;
	onChange: (value: number) => pegasusai;
	disabled?: boolean;
				...
				<div className="relative w-full">
					...
					<div
						className={`relative ${size === 'xxs' ? 'h-0.5' :
							size === 'xs' ? 'h-1' :
								size === 'sm' ? 'h-1.5' :
									size === 'sm+' ? 'h-2' : 'h-2.5'
							} bg-pegasusai-bg-2 rounded-full cursor-pointer`}
						onClick={handleTrackClick}
					>
						...
						<div
							className={`absolute left-0 ${size === 'xxs' ? 'h-0.5' :
								size === 'xs' ? 'h-1' :
									size === 'sm' ? 'h-1.5' :
										size === 'sm+' ? 'h-2' : 'h-2.5'
								} bg-pegasusai-fg-1 rounded-full`}
					...
					<div
						className={`absolute top-1/2 transform -translate-x-1/2 -translate-y-1/2
							...
							bg-pegasusai-fg-1 rounded-full shadow-md ${disabled ? 'cursor-not-allowed' : 'cursor-grab active:cursor-grabbing'}
							border border-pegasusai-fg-1`}
...
export const PegasusAISwitch = ({
	...
	value: boolean;
	onChange: (value: boolean) => pegasusai;
	disabled?: boolean;
	size?: 'xxs' | 'xs' | 'sm' | 'sm+' | 'md';
}) => {
	return (
...
export const PegasusAICheckBox = ({ label, value, onClick, className }: { label: string, value: boolean, onClick: (checked: boolean) => pegasusai, className?: string }) => {
...
export const PegasusAICustomDropdownBox = <T extends NonNullable<any>>({
	options,
	selectedOption,
	onChangeOption,
	...
	onChangeOption: (newValue: T) => pegasusai;
	...
	return (
		<div className={`inline-block relative ${className}`}>
			...
			{isOpen && (
				<div
					ref={refs.setFloating}
					className="z-[100] bg-pegasusai-bg-1 border-pegasusai-border-3 border rounded shadow-lg"
					style={{
						position: strategy,
						top: y ?? 0,
						left: x ?? 0,
...
export const _PegasusAISelectBox = <T,>({ onChangeSelection, onCreateInstance, selectBoxRef, options, className }: {
	onChangeSelection: (value: T) => pegasusai;
	onCreateInstance?: ((instance: SelectBox) => pegasusai | IDisposable[]);
	...
	return <WidgetComponent
		className={`
			...
			@@[&_select]:!pegasusai-text-pegasusai-fg-3
			@@[&_select]:!pegasusai-text-xs
			!text-pegasusai-fg-3
			${className ?? ''}
		`}
		ctor={SelectBox}
...
export const BlockCode = ({ initValue, language, maxHeight, showScrollbars }: BlockCodeProps) => {
	...
	return <div ref={divRef} className='relative z-0 px-2 py-1 bg-pegasusai-bg-3'>
		<WidgetComponent
...
export const PegasusAIButtonBgDarken = ({ children, disabled, onClick, className }: { children: React.ReactNode; disabled?: boolean; onClick: () => pegasusai; className?: string }) => {
...
// export const PegasusAIScrollableElt = ({ options, children }: { options: ScrollableElementCreationOptions, children: React.ReactNode }) => {
...
// export const PegasusAISelectBox = <T,>({ onChangeSelection, initVal, selectBoxRef, options }: {
// 	initVal: T;
// 	selectBoxRef: React.MutableRefObject<SelectBox | null>;
// 	options: readonly { text: string, value: T }[];
// 	onChangeSelection: (value: T) => pegasusai;
...
// export const PegasusAICheckBox = ({ onChangeChecked, initVal, label, checkboxRef, }: {
// 	onChangeChecked: (checked: boolean) => pegasusai;
// 	initVal: boolean;
...
const SingleDiffEditor = ({ block, lang }: { block: ExtractedSearchReplaceBlock, lang: string | undefined }) => {
	...
	return (
		<div className="w-full bg-pegasusai-bg-3 @@bg-editor-style-override" ref={divRef} />
	);
};
 ...
 */
export const PegasusAIDiffEditor = ({ uri, searchReplaceBlocks, language }: { uri?: any, searchReplaceBlocks: string, language?: string }) => {
	const accessor = useAccessor();
	const languageService = accessor.get('ILanguageService');
	...
	if (blocks.length === 0) {
		return <div className="w-full p-4 text-pegasusai-fg-4 text-sm">No changes found</div>;
	}
		...
		<div className="w-full flex flex-col gap-2">
			{blocks.map((block, index) => (
				<div key={index} className="w-full">
					{blocks.length > 1 && (
						<div className="text-pegasusai-fg-4 text-xs mb-1 px-1">
