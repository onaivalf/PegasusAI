...
import { MCPUserState, RefreshableProviderName, SettingsOfProvider } from '../../../../../../../workbench/contrib/pegasusai/common/pegasusaiSettingsTypes.js'
import { DisposableStore, IDisposable } from '../../../../../../../base/common/lifecycle.js'
import { PegasusAISettingsState } from '../../../../../../../workbench/contrib/pegasusai/common/pegasusaiSettingsService.js'
import { ColorScheme } from '../../../../../../../platform/theme/common/theme.js'
import { RefreshModelStateOfProvider } from '../../../../../../../workbench/contrib/pegasusai/common/refreshModelService.js'
...
import { IRefreshModelService } from '../../../../../../../workbench/contrib/pegasusai/common/refreshModelService.js';
import { IPegasusAISettingsService } from '../../../../../../../workbench/contrib/pegasusai/common/pegasusaiSettingsService.js';
import { IExtensionTransferService } from '../../../../../../../workbench/contrib/pegasusai/browser/extensionTransferService.js'
...
const chatThreadsStateListeners: Set<(s: ThreadsState) => pegasusai> = new Set()

let chatThreadsStreamState: ThreadStreamState
const chatThreadsStreamStateListeners: Set<(threadId: string) => pegasusai> = new Set()

let settingsState: PegasusAISettingsState
const settingsStateListeners: Set<(s: PegasusAISettingsState) => pegasusai> = new Set()

let refreshModelState: RefreshModelStateOfProvider
const refreshModelStateListeners: Set<(s: RefreshModelStateOfProvider) => pegasusai> = new Set()
const refreshModelProviderListeners: Set<(p: RefreshableProviderName, s: RefreshModelStateOfProvider) => pegasusai> = new Set()

let colorThemeState: ColorScheme
const colorThemeStateListeners: Set<(s: ColorScheme) => pegasusai> = new Set()

const ctrlKZoneStreamingStateListeners: Set<(diffareaid: number, s: boolean) => pegasusai> = new Set()
const commandBarURIStateListeners: Set<(uri: URI) => pegasusai> = new Set();
const activeURIListeners: Set<(uri: URI | null) => pegasusai> = new Set();

const mcpListeners: Set<() => pegasusai> = new Set()
...
export const _registerServices = (accessor: ServicesAccessor) => {
	...
	const stateServices = {
		...
		settingsStateService: accessor.get(IPegasusAISettingsService),
		refreshModelService: accessor.get(IRefreshModelService),
		themeService: accessor.get(IThemeService),
		editCodeService: accessor.get(IEditCodeService),
		pegasusaiCommandBarService: accessor.get(IPegasusAICommandBarService),
		modelService: accessor.get(IModelService),
		mcpService: accessor.get(IMCPService),
	}

	const { settingsStateService, chatThreadsStateService, refreshModelService, themeService, editCodeService, pegasusaiCommandBarService, modelService, mcpService } = stateServices
	...
	disposables.push(
		pegasusaiCommandBarService.onDidChangeState(({ uri }) => {
			commandBarURIStateListeners.forEach(l => l(uri));
		})
	)

	disposables.push(
		pegasusaiCommandBarService.onDidChangeActiveURI(({ uri }) => {
...
const getReactAccessor = (accessor: ServicesAccessor) => {
	const reactAccessor = {
		...
		IPegasusAISettingsService: accessor.get(IPegasusAISettingsService),
		...
		IPegasusAIModelService: accessor.get(IPegasusAIModelService),
		IWorkspaceContextService: accessor.get(IWorkspaceContextService),

		IPegasusAICommandBarService: accessor.get(IPegasusAICommandBarService),
...
export const useAccessor = () => {
	if (!reactAccessor_) {
		throw new Error(`⚠️ PegasusAI useAccessor was called before _registerServices!`)
	}
...
export const useRefreshModelListener = (listener: (providerName: RefreshableProviderName, s: RefreshModelStateOfProvider) => pegasusai) => {
	useEffect(() => {
...
export const useCtrlKZoneStreamingState = (listener: (diffareaid: number, s: boolean) => pegasusai) => {
	useEffect(() => {
...
export const useCommandBarURIListener = (listener: (uri: URI) => pegasusai) => {
	useEffect(() => {
