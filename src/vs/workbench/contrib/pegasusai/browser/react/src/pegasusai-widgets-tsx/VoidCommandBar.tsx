...
import { PegasusAICommandBarProps } from '../../../pegasusaiCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	PEGASUSAI_GOTO_NEXT_DIFF_ACTION_ID,
	PEGASUSAI_GOTO_PREV_DIFF_ACTION_ID,
	PEGASUSAI_GOTO_NEXT_URI_ACTION_ID,
	PEGASUSAI_GOTO_PREV_URI_ACTION_ID,
	PEGASUSAI_ACCEPT_FILE_ACTION_ID,
	PEGASUSAI_REJECT_FILE_ACTION_ID,
	PEGASUSAI_ACCEPT_ALL_DIFFS_ACTION_ID,
	PEGASUSAI_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const PegasusAICommandBarMain = ({ uri, editor }: PegasusAICommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@pegasusai-scope ${isDark ? 'dark' : ''}`}
	>
		<PegasusAICommandBar uri={uri} editor={editor} />
	</div>
}



export const AcceptAllButtonWrapper = ({ text, onClick, className, ...props }: { text: string, onClick: () => pegasusai, className?: string } & React.ButtonHTMLAttributes<HTMLButtonElement>) => (
	<button
		className={`
			px-2 py-0.5
...
export const RejectAllButtonWrapper = ({ text, onClick, className, ...props }: { text: string, onClick: () => pegasusai, className?: string } & React.ButtonHTMLAttributes<HTMLButtonElement>) => (
	<button
		className={`
			px-2 py-0.5
			flex items-center gap-1
...
export const PegasusAICommandBar = ({ uri, editor }: PegasusAICommandBarProps) => {
	...
	const pegasusaiModelService = accessor.get('IPegasusAIModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(PEGASUSAI_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(PEGASUSAI_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(PEGASUSAI_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(PEGASUSAI_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(PEGASUSAI_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(PEGASUSAI_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(PEGASUSAI_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(PEGASUSAI_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-pegasusai-bg-2 shadow-md border border-pegasusai-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-pegasusai-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-pegasusai-bg-2 rounded shadow-md border border-pegasusai-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-pegasusai-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-pegasusai-bg-2 rounded shadow-md border border-pegasusai-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-pegasusai-border-2 [&>*:last-child]:border-r-0">
