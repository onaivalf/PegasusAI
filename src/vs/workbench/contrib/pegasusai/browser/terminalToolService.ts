...
export interface ITerminalToolService {
	...
	): Promise<{ interrupt: () => pegasusai; resPromise: Promise<{ result: string, resolveReason: TerminalResolveReason }> }>;

	focusPersistentTerminal(terminalId: string): Promise<pegasusai>
	...
	killPersistentTerminal(terminalId: string): Promise<pegasusai>
...
export const persistentTerminalNameOfId = (id: string) => {
	if (id === '1') return 'PegasusAI Agent'
	return `PegasusAI Agent (${id})`
}
export const idOfPersistentTerminalName = (name: string) => {
	if (name === 'PegasusAI Agent') return '1'

	const match = name.match(/PegasusAI Agent \((\d+)\)/)
...
export class TerminalToolService extends Disposable implements ITerminalToolService {
	...
	private async _createTerminal(props: { cwd: string | null, config: ICreateTerminalOptions['config'], hidden?: boolean }) {
		...
		// const waitForMount = new Promise<pegasusai>(res => {
		// 	let data = ''
		...
		// 	disposables.push(d)
		// })
		// const waitForTimeout = new Promise<pegasusai>(res => { setTimeout(() => { res() }, 5000) })
	...
	runCommand: ITerminalToolService['runCommand'] = async (command, params) => {
		...
		const waitForResult = async () => {
			...
			// if (!cmdCap) throw new Error(`There was an error using the terminal: CommandDetection capability did not mount yet. Please try again in a few seconds or report this to the PegasusAI team.`)

			// Prefer the structured command-detection capability when available

			const waitUntilDone = new Promise<pegasusai>(resolve => {
				if (!cmdCap) return
