...
import { mountPegasusAICommandBar } from './react/out/pegasusai-widgets-tsx/index.js'
...
import { PEGASUSAI_ACCEPT_DIFF_ACTION_ID, PEGASUSAI_REJECT_DIFF_ACTION_ID, PEGASUSAI_GOTO_NEXT_DIFF_ACTION_ID, PEGASUSAI_GOTO_PREV_DIFF_ACTION_ID, PEGASUSAI_GOTO_NEXT_URI_ACTION_ID, PEGASUSAI_GOTO_PREV_URI_ACTION_ID, PEGASUSAI_ACCEPT_FILE_ACTION_ID, PEGASUSAI_REJECT_FILE_ACTION_ID, PEGASUSAI_ACCEPT_ALL_DIFFS_ACTION_ID, PEGASUSAI_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface IPegasusAICommandBarService {
	...
	setDiffIdx(uri: URI, newIdx: number | null): pegasusai;

	getNextDiffIdx(step: 1 | -1): number | null;
	getNextUriIdx(step: 1 | -1): number | null;
	goToDiffIdx(idx: number | null): pegasusai;
	goToURIIdx(idx: number | null): Promise<pegasusai>;

	acceptOrRejectAllFiles(opts: { behavior: 'reject' | 'accept' }): pegasusai;
	anyFileIsStreaming(): boolean;

}


export const IPegasusAICommandBarService = createDecorator<IPegasusAICommandBarService>('PegasusAICommandBarService');
...
export class PegasusAICommandBarService extends Disposable implements IPegasusAICommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'pegasusai.PegasusAICommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<pegasusai> {
		...
		const { model } = await this._pegasusaiModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IPegasusAICommandBarService, PegasusAICommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type PegasusAICommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountPegasusAICommandBar(root, accessor, { uri, editor } satisfies PegasusAICommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies PegasusAICommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PEGASUSAI_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('pegasusaiAcceptDiffAction', 'PegasusAI: Accept Diff'),
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Alt | KeyMod.Shift | KeyCode.Enter,
				mac: { primary: KeyMod.WinCtrl | KeyMod.Alt | KeyCode.Enter },
				weight: KeybindingWeight.PegasusAIExtension,
			}
		});
	}
	...
	async run(accessor: ServicesAccessor): Promise<pegasusai> {
		...
		const commandBarService = accessor.get(IPegasusAICommandBarService);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PEGASUSAI_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('pegasusaiRejectDiffAction', 'PegasusAI: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PEGASUSAI_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('pegasusaiGoToNextDiffAction', 'PegasusAI: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PEGASUSAI_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('pegasusaiGoToPrevDiffAction', 'PegasusAI: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PEGASUSAI_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('pegasusaiGoToNextUriAction', 'PegasusAI: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PEGASUSAI_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('pegasusaiGoToPrevUriAction', 'PegasusAI: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PEGASUSAI_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('pegasusaiAcceptFileAction', 'PegasusAI: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PEGASUSAI_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('pegasusaiRejectFileAction', 'PegasusAI: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PEGASUSAI_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('pegasusaiAcceptAllDiffsAction', 'PegasusAI: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PEGASUSAI_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('pegasusaiRejectAllDiffsAction', 'PegasusAI: Reject All Diffs in All Files'),
			keybinding: {
