...
import { pegasusaiPrefixAndSuffix, ctrlKStream_userMessage, ctrlKStream_systemMessage, defaultQuickEditFimTags, rewriteCode_systemMessage, rewriteCode_userMessage, searchReplaceGivenDescription_systemMessage, searchReplaceGivenDescription_userMessage, tripleTick, } from '../common/prompt/prompts.js';
import { IPegasusAICommandBarService } from './pegasusaiCommandBarService.js';
import { IKeybindingService } from '../../../../platform/keybinding/common/keybinding.js';
import { PEGASUSAI_ACCEPT_DIFF_ACTION_ID, PEGASUSAI_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { FeatureName } from '../common/pegasusaiSettingsTypes.js';
import { IPegasusAIModelService } from '../common/pegasusaiModelService.js';
import { deepClone } from '../../../../base/common/objects.js';
import { acceptBg, acceptBorder, buttonFontSize, buttonTextColor, rejectBg, rejectBorder } from '../common/helpers/colors.js';
import { DiffArea, Diff, CtrlKZone, PegasusAIFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._pegasusaiModelService.initializeModel(model.uri)
	...
	// 		message: `PegasusAI Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'pegasusai.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open PegasusAI's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(PEGASUSAI_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/pegasusai/pegasusai/issues/new) it.` : undefined
	// 	})
	// }
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'pegasusai-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'pegasusai-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'pegasusai-highlightBG')
	...
	private _addCtrlKZoneInput = (ctrlKZone: CtrlKZone) => {
		...
		const itemId = this._consistentEditorItemService.addToEditor(editor, () => {
			...
			// mount react
			let disposeFn: (() => pegasusai) | undefined = undefined
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		const disposeInThisEditorFns: (() => pegasusai)[] = []

		const { model } = this._pegasusaiModelService.getModel(uri)

		// green decoration and minimap decoration
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'pegasusai-greenBG', {
		...
		if (type !== 'insertion') {
			const consistentZoneId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					const domNode = document.createElement('div');
					domNode.className = 'pegasusai-redBG'
		...
		if (diffZone.type === 'DiffZone' && !diffZone._streamState.isStreaming) {
			...
			const consistentWidgetId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					else { throw new Error('PegasusAI 1') }
	...
	private _getCurrentPegasusAIFileSnapshot = (uri: URI): PegasusAIFileSnapshot => {
		const { model } = this._pegasusaiModelService.getModel(uri)
	...
	private _restorePegasusAIFileSnapshot = async (uri: URI, snapshot: PegasusAIFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => pegasusai }) {
		const beforeSnapshot: PegasusAIFileSnapshot = this._getCurrentPegasusAIFileSnapshot(uri)
		let afterSnapshot: PegasusAIFileSnapshot | null = null

		const elt: IUndoRedoElement = {
			type: UndoRedoElementType.Resource,
			resource: uri,
			label: 'PegasusAI Agent',
			code: 'undoredo.editCode',
			undo: async () => { opts?.onWillUndo?.(); await this._restorePegasusAIFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restorePegasusAIFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentPegasusAIFileSnapshot(uri)
			await this._pegasusaiModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}


	public getPegasusAIFileSnapshot(uri: URI) {
		return this._getCurrentPegasusAIFileSnapshot(uri)
	}


	public restorePegasusAIFileSnapshot(uri: URI, snapshot: PegasusAIFileSnapshot): pegasusai {
		this._restorePegasusAIFileSnapshot(uri, snapshot)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`PegasusAI: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._pegasusaiModelService.initializeModel(uri)
		await this._pegasusaiModelService.saveModel(uri) // save the URI
	}


	// the applyDonePromise this returns can reject, and should be caught with .catch
	public startApplying(opts: StartApplyingOpts): [URI, Promise<pegasusai>] | null {
		let res: [DiffZone, Promise<pegasusai>] | undefined = undefined
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<pegasusai>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`PegasusAI: diff.type not recognized on: ${from}`)
		}
		...
		if (from === 'ClickApply') {
			...
			const { prefix, suffix } = pegasusaiPrefixAndSuffix({ fullFileStr: originalFileCode, startLine, endLine })
		...
		const runWriteover = async () => {
			...
			while (shouldSendAnotherMessage) {
				...
				let resMessageDonePromise: () => pegasusai = () => { }
				const messageDonePromise = new Promise<pegasusai>((res_) => { resMessageDonePromise = res_ })
		...
		const applyDonePromise = new Promise<pegasusai>((res, rej) => { runWriteover().then(res).catch(rej) })
	...
	private _initializeSearchAndReplaceStream(opts: StartApplyingOpts & { from: 'ClickApply' }): [DiffZone, Promise<pegasusai>] | undefined {
		...
		const runSearchReplace = async () => {
			...
			while (shouldSendAnotherMessage) {
				...
				let resMessageDonePromise: () => pegasusai = () => { }
				const messageDonePromise = new Promise<pegasusai>((res, rej) => { resMessageDonePromise = res })
					...
					for (let blockNum = currStreamingBlockNum; blockNum < blocks.length; blockNum += 1) {
						...
						if (!(blockNum in addedTrackingZoneOfBlockNum)) {
							...
							if (typeof originalBounds === 'string' || hasOverlap) {
								...
								const retryMsg = 'All of your previous outputs have been ignored. Please re-output ALL SEARCH/REPLACE blocks starting from the first one, and apegasusai the error this time.'
								messages.push(
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`PegasusAI: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
		...
		const applyDonePromise = new Promise<pegasusai>((res, rej) => { runSearchReplace().then(res).catch(rej) })
		return [diffZone, applyDonePromise]
	}
	...
	// called on pegasusai.acceptDiff
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`PegasusAI error: ${diff}.type not recognized`)
		}
	...
	// called on pegasusai.rejectDiff
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@IPegasusAICommandBarService private readonly _pegasusaiCommandBarService: IPegasusAICommandBarService,
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(PEGASUSAI_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(PEGASUSAI_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._pegasusaiCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._pegasusaiCommandBarService.onDidChangeState(e => {
