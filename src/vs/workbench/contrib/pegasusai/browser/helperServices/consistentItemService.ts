...
type AddItemInputs = { uri: URI; fn: (editor: ICodeEditor) => (() => pegasusai); }
...
export interface IConsistentItemService {
	...
	removeConsistentItemFromURI(consistentItemId: string): pegasusai;
}
...
export class ConsistentItemService extends Disposable implements IConsistentItemService {
	...
	private readonly disposeFnOfItemId: Record<string, () => pegasusai> = {}
...
export interface IConsistentEditorItemService {
	readonly _serviceBrand: undefined;
	addToEditor(editor: ICodeEditor, fn: () => () => pegasusai): string;
	removeFromEditor(itemId: string): pegasusai;
}
...
export class ConsistentEditorItemService extends Disposable {
	...
	private readonly itemInfoById: Record<
		string,
		{
			editorId: string;
			uriFsPath: string;
			fn: (editor: ICodeEditor) => () => pegasusai;
			disposeFn?: () => pegasusai;
		}
	> = {};
