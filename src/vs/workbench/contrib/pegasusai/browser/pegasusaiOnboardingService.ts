...
import { mountPegasusAIOnboarding } from './react/out/pegasusai-onboarding/index.js'
...
export class OnboardingContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'workbench.contrib.pegasusaiOnboarding';
	...
	private initialize(): pegasusai {
		...
		if (workbench) {
			...
			const onboardingContainer = h('div.pegasusai-onboarding-container').root;
			workbench.appendChild(onboardingContainer);
			this.instantiationService.invokeFunction((accessor: ServicesAccessor) => {
				const result = mountPegasusAIOnboarding(onboardingContainer, accessor);
