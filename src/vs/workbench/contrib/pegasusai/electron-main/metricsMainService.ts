...
export class MetricsMainService extends Disposable implements IMetricsService {
	...
	// helper - looks like this is stored in a .vscdb file in ~/Library/Application Support/PegasusAI
	...
	private get oldId() {
		...
		const newKey = 'pegasusai.app.oldMachineId'
		...
		const oldValue = this._appStorage.get('pegasusai.machineId', StorageScope.APPLICATION) ?? 'NULL' // the old way of getting the key
		...
		// private get oldId() {
		// 	return this._memoStorage('pegasusai.app.oldMachineId', StorageTarget.MACHINE, 'NULL')
		// }
	}
	...
	private get distinctId() {
		...
		return this._memoStorage('pegasusai.app.machineId', StorageTarget.MACHINE, setValIfNotExist)
	}

	// just to see if there are ever multiple machineIDs per userID (instead of this, we should just track by the user's email)
	private get userId() {
		return this._memoStorage('pegasusai.app.userMachineId', StorageTarget.USER)
	}
	...
	async initialize() {
		...
		const { commit, version, pegasusaiVersion, release, quality } = this._productService
		...
		this._initProperties = {
			...
			pegasusaiVersion: pegasusaiVersion,
			release,
			os,
			quality,
		...
		console.log('User is opted out of basic PegasusAI metrics?', didOptOut)
		if (didOptOut) {
			this.client.optOut()
		}
		else {
		...
		console.log('PegasusAI posthog metrics info:', JSON.stringify(identifyMessage, null, 2))
	}
