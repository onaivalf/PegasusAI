...
export class LLMMessageChannel implements IServerChannel {
	...
	// aborters for above
	private readonly _infoOfRunningRequest: Record<string, { waitForSend: Promise<pegasusai> | undefined, abortRef: AbortRef }> = {}
	...
	async call(_: unknown, command: string, params: any): Promise<any> {
		try {
			if (command === 'sendLLMMessage') {
				...
				throw new Error(`PegasusAI sendLLM: command "${command}" not recognized.`)
			}
		}
		catch (e) {
