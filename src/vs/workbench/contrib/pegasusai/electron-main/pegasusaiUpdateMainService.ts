...
export class PegasusAIMainUpdateService extends Disposable implements IPegasusAIUpdateService {
	...
	async check(explicit: boolean): Promise<PegasusAICheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart PegasusAI to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<PegasusAICheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/pegasusai/binaries/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of PegasusAI is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'PegasusAI is up-to-date!'
					}
				}
				else {
