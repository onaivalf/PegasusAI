...
import { ChatMode, displayInfoOfProviderName, ModelSelectionOptions, OverridesOfModel, ProviderName, SettingsOfProvider } from '../../common/pegasusaiSettingsTypes.js';
...
type InternalCommonMessageParams = {
	...
	modelName: string;
	_setAborter: (aborter: () => pegasusai) => pegasusai;
}
...
const newOpenAICompatibleSDK = async ({ settingsOfProvider, providerName, includeInPayload }: { settingsOfProvider: SettingsOfProvider, providerName: ProviderName, includeInPayload?: { [s: string]: any } }) => {
	...
	if (providerName === 'openAI') {
		...
		return new OpenAI({
			...
			defaultHeaders: {
				'HTTP-Referer': 'https://pegasusai.dev', // Optional, for including your app on openrouter.ai rankings.
				'X-Title': 'PegasusAI', // Optional. Shows in rankings on openrouter.ai.
			},
			...commonPayloadOpts,
		})
	}
	...
	else throw new Error(`PegasusAI providerName was invalid: ${providerName}.`)
}
	...
	openai.chat.completions
		...
		.then(async response => {
			...
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				onError({ message: 'PegasusAI: Response from model was empty.', fullError: null })
			}
			else {
...
const newOllamaSDK = ({ endpoint }: { endpoint: string }) => {
	...
	if (!endpoint) throw new Error(`Ollama Endpoint was empty (please enter ${defaultProviderSettings.ollama.endpoint} in PegasusAI if you want the default url).`)
...
type CallFnOfProvider = {
	[providerName in ProviderName]: {
		sendChat: (params: SendChatParams_Internal) => Promise<pegasusai>;
		sendFIM: ((params: SendFIMParams_Internal) => pegasusai) | null;
		list: ((params: ListParams_Internal<any>) => pegasusai) | null;
	}
}
