...
import { displayInfoOfProviderName } from '../../common/pegasusaiSettingsTypes.js';
...
export const sendLLMMessage = async ({
	...
	let _fullTextSoFar = ''
	let _aborter: (() => pegasusai) | null = null
	let _setAborter = (fn: () => pegasusai) => { _aborter = fn }
	let _didAbort = false
	...
	const onError: OnError = ({ message: errorMessage, fullError }) => {
		...
		if (errorMessage === 'TypeError: fetch failed')
			errorMessage = `Failed to fetch from ${displayInfoOfProviderName(providerName).title}. This likely means you specified the wrong endpoint in PegasusAI's Settings, or your local model provider like Ollama is powered off.`
