...
import { PEGASUSAI_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, PegasusAIStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './pegasusaiSettingsTypes.js';
...
type SetGlobalSettingFn = <T extends GlobalSettingName>(settingName: T, newVal: GlobalSettings[T]) => pegasusai;

type SetOptionsOfModelSelection = (featureName: FeatureName, providerName: ProviderName, modelName: string, newVal: Partial<ModelSelectionOptions>) => pegasusai


export type PegasusAISettingsState = {
...
// type RealPegasusAISettings = Exclude<keyof PegasusAISettingsState, '_modelOptions'>
// type EventProp<T extends RealPegasusAISettings = RealPegasusAISettings> = T extends 'globalSettings' ? [T, keyof PegasusAISettingsState[T]] : T | 'all'


export interface IPegasusAISettingsService {
	readonly _serviceBrand: undefined;
	readonly state: PegasusAISettingsState; // in order to play nicely with react, you should immutably change state
	readonly waitForInitState: Promise<pegasusai>;

	onDidChangeState: Event<pegasusai>;

	setSettingOfProvider: SetSettingOfProviderFn;
	setModelSelectionOfFeature: SetModelSelectionOfFeatureFn;
	setOptionsOfModelSelection: SetOptionsOfModelSelection;
	setGlobalSetting: SetGlobalSettingFn;
	// setMCPServerStates: (newStates: MCPServerStates) => Promise<pegasusai>;

	// setting to undefined CLEARS it, unlike others:
	setOverridesOfModel(providerName: ProviderName, modelName: string, overrides: Partial<ModelOverrides> | undefined): Promise<pegasusai>;

	dangerousSetState(newState: PegasusAISettingsState): Promise<pegasusai>;
	resetState(): Promise<pegasusai>;

	setAutodetectedModels(providerName: ProviderName, modelNames: string[], logging: object): pegasusai;
	toggleModelHidden(providerName: ProviderName, modelName: string): pegasusai;
	addModel(providerName: ProviderName, modelName: string): pegasusai;
	deleteModel(providerName: ProviderName, modelName: string): boolean;

	addMCPUserStateOfNames(userStateOfName: MCPUserStateOfName): Promise<pegasusai>;
	removeMCPUserStateOfNames(serverNames: string[]): Promise<pegasusai>;
	setMCPServerState(serverName: string, state: MCPUserState): Promise<pegasusai>;
}




const _modelsWithSwappedInNewModels = (options: { existingModels: PegasusAIStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, PegasusAIStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: PegasusAISettingsState): PegasusAISettingsState => {
...
const _validatedModelState = (state: Omit<PegasusAISettingsState, '_modelOptions'>): PegasusAISettingsState => {
	...
	} satisfies PegasusAISettingsState
...
const defaultState = () => {
	const d: PegasusAISettingsState = {
...
export const IPegasusAISettingsService = createDecorator<IPegasusAISettingsService>('PegasusAISettingsService');
class PegasusAISettingsService extends Disposable implements IPegasusAISettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<pegasusai>();
	readonly onDidChangeState: Event<pegasusai> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: PegasusAISettingsState;

	private readonly _resolver: () => pegasusai
	waitForInitState: Promise<pegasusai> // await this if you need a valid state initially
	...
	dangerousSetState = async (newState: PegasusAISettingsState) => {
	...
	async readAndInitializeState() {
		let readS: PegasusAISettingsState
		try {
	...
	private async _readState(): Promise<PegasusAISettingsState> {
		const encryptedState = this._storageService.get(PEGASUSAI_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(PEGASUSAI_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: PegasusAISettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: PegasusAIStatefulModelInfo[] = [
...
registerSingleton(IPegasusAISettingsService, PegasusAISettingsService, InstantiationType.Eager);
