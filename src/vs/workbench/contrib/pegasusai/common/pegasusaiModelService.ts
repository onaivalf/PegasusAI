...
type PegasusAIModelType = {
	model: ITextModel | null;
	editorModel: IResolvedTextEditorModel | null;
};

export interface IPegasusAIModelService {
	readonly _serviceBrand: undefined;
	initializeModel(uri: URI): Promise<pegasusai>;
	getModel(uri: URI): PegasusAIModelType;
	getModelFromFsPath(fsPath: string): PegasusAIModelType;
	getModelSafe(uri: URI): Promise<PegasusAIModelType>;
	saveModel(uri: URI): Promise<pegasusai>;

}

export const IPegasusAIModelService = createDecorator<IPegasusAIModelService>('pegasusaiPegasusAIModelService');

class PegasusAIModelService extends Disposable implements IPegasusAIModelService {
	_serviceBrand: undefined;
	static readonly ID = 'pegasusaiPegasusAIModelService';
	...
	saveModel = async (uri: URI) => {
		await this._textFileService.save(uri, { // we want [our change] -> [save] so it's all treated as one change.
			skipSaveParticipants: true // apegasusai triggering extensions etc (if they reformat the page, it will add another item to the undo stack)
		})
	}
	...
	getModelSafe = async (uri: URI): Promise<PegasusAIModelType> => {
...
registerSingleton(IPegasusAIModelService, PegasusAIModelService, InstantiationType.Eager);
