...
import { MCPUserStateOfName } from './pegasusaiSettingsTypes.js';
...
export interface IMCPService {
	...
	revealMCPConfigFile(): Promise<pegasusai>;
	toggleServerIsOn(serverName: string, isOn: boolean): Promise<pegasusai>;
...
class MCPService extends Disposable implements IMCPService {
	...
	constructor(
		...
		super();
		this.channel = this.mainProcessService.getChannel('pegasusai-channel-mcp')
	...
	private async _initialize() {
		try {
			await this.pegasusaiSettingsService.waitForInitState;
	...
	private async _createMCPConfigFile(mcpConfigUri: URI): Promise<pegasusai> {
	...
	private async _addMCPConfigFileWatcher(): Promise<pegasusai> {
		const mcpConfigUri = await this._getMCPConfigFilePath();
		this._register(
	...
	public async revealMCPConfigFile(): Promise<pegasusai> {
		try {
			const mcpConfigUri = await this._getMCPConfigFilePath();
	...
	private async _refreshMCPServers(): Promise<pegasusai> {
		...
		await this.pegasusaiSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.pegasusaiSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.pegasusaiSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	// toggle MCP server and update isOn in pegasusai settings
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<pegasusai> {
		this._setMCPServerState(serverName, { status: 'loading', tools: [] })

		await this.pegasusaiSettingsService.setMCPServerState(serverName, { isOn });
