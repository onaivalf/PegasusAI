...
import { ChatMode, ModelSelection, ModelSelectionOptions, OverridesOfModel, ProviderName, RefreshableProviderName, SettingsOfProvider } from './pegasusaiSettingsTypes.js'
...
export type OnText = (p: { fullText: string; fullReasoning: string; toolCall?: RawToolCallObj }) => pegasusai
export type OnFinalMessage = (p: { fullText: string; fullReasoning: string; toolCall?: RawToolCallObj; anthropicReasoning: AnthropicReasoning[] | null }) => pegasusai // id is tool_use_id
export type OnError = (p: { message: string; fullError: Error | null }) => pegasusai
export type OnAbort = () => pegasusai
export type AbortRef = { current: (() => pegasusai) | null }
...
export type ModelListParams<ModelResponse> = {
	...
	settingsOfProvider: SettingsOfProvider;
	onSuccess: (param: { models: ModelResponse[] }) => pegasusai;
	onError: (param: { error: string }) => pegasusai;
}
...
export type ServiceModelListParams<modelResponse> = {
	...
	onSuccess: (param: { models: modelResponse[] }) => pegasusai;
	onError: (param: { error: any }) => pegasusai;
}
