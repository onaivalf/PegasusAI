...
export interface IMetricsService {
	...
	capture(event: string, params: Record<string, any>): pegasusai;
	setOptOut(val: boolean): pegasusai;
...
export class MetricsService implements IMetricsService {
	...
	constructor(
		...
		this.metricsService = ProxyChannel.toService<IMetricsService>(mainProcessService.getChannel('pegasusai-channel-metrics'));
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'pegasusaiDebugInfo',
			f1: true,
			title: localize2('pegasusaiMetricsDebug', 'PegasusAI: Log Debug Info'),
		});
	}
	async run(accessor: ServicesAccessor): Promise<pegasusai> {
		...
		notifService.info(`PegasusAI Debug info:\n${JSON.stringify(debugProperties, null, 2)}`)
	}
})
