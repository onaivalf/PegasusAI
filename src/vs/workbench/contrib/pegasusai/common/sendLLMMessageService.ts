...
import { IPegasusAISettingsService } from './pegasusaiSettingsService.js';
...
export interface ILLMMessageService {
	...
	abort: (requestId: string) => pegasusai;
	ollamaList: (params: ServiceModelListParams<OllamaModelResponse>) => pegasusai;
	openAICompatibleList: (params: ServiceModelListParams<OpenaiCompatibleModelResponse>) => pegasusai;
}
...
export class LLMMessageService extends Disposable implements ILLMMessageService {
	...
	private readonly llmMessageHooks = {
		onText: {} as { [eventId: string]: ((params: EventLLMMessageOnTextParams) => pegasusai) },
		onFinalMessage: {} as { [eventId: string]: ((params: EventLLMMessageOnFinalMessageParams) => pegasusai) },
		onError: {} as { [eventId: string]: ((params: EventLLMMessageOnErrorParams) => pegasusai) },
		onAbort: {} as { [eventId: string]: (() => pegasusai) }, // NOT sent over the channel, result is instant when we call .abort()
	}

	// list hooks
	private readonly listHooks = {
		ollama: {
			success: {} as { [eventId: string]: ((params: EventModelListOnSuccessParams<OllamaModelResponse>) => pegasusai) },
			error: {} as { [eventId: string]: ((params: EventModelListOnErrorParams<OllamaModelResponse>) => pegasusai) },
		},
		openAICompat: {
			success: {} as { [eventId: string]: ((params: EventModelListOnSuccessParams<OpenaiCompatibleModelResponse>) => pegasusai) },
			error: {} as { [eventId: string]: ((params: EventModelListOnErrorParams<OpenaiCompatibleModelResponse>) => pegasusai) },
		}
	} satisfies {
		[providerName in 'ollama' | 'openAICompat']: {
			success: { [eventId: string]: ((params: EventModelListOnSuccessParams<any>) => pegasusai) },
			error: { [eventId: string]: ((params: EventModelListOnErrorParams<any>) => pegasusai) },
		}
	}
	...
	constructor(
		...
		// const service = ProxyChannel.toService<LLMMessageChannel>(mainProcessService.getChannel('pegasusai-channel-sendLLMMessage')); // lets you call it like a service
		// see llmMessageChannel.ts
		this.channel = this.mainProcessService.getChannel('pegasusai-channel-llmMessage')
	...
	sendLLMMessage(params: ServiceSendLLMMessageParams) {
		...
		if (modelSelection === null) {
			const message = `Please add a provider in PegasusAI's Settings.`
		...
		const { settingsOfProvider, } = this.pegasusaiSettingsService.state
