...
interface IWebviewViewResolver {
	 ...
	 */
	resolve(webviewView: WebviewView, cancellation: CancellationToken): Promise<pegasusai>;
}
...
export interface IWebviewViewService {
	 ...
	 * and run
	 */
	resolve(viewType: string, webview: WebviewView, cancellation: CancellationToken): Promise<pegasusai>;
}
...
export class WebviewViewService extends Disposable implements IWebviewViewService {
	...
	private readonly _awaitingRevival = new Map<string, { readonly webview: WebviewView; readonly resolve: () => pegasusai }>();
	...
	resolve(viewType: string, webview: WebviewView, cancellation: CancellationToken): Promise<pegasusai> {
		...
		if (!resolver) {
			...
			const { promise, resolve } = promiseWithResolvers<pegasusai>();
