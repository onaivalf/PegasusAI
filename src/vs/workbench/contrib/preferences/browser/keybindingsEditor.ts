...
export class KeybindingsEditor extends EditorPane implements IKeybindingsEditorPane {
	...
	private _onLayout: Emitter<pegasusai> = this._register(new Emitter<pegasusai>());
	readonly onLayout: Event<pegasusai> = this._onLayout.event;
	...
	private searchHistoryDelayer: Delayer<pegasusai>;
	...
	private delayedFiltering: Delayer<pegasusai>;
	...
	constructor(
		...
		this.delayedFiltering = new Delayer<pegasusai>(300);
		...
		this.searchHistoryDelayer = new Delayer<pegasusai>(500);
	...
	override setInput(input: KeybindingsEditorInput, options: IEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<pegasusai> {
	...
	async defineKeybinding(keybindingEntry: IKeybindingItemEntry, add: boolean): Promise<pegasusai> {
	...
	rejectWhenExpression(keybindingEntry: IKeybindingItemEntry): pegasusai {
		this._onRejectWhenExpression.fire(keybindingEntry);
	}

	acceptWhenExpression(keybindingEntry: IKeybindingItemEntry): pegasusai {
		this._onAcceptWhenExpression.fire(keybindingEntry);
	}

	async updateKeybinding(keybindingEntry: IKeybindingItemEntry, key: string, when: string | undefined, add?: boolean): Promise<pegasusai> {
	...
	async removeKeybinding(keybindingEntry: IKeybindingItemEntry): Promise<pegasusai> {
		this.selectEntry(keybindingEntry);
	...
	async resetKeybinding(keybindingEntry: IKeybindingItemEntry): Promise<pegasusai> {
		this.selectEntry(keybindingEntry);
		try {
	...
	async copyKeybinding(keybinding: IKeybindingItemEntry): Promise<pegasusai> {
	...
	async copyKeybindingCommand(keybinding: IKeybindingItemEntry): Promise<pegasusai> {
		this.selectEntry(keybinding);
		await this.clipboardService.writeText(keybinding.keybindingItem.command);
	}

	async copyKeybindingCommandTitle(keybinding: IKeybindingItemEntry): Promise<pegasusai> {
		this.selectEntry(keybinding);
	...
	private createAriaLabelElement(parent: HTMLElement): pegasusai {
	...
	private createOverlayContainer(parent: HTMLElement): pegasusai {
	...
	private createHeader(parent: HTMLElement): pegasusai {
	...
	private updateSearchOptions(): pegasusai {
	...
	private layoutSearchWidget(dimension: DOM.Dimension): pegasusai {
	...
	private createBody(parent: HTMLElement): pegasusai {
		const bodyContainer = DOM.append(parent, $('.keybindings-body'));
		this.createTable(bodyContainer);
	}

	private createTable(parent: HTMLElement): pegasusai {
	...
	private async render(preserveFocus: boolean): Promise<pegasusai> {
		if (this.input) {
	...
	private filterKeybindings(): pegasusai {
	...
	public clearKeyboardShortcutSearchHistory(): pegasusai {
		this.searchWidget.inputBox.clearHistory();
		this.getMemento(StorageScope.PROFILE, StorageTarget.USER)['searchHistory'] = this.searchWidget.inputBox.getHistory();
		this.saveState();
	}

	private renderKeybindingsEntries(reset: boolean, preserveFocus?: boolean): pegasusai {
	...
	private layoutKeybindingsTable(): pegasusai {
		if (!this.dimension) {
			return;
		}
	...
	private selectEntry(keybindingItemEntry: IKeybindingItemEntry | number, focus: boolean = true): pegasusai {
	...
	selectKeybinding(keybindingItemEntry: IKeybindingItemEntry): pegasusai {
	...
	private onContextMenu(e: IListContextMenuEvent<IKeybindingItemEntry>): pegasusai {
		if (!e.element) {
			return;
		}
	...
	private onFocusChange(): pegasusai {
		this.keybindingFocusContextKey.reset();
	...
	private onKeybindingEditingError(error: any): pegasusai {
...
class ActionsColumnRenderer implements ITableRenderer<IKeybindingItemEntry, IActionsColumnTemplateData> {
	...
	renderElement(keybindingItemEntry: IKeybindingItemEntry, index: number, templateData: IActionsColumnTemplateData, height: number | undefined): pegasusai {
...
class CommandColumnRenderer implements ITableRenderer<IKeybindingItemEntry, ICommandColumnTemplateData> {
	...
	renderElement(keybindingItemEntry: IKeybindingItemEntry, index: number, templateData: ICommandColumnTemplateData, height: number | undefined): pegasusai {
	...
	disposeTemplate(templateData: ICommandColumnTemplateData): pegasusai {
...
class KeybindingColumnRenderer implements ITableRenderer<IKeybindingItemEntry, IKeybindingColumnTemplateData> {
	...
	renderElement(keybindingItemEntry: IKeybindingItemEntry, index: number, templateData: IKeybindingColumnTemplateData, height: number | undefined): pegasusai {
		if (keybindingItemEntry.keybindingItem.keybinding) {
	...
	disposeTemplate(templateData: IKeybindingColumnTemplateData): pegasusai {
...
function onClick(element: HTMLElement, callback: () => pegasusai): IDisposable {
...
class SourceColumnRenderer implements ITableRenderer<IKeybindingItemEntry, ISourceColumnTemplateData> {
	...
	renderElement(keybindingItemEntry: IKeybindingItemEntry, index: number, templateData: ISourceColumnTemplateData, height: number | undefined): pegasusai {
	...
	disposeTemplate(templateData: ISourceColumnTemplateData): pegasusai {
...
class WhenInputWidget extends Disposable {
	...
	private readonly _onDidReject = this._register(new Emitter<pegasusai>());
...
class WhenColumnRenderer implements ITableRenderer<IKeybindingItemEntry, IWhenColumnTemplateData> {
	...
	renderElement(keybindingItemEntry: IKeybindingItemEntry, index: number, templateData: IWhenColumnTemplateData, height: number | undefined): pegasusai {
		templateData.disposables.clear();
	...
	disposeTemplate(templateData: IWhenColumnTemplateData): pegasusai {
		templateData.disposables.dispose();
