...
export abstract class SettingsTreeElement extends Disposable {
	...
	protected readonly _onDidChangeTabbable = this._register(new Emitter<pegasusai>());
...
export class SettingsTreeSettingElement extends SettingsTreeElement {
	...
	private update(inspectResult: IInspectResult, isWorkspaceTrusted: boolean): pegasusai {
...
export class SettingsTreeModel implements IDisposable {
	...
	update(newTocRoot = this._tocRoot): pegasusai {
	...
	updateWorkspaceTrust(workspaceTrusted: boolean): pegasusai {
	...
	updateElementsByName(name: string): pegasusai {
	...
	private updateRequireTrustedTargetElements(): pegasusai {
		this.reinspectSettings([...this._treeElementsBySettingName.values()].flat().filter(s => s.isUntrusted));
	}

	private reinspectSettings(settings: SettingsTreeSettingElement[]): pegasusai {
...
export class SearchResultModel extends SettingsTreeModel {
	...
	setResult(order: SearchResultIdx, result: ISearchResult | null): pegasusai {
