...
export class UserDataSyncWorkbenchContribution extends Disposable implements IWorkbenchContribution {
	...
	private async acceptLocal(syncResource: IUserDataSyncResource, conflict: IResourcePreview): Promise<pegasusai> {
		try {
	...
	private onAutoSyncError(error: UserDataSyncError): pegasusai {
		switch (error.code) {
	...
	private handleTooLargeError(resource: SyncResource, message: string, error: UserDataSyncError): pegasusai {
		const operationId = error.operationId ? localize('operationId', "Operation Id: {0}", error.operationId) : undefined;
		this.notificationService.notify({
			severity: Severity.Error,
			message: operationId ? `${message} ${operationId}` : message,
	...
	private onSynchronizerErrors(errors: IUserDataSyncResourceError[]): pegasusai {
		if (errors.length) {
	...
	private handleInvalidContentError({ profile, syncResource: source }: IUserDataSyncResource): pegasusai {
	...
	private async updateGlobalActivityBadge(): Promise<pegasusai> {
	...
	private async updateAccountBadge(): Promise<pegasusai> {
	...
	private async turnOn(): Promise<pegasusai> {
		try {
	...
	private updateConfiguration(items: ConfigureSyncQuickPickItem[], selectedItems: ReadonlyArray<ConfigureSyncQuickPickItem>): pegasusai {
	...
	private async configureSyncOptions(): Promise<pegasusai> {
	...
	private async turnOff(): Promise<pegasusai> {
	...
	private disableSync(source: SyncResource): pegasusai {
		switch (source) {
	...
	private showSyncActivity(): Promise<pegasusai> {
		return this.outputService.showChannel(USER_DATA_SYNC_LOG_ID);
	}

	private async selectSettingsSyncService(userDataSyncStore: IUserDataSyncStore): Promise<pegasusai> {
	...
	private registerActions(): pegasusai {
	...
	private registerTurnOnSyncAction(): pegasusai {
		const that = this;
	...
	private registerTurningOnSyncAction(): pegasusai {
	...
	private registerCancelTurnOnSyncAction(): pegasusai {
		const that = this;
		this._register(registerAction2(class TurningOnSyncAction extends Action2 {
			constructor() {
				super({
	...
	private registerSignInAction(): pegasusai {
		const that = this;
	...
	private registerShowConflictsAction(): pegasusai {
	...
	private registerManageSyncAction(): pegasusai {
		const that = this;
	...
	private registerEnableSyncViewsAction(): pegasusai {
		const that = this;
		...
		this._register(registerAction2(class SyncStatusAction extends Action2 {
			...
			run(accessor: ServicesAccessor): Promise<pegasusai> {
				return that.userDataSyncWorkbenchService.showSyncActivity();
			}
		}));
	}

	private registerSyncNowAction(): pegasusai {
		const that = this;
	...
	private registerTurnOffSyncAction(): pegasusai {
		const that = this;
		this._register(registerAction2(class StopSyncAction extends Action2 {
			constructor() {
				super({
	...
	private registerConfigureSyncAction(): pegasusai {
		const that = this;
	...
	private registerShowLogAction(): pegasusai {
		const that = this;
	...
	private registerShowSettingsAction(): pegasusai {
	...
	private registerHelpAction(): pegasusai {
		const that = this;
	...
	private registerAcceptMergesAction(): pegasusai {
		const that = this;
		this._register(registerAction2(class AcceptMergesAction extends Action2 {
			...
			async run(accessor: ServicesAccessor, previewResource: URI): Promise<pegasusai> {
	...
	private registerDownloadSyncActivityAction(): pegasusai {
	...
	private registerViews(): pegasusai {
	...
	private registerResetSyncDataAction(): pegasusai {
		const that = this;
	...
	private registerDataViews(container: ViewContainer): pegasusai {
