...
export class MultiDiffEditorInput extends EditorInput implements ILanguageSupport {
	...
	public setLanguageId(languageId: string, source?: string | undefined): pegasusai {
	...
	override  revert(group: GroupIdentifier, options?: IRevertOptions): Promise<pegasusai> {
		return this.doSaveOrRevert('revert', group, options);
	}

	private async doSaveOrRevert(mode: 'save', group: GroupIdentifier, options?: ISaveOptions): Promise<pegasusai>;
	private async doSaveOrRevert(mode: 'revert', group: GroupIdentifier, options?: IRevertOptions): Promise<pegasusai>;
	private async doSaveOrRevert(mode: 'save' | 'revert', group: GroupIdentifier, options?: ISaveOptions | IRevertOptions): Promise<pegasusai> {
...
class FastEventDispatcher<T, TKey> {
	private _count = 0;
	private readonly _buckets = new Map<string, Set<(value: T) => pegasusai>>();
