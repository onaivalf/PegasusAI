...
class TestCommentController implements ICommentController {
	...
	owner: string = 'test';
	features = {};
	createCommentThreadTemplate(resource: UriComponents, range: IRange | undefined): Promise<pegasusai> {
	...
	toggleReaction(uri: URI, thread: CommentThread<IRange>, comment: Comment, reaction: CommentReaction, token: CancellationToken): Promise<pegasusai> {
		throw new Error('Method not implemented.');
	}
