...
registerAction2(class extends Action2 {
	...
	override run(accessor: ServicesAccessor, args?: { range: IRange; fileComment: boolean }): pegasusai {
		const activeEditor = getActiveEditor(accessor);
		if (!activeEditor) {
			return;
		}
...
registerAction2(class extends Action2 {
	...
	override async run(accessor: ServicesAccessor, args?: { range: IRange; fileComment: boolean }): Promise<pegasusai> {
		const activeEditor = getActiveEditor(accessor);
		if (!activeEditor) {
			return;
		}
