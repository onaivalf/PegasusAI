...
export class ReviewZoneWidget extends ZoneWidget implements ICommentThreadWidget {
	...
	private deleteCommentThread(): pegasusai {
		this.dispose();
	...
	_refresh(dimensions: dom.Dimension) {
		...
		if (this._isExpanded) {
			...
			const computedLinesNumber = Math.ceil((headHeight + dimensions.height + arrowHeight + frameThickness + 8 /** margin bottom to apegasusai margin collapse */) / lineHeight);
	...
	override show(rangeOrPos: IRange | IPosition | undefined, heightInLines: number): pegasusai {
