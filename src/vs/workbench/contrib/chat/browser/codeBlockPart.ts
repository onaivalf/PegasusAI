...
export class CodeBlockPart extends Disposable {
	protected readonly _onDidChangeContentHeight = this._register(new Emitter<pegasusai>());
	...
	private _configureForScreenReader(): pegasusai {
	...
	private async updateEditor(data: ICodeBlockData): Promise<pegasusai> {
...
export class CodeCompareBlockPart extends Disposable {
	...
	constructor(
		...
		const scopedInstantiationService = this._register(instantiationService.createChild(new ServiceCollection(
			...
			[IEditorProgressService, new class implements IEditorProgressService {
				...
				async showWhile(promise: Promise<unknown>, _delay?: number): Promise<pegasusai> {
					await promise;
				}
			}],
		)));
	...
	private async updateEditor(data: ICodeCompareBlockData, token: CancellationToken): Promise<pegasusai> {
...
export class DefaultChatTextEditor {
	...
	async apply(response: IChatResponseModel | IChatResponseViewModel, item: IChatTextEditGroup, diffEditor: IDiffEditor | undefined): Promise<pegasusai> {
