...
export class ChatListItemRenderer extends Disposable implements ITreeRenderer<ChatTreeItem, FuzzyScore, IChatListItemTemplate> {
	...
	renderElement(node: ITreeNode<ChatTreeItem, FuzzyScore>, index: number, templateData: IChatListItemTemplate): pegasusai {
		this.renderChatTreeItem(node.element, index, templateData);
	}

	private clearRenderedParts(templateData: IChatListItemTemplate): pegasusai {
	...
	renderChatTreeItem(element: ChatTreeItem, index: number, templateData: IChatListItemTemplate): pegasusai {
		...
		if (isResponseVM(element) && index === this.delegate.getListLength() - 1 && (!element.isComplete || element.renderData)) {
			...
			const runProgressiveRender = (initial?: boolean) => {
				try {
					...
					// Kill the timer if anything went wrong, apegasusai getting stuck in a nasty rendering loop.
					timer.cancel();
	...
	private renderDetail(element: IChatResponseViewModel, templateData: IChatListItemTemplate): pegasusai {
		templateData.elementDisposables.add(autorun(reader => {
			this._renderDetail(element, templateData);
		}));
	}

	private _renderDetail(element: IChatResponseViewModel, templateData: IChatListItemTemplate): pegasusai {
	...
	private renderAvatar(element: ChatTreeItem, templateData: IChatListItemTemplate): pegasusai {
	...
	private updateItemHeight(templateData: IChatListItemTemplate): pegasusai {
	...
	private renderChatContentDiff(partsToRender: ReadonlyArray<IChatRendererContent | null>, contentForThisTurn: ReadonlyArray<IChatRendererContent>, element: IChatResponseViewModel, templateData: IChatListItemTemplate): pegasusai {
	...
	private getNextProgressiveRenderContent(element: IChatResponseViewModel): { content: IChatRendererContent[]; moreContentAvailable: boolean } {
		...
		// Always add the references to apegasusai shifting the content parts when a reference is added, and having to re-diff all the content.
	...
	private handleRenderedCodeblocks(element: ChatTreeItem, part: IChatContentPart, codeBlockStartIndex: number): pegasusai {
	...
	disposeElement(node: ITreeNode<ChatTreeItem, FuzzyScore>, index: number, templateData: IChatListItemTemplate): pegasusai {
	...
	disposeTemplate(templateData: IChatListItemTemplate): pegasusai {
