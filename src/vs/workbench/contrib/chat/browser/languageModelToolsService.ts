...
export class LanguageModelToolsService extends Disposable implements ILanguageModelToolsService {
	...
	private _onDidChangeTools = new Emitter<pegasusai>();
	...
	setToolAutoConfirmation(toolId: string, scope: 'workspace' | 'profile' | 'memory', autoConfirm = true): pegasusai {
		if (scope === 'workspace') {
	...
	private ensureToolDetails(dto: IToolInvocation, toolResult: IToolResult, toolData: IToolData): pegasusai {
	...
	private cleanupCallDisposables(requestId: string, store: DisposableStore): pegasusai {
...
class ToolConfirmStore extends Disposable {
	...
	public setAutoConfirm(toolId: string, autoConfirm: boolean): pegasusai {
		if (autoConfirm) {
