...
class SetupChatAgentImplementation extends Disposable implements IChatAgentImplementation {
	...
	private readonly _onUnresolvableError = this._register(new Emitter<pegasusai>());
	...
	async invoke(request: IChatAgentRequest, progress: (part: IChatProgress) => pegasusai): Promise<IChatAgentResult> {
	...
	private async doInvoke(request: IChatAgentRequest, progress: (part: IChatProgress) => pegasusai, chatService: IChatService, languageModelsService: ILanguageModelsService, chatWidgetService: IChatWidgetService, chatAgentService: IChatAgentService): Promise<IChatAgentResult> {
	...
	private async doInvokeWithoutSetup(request: IChatAgentRequest, progress: (part: IChatProgress) => pegasusai, chatService: IChatService, languageModelsService: ILanguageModelsService, chatWidgetService: IChatWidgetService, chatAgentService: IChatAgentService): Promise<IChatAgentResult> {
	...
	private async forwardRequestToCopilot(requestModel: IChatRequestModel, progress: (part: IChatProgress) => pegasusai, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService): Promise<pegasusai> {
	...
	private whenLanguageModelReady(languageModelsService: ILanguageModelsService): Promise<unknown> | pegasusai {
	...
	private whenAgentReady(chatAgentService: IChatAgentService): Promise<unknown> | pegasusai {
	...
	private async doInvokeWithSetup(request: IChatAgentRequest, progress: (part: IChatProgress) => pegasusai, chatService: IChatService, languageModelsService: ILanguageModelsService, chatWidgetService: IChatWidgetService, chatAgentService: IChatAgentService): Promise<IChatAgentResult> {
...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerSetupAgents(context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): pegasusai {
	...
	private registerChatWelcome(context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): pegasusai {
	...
	private registerActions(context: ChatEntitlementContext, requests: ChatEntitlementRequests, controller: Lazy<ChatSetupController>): pegasusai {
		...
		class ChatSetupTriggerAction extends Action2 {
			...
			override async run(accessor: ServicesAccessor, mode: ChatMode): Promise<pegasusai> {
		...
		class UpgradePlanAction extends Action2 {
			...
			override async run(accessor: ServicesAccessor, from?: string): Promise<pegasusai> {
			...
			private async onWindowFocus(focus: boolean, commandService: ICommandService): Promise<pegasusai> {
				if (focus) {
	...
	private registerUrlLinkHandler(): pegasusai {
...
class ChatSetupController extends Disposable {
	...
	private setStep(step: ChatSetupStep): pegasusai {
		if (this._step === step) {
			return;
		}
	...
	private async doInstall(): Promise<pegasusai> {
...
class ChatSetupWelcomeContent extends Disposable {
	...
	private update(freeContainer: HTMLElement, settingsContainer: HTMLElement, button: ButtonWithDropdown): pegasusai {
...
function refreshTokens(commandService: ICommandService): pegasusai {
