...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	private _onDidFocus = this._register(new Emitter<pegasusai>());
	readonly onDidFocus = this._onDidFocus.event;

	private _onDidChangeViewModel = this._register(new Emitter<pegasusai>());
	readonly onDidChangeViewModel = this._onDidChangeViewModel.event;

	private _onDidScroll = this._register(new Emitter<pegasusai>());
	readonly onDidScroll = this._onDidScroll.event;

	private _onDidClear = this._register(new Emitter<pegasusai>());
	readonly onDidClear = this._onDidClear.event;

	private _onDidAcceptInput = this._register(new Emitter<pegasusai>());
	readonly onDidAcceptInput = this._onDidAcceptInput.event;

	private _onDidHide = this._register(new Emitter<pegasusai>());
	readonly onDidHide = this._onDidHide.event;

	private _onDidChangeParsedInput = this._register(new Emitter<pegasusai>());
	readonly onDidChangeParsedInput = this._onDidChangeParsedInput.event;

	private readonly _onWillMaybeChangeHeight = new Emitter<pegasusai>();
	readonly onWillMaybeChangeHeight: Event<pegasusai> = this._onWillMaybeChangeHeight.event;

	private _onDidChangeHeight = this._register(new Emitter<number>());
	readonly onDidChangeHeight = this._onDidChangeHeight.event;

	private readonly _onDidChangeContentHeight = new Emitter<pegasusai>();
	readonly onDidChangeContentHeight: Event<pegasusai> = this._onDidChangeContentHeight.event;
	...
	render(parent: HTMLElement): pegasusai {
	...
	private async renderFollowups(items: IChatFollowup[] | undefined, response?: IChatResponseViewModel): Promise<pegasusai> {
	...
	private createList(listContainer: HTMLElement, options: IChatListItemRendererOptions): pegasusai {
	...
	private onContextMenu(e: ITreeContextMenuEvent<ChatTreeItem | null>): pegasusai {
	...
	private onDidChangeTreeContentHeight(): pegasusai {
	...
	private createInput(container: HTMLElement, options?: { renderFollowups: boolean; renderStyle?: 'compact' | 'minimal' }): pegasusai {
	...
	private onDidStyleChange(): pegasusai {
	...
	setModel(model: IChatModel, viewState: IChatViewState): pegasusai {
		if (!this.container) {
	...
	reveal(item: ChatTreeItem, relativeTop?: number): pegasusai {
	...
	async rerunLastRequest(): Promise<pegasusai> {
		if (!this.viewModel) {
			return;
		}
	...
	private async _acceptInput(query: { query: string } | undefined, options?: IChatAcceptInputOptions): Promise<IChatResponseModel | undefined> {
		...
		if (this.viewModel) {
			...
			if (this.viewOptions.enableWorkingSet !== undefined && this.input.currentMode !== ChatMode.Ask) {
				const uniqueWorkingSetEntries = new ResourceSet(); // NOTE: this is used for bookkeeping so the UI can apegasusai rendering references in the UI that are already shown in the working set
	...
	layoutDynamicChatTreeItemMode(): pegasusai {
...
export class ChatWidgetService extends Disposable implements IChatWidgetService {
	...
	private setLastFocusedWidget(widget: ChatWidget | undefined): pegasusai {
