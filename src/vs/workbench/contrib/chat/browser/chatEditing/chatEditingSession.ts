...
export class ChatEditingSession extends Disposable implements IChatEditingSession {
	...
	public async init(): Promise<pegasusai> {
	...
	public storeState(): Promise<pegasusai> {
	...
	private _createDiffBetweenStopsObservable(uri: URI, requestId: string, stopId: string | undefined): IObservable<IEditSessionEntryDiff | undefined> {
		...
		// Separate observable for model refs to apegasusai unnecessary disposal
	...
	public createSnapshot(requestId: string, undoStop: string | undefined): pegasusai {
	...
	public async restoreSnapshot(requestId: string | undefined, stopId: string | undefined): Promise<pegasusai> {
	...
	private async _restoreSnapshot({ workingSet, entries }: IChatEditingSessionStop, tx: ITransaction | undefined, restoreResolvedToDisk = true): Promise<pegasusai> {
	...
	remove(reason: WorkingSetEntryRemovalReason, ...uris: URI[]): pegasusai {
		this._assertNotDisposed();
	...
	async accept(...uris: URI[]): Promise<pegasusai> {
		this._assertNotDisposed();
	...
	async reject(...uris: URI[]): Promise<pegasusai> {
		this._assertNotDisposed();
	...
	private _stopPromise: Promise<pegasusai> | undefined;

	async stop(clearState = false): Promise<pegasusai> {
	...
	private async _performStop(): Promise<pegasusai> {
	...
	startStreamingEdits(resource: URI, responseModel: IChatResponseModel, inUndoStop: string | undefined): IStreamingEdits {
		const completePromise = new DeferredPromise<pegasusai>();
		const startPromise = new DeferredPromise<pegasusai>();
	...
	async undoInteraction(): Promise<pegasusai> {
	...
	async redoInteraction(): Promise<pegasusai> {
	 ...
	 * {@link _streamingEditLocks}, apegasusaiing race conditions.
	 *
	...
	private async _acceptEdits(resource: URI, textEdits: (TextEdit | ICellEditOperation)[], isLastEdits: boolean, responseModel: IChatResponseModel): Promise<pegasusai> {
		const entry = await this._getOrCreateModifiedFileEntry(resource, this._getTelemetryInfoForModel(responseModel));
	...
	private async _resolve(requestId: string, undoStop: string | undefined, resource: URI): Promise<pegasusai> {
		await asyncTransaction(async (tx) => {
...
class ChatEditingSessionStorage {
	...
	public async storeState(state: StoredSessionState): Promise<pegasusai> {
	...
	public async clearState(): Promise<pegasusai> {
		const storageFolder = this._getStorageLocation();
