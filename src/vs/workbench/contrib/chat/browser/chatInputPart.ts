...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	private _onDidChangeHeight = this._register(new Emitter<pegasusai>());
	...
	private _onDidChangeCurrentChatMode = this._register(new Emitter<pegasusai>());
	...
	public switchToNextModel(): pegasusai {
	...
	initForNewChatModel(state: IChatViewState, modelIsEmpty: boolean): pegasusai {
		this.history = this.loadHistory();
		this.history.add({
	...
	async showPreviousValue(): Promise<pegasusai> {
	...
	async showNextValue(): Promise<pegasusai> {
		const inputState = this.getInputState();
		if (this.history.isAtEnd()) {
			return;
		} else {
	...
	private async navigateHistory(previous: boolean): Promise<pegasusai> {
	...
	setValue(value: string, transient: boolean): pegasusai {
	...
	private saveCurrentValue(inputState: IChatInputState): pegasusai {
	 ...
	 */
	async acceptInput(isUserQuery?: boolean): Promise<pegasusai> {
		if (isUserQuery) {
	...
	private _acceptInputForVoiceover(): pegasusai {
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
		...
		this.executeToolbar = this._register(this.instantiationService.createInstance(MenuWorkbenchToolBar, toolbarsContainer, this.options.menus.executeToolbar, {
			...
			hoverDelegate,
			hiddenItemStrategy: HiddenItemStrategy.Ignore, // keep it lean when hiding items and apegasusai a "..." overflow menu
	...
	async renderFollowups(items: IChatFollowup[] | undefined, response: IChatResponseViewModel | undefined): Promise<pegasusai> {
	...
	private _layout(height: number, width: number, allowRecurse = true): pegasusai {
		const data = this.getLayoutData();
...
interface ModelPickerDelegate {
	...
	setModel(selectedModelId: ILanguageModelChatMetadataAndIdentifier): pegasusai;
...
interface IModePickerDelegate {
	onDidChangeMode: Event<pegasusai>;
	getMode(): ChatMode;
}
