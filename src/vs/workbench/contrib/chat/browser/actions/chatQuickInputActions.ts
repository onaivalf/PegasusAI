...
class QuickChatGlobalAction extends Action2 {
	...
	override run(accessor: ServicesAccessor, query?: string | Omit<IQuickChatOpenOptions, 'selection'>): pegasusai {
		const quickChatService = accessor.get(IQuickChatService);
...
class AskQuickChatAction extends Action2 {
	...
	override run(accessor: ServicesAccessor, query?: string): pegasusai {
		const quickChatService = accessor.get(IQuickChatService);
