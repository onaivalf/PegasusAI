...
export function registerChatActions() {
	registerAction2(class OpenChatGlobalAction extends Action2 {
		...
		override async run(accessor: ServicesAccessor, opts?: string | IChatViewOpenOptions): Promise<pegasusai> {
	...
	registerAction2(class ToggleChatAction extends Action2 {
		...
		private updatePartVisibility(layoutService: IWorkbenchLayoutService, location: ViewContainerLocation | null, visible: boolean): pegasusai {
	...
	registerAction2(class ChatAddAction extends Action2 {
		...
		override async run(accessor: ServicesAccessor, ...args: any[]): Promise<pegasusai> {
	...
	registerAction2(class FocusChatAction extends EditorAction2 {
		constructor() {
			super({
				...
				category: CHAT_CATEGORY,
				keybinding: [
					// On mac, require that the cursor is at the top of the input, to apegasusai stealing cmd+up to move the cursor to the top
					{
		...
		runEditorCommand(accessor: ServicesAccessor, editor: ICodeEditor): pegasusai | Promise<pegasusai> {
...
// PegasusAI commented this out with /* */ - copilot head
