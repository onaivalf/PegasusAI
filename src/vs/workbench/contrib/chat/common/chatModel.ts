...
export interface IChatResponseModel {
	...
	addUndoStop(undoStop: IChatUndoStop): pegasusai;
	setVote(vote: ChatAgentVoteDirection): pegasusai;
	setVoteDownReason(reason: ChatAgentVoteDownReason | undefined): pegasusai;
	setEditApplied(edit: IChatTextEditGroup, editCount: number): boolean;
	setPaused(isPause: boolean, tx?: ITransaction): pegasusai;
	/**
...
export class Response extends AbstractResponse implements IDisposable {
	private _onDidChangeValue = new Emitter<pegasusai>();
	...
	updateContent(progress: IChatProgressResponseContent | IChatTextEdit | IChatNotebookEdit | IChatTask | IChatUndoStop, quiet?: boolean): pegasusai {
...
export class ChatResponseModel extends Disposable implements IChatResponseModel {
	...
	private bufferedPauseContent?: (() => pegasusai)[];
	...
	setResult(result: IChatAgentResult): pegasusai {
		this._result = result;
		this._onDidChange.fire(defaultChatResponseModelChangeReason);
	}
	...
	setFollowups(followups: IChatFollowup[] | undefined): pegasusai {
	...
	private bufferWhenPaused(apply: () => pegasusai) {
...
export interface IChatModel {
	...
	toggleLastRequestPaused(paused?: boolean): pegasusai;
	/**
	 ...
	 */
	setDisabledRequests(requestIds: IChatRequestDisablement[]): pegasusai;
...
function normalizeOldFields(raw: ISerializableChatDataIn): pegasusai {
...
export class ChatModel extends Disposable implements IChatModel {
	...
	private _isInitializedDeferred = new DeferredPromise<pegasusai>();
	...
	startEditingSession(isGlobalEditingSession?: boolean): pegasusai {
	...
	deinitialize(): pegasusai {
		this._initState = ChatModelInitState.Created;
		this._isInitializedDeferred = new DeferredPromise<pegasusai>();
	}

	initialize(sampleQuestions?: IChatFollowup[]): pegasusai {
	...
	setInitializationError(error: Error): pegasusai {
		if (this.initState !== ChatModelInitState.Initializing) {
	...
	waitForInitialization(): Promise<pegasusai> {
	...
	setCustomTitle(title: string): pegasusai {
	...
	adoptRequest(request: ChatRequestModel): pegasusai {
	...
	acceptResponseProgress(request: ChatRequestModel, progress: IChatProgress, quiet?: boolean): pegasusai {
		if (!request.response) {
	...
	removeRequest(id: string, reason: ChatRequestRemovalReason = ChatRequestRemovalReason.Removal): pegasusai {
	...
	cancelRequest(request: ChatRequestModel): pegasusai {
		if (request.response) {
			request.response.cancel();
		}
	}

	setResponse(request: ChatRequestModel, result: IChatAgentResult): pegasusai {
		if (!request.response) {
			request.response = new ChatResponseModel([], this, undefined, undefined, request.id);
		}
	...
	completeResponse(request: ChatRequestModel): pegasusai {
		if (!request.response) {
	...
	setFollowups(request: ChatRequestModel, followups: IChatFollowup[] | undefined): pegasusai {
		if (!request.response) {
	...
	setResponseModel(request: ChatRequestModel, response: ChatResponseModel): pegasusai {
