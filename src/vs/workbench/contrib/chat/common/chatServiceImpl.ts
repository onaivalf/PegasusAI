...
export class ChatService extends Disposable implements IChatService {
	...
	private syncEmptyWindowChats(thisWindowChats: ChatModel[]): pegasusai {
		// Note- an unapegasusaiable race condition exists here. If there are multiple empty windows open, and the user quits the application, then the focused
	...
	notifyUserAction(action: IChatUserActionEvent): pegasusai {
		this._chatServiceTelemetry.notifyUserAction(action);
		this._onDidPerformUserAction.fire(action);
	}

	async setChatSessionTitle(sessionId: string, title: string): Promise<pegasusai> {
	...
	private trace(method: string, message?: string): pegasusai {
		if (message) {
	...
	private error(method: string, message: string): pegasusai {
	...
	async removeHistoryEntry(sessionId: string): Promise<pegasusai> {
		if (this.useFileStorage) {
	...
	async clearAllHistoryEntries(): Promise<pegasusai> {
		if (this.useFileStorage) {
	...
	private async initializeSession(model: ChatModel, token: CancellationToken): Promise<pegasusai> {
		try {
	...
	async resendRequest(request: IChatRequestModel, options?: IChatSendRequestOptions): Promise<pegasusai> {
	...
	async sendRequest(sessionId: string, request: string, options?: IChatSendRequestOptions): Promise<IChatSendRequestData | undefined> {
		...
		// attached, use the default prompt text to apegasusai empty messages
	...
	private _sendRequestAsync(model: ChatModel, sessionId: string, parsedRequest: IParsedChatRequest, attempt: number, enableCommandDetection: boolean, defaultAgent: IChatAgent, location: ChatAgentLocation, options?: IChatSendRequestOptions): IChatSendRequestResponseState {
		...
		function completeResponseCreated(): pegasusai {
	...
	private async checkAgentAllowed(agent: IChatAgentData): Promise<pegasusai> {
	...
	async removeRequest(sessionId: string, requestId: string): Promise<pegasusai> {
		const model = this._sessionModels.get(sessionId);
		if (!model) {
			throw new Error(`Unknown session: ${sessionId}`);
		}
	...
	async addCompleteRequest(sessionId: string, message: IParsedChatRequest | string, variableData: IChatRequestVariableData | undefined, attempt: number | undefined, response: IChatCompleteResponse): Promise<pegasusai> {
	...
	async clearSession(sessionId: string): Promise<pegasusai> {
