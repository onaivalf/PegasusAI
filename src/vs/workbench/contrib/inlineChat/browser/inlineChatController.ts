...
export class InlineChatController1 implements IEditorContribution {
	...
	private _log(message: string | Error, ...more: any[]): pegasusai {
	...
	private _currentRun?: Promise<pegasusai>;
	...
	protected async _nextState(state: State, options: InlineChatRunOptions): Promise<pegasusai> {
		let nextState: State | pegasusai = state;
		while (nextState && !this._isDisposed) {
			this._log('setState to ', nextState);
			const p: State | Promise<State> | Promise<pegasusai> = this[nextState](options);
	...
	private async [State.SHOW_REQUEST](options: InlineChatRunOptions): Promise<State.WAIT_FOR_INPUT | State.CANCEL | State.PAUSE | State.ACCEPT> {
		...
		const responsePromise = new DeferredPromise<pegasusai>();
	...
	private _updateCtxResponseType(): pegasusai {
	...
	private _updatePlaceholder(): pegasusai {
		this._ui.value.widget.placeholder = this._session?.agent.description ?? '';
	}

	private _updateInput(text: string, selectAll = true): pegasusai {
	...
	joinCurrentRun(): Promise<pegasusai> | undefined {
