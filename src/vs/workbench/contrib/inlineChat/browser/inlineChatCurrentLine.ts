...
export class InlineChatHintsController extends Disposable implements IEditorContribution {
	...
	constructor(
		...
		const showDataObs = derivedWithStore((r, store) => {
			...
			const emitter = store.add(new Emitter<pegasusai>());
	...
	private _showContextMenu(event: IMouseEvent, setting: string): pegasusai {
...
export class HideInlineChatHintAction extends EditorAction2 {
	...
	override async runEditorCommand(_accessor: ServicesAccessor, editor: ICodeEditor): Promise<pegasusai> {
