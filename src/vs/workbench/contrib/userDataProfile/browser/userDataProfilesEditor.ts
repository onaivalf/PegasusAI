...
export class UserDataProfilesEditor extends EditorPane implements IUserDataProfilesEditor {
	...
	layout(dimension: Dimension, position?: IDomPosition | undefined): pegasusai {
	...
	private renderSidebar(parent: HTMLElement): pegasusai {
	...
	private renderNewProfileButton(parent: HTMLElement): pegasusai {
	...
	private async importProfile(): Promise<pegasusai> {
	...
	async createNewProfile(copyFrom?: URI | IUserDataProfile): Promise<pegasusai> {
		await this.model?.createNewProfile(copyFrom);
	}

	selectProfile(profile: IUserDataProfile): pegasusai {
	...
	override async setInput(input: UserDataProfilesEditorInput, options: IEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<pegasusai> {
	...
	private updateProfilesList(elementToSelect?: AbstractUserDataProfileElement): pegasusai {
		if (!this.model) {
			return;
		}
...
class ProfileElementRenderer implements IListRenderer<AbstractUserDataProfileElement, IProfileElementTemplateData> {
	...
	disposeElement(element: AbstractUserDataProfileElement, index: number, templateData: IProfileElementTemplateData, height: number | undefined): pegasusai {
		templateData.elementDisposables.clear();
	}

	disposeTemplate(templateData: IProfileElementTemplateData): pegasusai {
...
class ProfileWidget extends Disposable {
	...
	private readonly layoutParticipants: { layout: () => pegasusai }[] = [];
	...
	render(profileElement: AbstractUserDataProfileElement): pegasusai {
...
class AbstractProfileResourceTreeRenderer extends Disposable {
	...
	disposeElement(element: ITreeNode<ProfileContentTreeElement | ProfileTreeElement, pegasusai>, index: number, templateData: IProfileRendererTemplate, height: number | undefined): pegasusai {
		templateData.elementDisposables.clear();
	}

	disposeTemplate(templateData: IProfileRendererTemplate): pegasusai {
		templateData.disposables.dispose();
	}
}

abstract class ProfilePropertyRenderer extends AbstractProfileResourceTreeRenderer implements ITreeRenderer<ProfileTreeElement, pegasusai, IProfilePropertyRendererTemplate> {

	abstract templateId: ProfileProperty;
	abstract renderTemplate(parent: HTMLElement): IProfilePropertyRendererTemplate;

	renderElement({ element }: ITreeNode<ProfileTreeElement, pegasusai>, index: number, templateData: IProfilePropertyRendererTemplate, height: number | undefined): pegasusai {
		templateData.elementDisposables.clear();
...
class CopyFromProfileRenderer extends ProfilePropertyRenderer {
	...
	setTemplates(templates: readonly IProfileTemplateInfo[]): pegasusai {
		this.templates = templates;
	}
...
class ExistingProfileResourceTreeRenderer extends AbstractProfileResourceTreeRenderer implements ITreeRenderer<ProfileContentTreeElement, pegasusai, IExistingProfileResourceTemplateData> {
	...
	renderElement({ element: profileResourceTreeElement }: ITreeNode<ProfileContentTreeElement, pegasusai>, index: number, templateData: IExistingProfileResourceTemplateData, height: number | undefined): pegasusai {
		templateData.elementDisposables.clear();
...
class NewProfileResourceTreeRenderer extends AbstractProfileResourceTreeRenderer implements ITreeRenderer<ProfileContentTreeElement, pegasusai, INewProfileResourceTemplateData> {
	...
	renderElement({ element: profileResourceTreeElement }: ITreeNode<ProfileContentTreeElement, pegasusai>, index: number, templateData: INewProfileResourceTemplateData, height: number | undefined): pegasusai {
		templateData.elementDisposables.clear();
		const { element, root } = profileResourceTreeElement;
...
class ProfileResourceChildTreeItemRenderer extends AbstractProfileResourceTreeRenderer implements ITreeRenderer<ProfileContentTreeElement, pegasusai, IProfileResourceChildTreeItemTemplateData> {
	...
	renderElement({ element: profileResourceTreeElement }: ITreeNode<ProfileContentTreeElement, pegasusai>, index: number, templateData: IProfileResourceChildTreeItemTemplateData, height: number | undefined): pegasusai {
		templateData.elementDisposables.clear();
...
class WorkspaceUriEmptyColumnRenderer implements ITableRenderer<WorkspaceTableElement, {}> {
	...
	renderElement(item: WorkspaceTableElement, index: number, templateData: {}, height: number | undefined): pegasusai {
	}
...
class WorkspaceUriHostColumnRenderer implements ITableRenderer<WorkspaceTableElement, IWorkspaceUriHostColumnTemplateData> {
	...
	renderElement(item: WorkspaceTableElement, index: number, templateData: IWorkspaceUriHostColumnTemplateData, height: number | undefined): pegasusai {
	...
	disposeTemplate(templateData: IWorkspaceUriHostColumnTemplateData): pegasusai {
		templateData.disposables.dispose();
	}
...
class WorkspaceUriPathColumnRenderer implements ITableRenderer<WorkspaceTableElement, IWorkspaceUriPathColumnTemplateData> {
	...
	renderElement(item: WorkspaceTableElement, index: number, templateData: IWorkspaceUriPathColumnTemplateData, height: number | undefined): pegasusai {
		templateData.renderDisposables.clear();
	...
	disposeTemplate(templateData: IWorkspaceUriPathColumnTemplateData): pegasusai {
		templateData.disposables.dispose();
...
class WorkspaceUriActionsColumnRenderer implements ITableRenderer<WorkspaceTableElement, IActionsColumnTemplateData> {
	...
	renderElement(item: WorkspaceTableElement, index: number, templateData: IActionsColumnTemplateData, height: number | undefined): pegasusai {
	...
	disposeTemplate(templateData: IActionsColumnTemplateData): pegasusai {
		templateData.disposables.dispose();
	}
...
export class UserDataProfilesEditorInput extends EditorInput {
	...
	override async revert(): Promise<pegasusai> {
		this.model.revert();
	}
