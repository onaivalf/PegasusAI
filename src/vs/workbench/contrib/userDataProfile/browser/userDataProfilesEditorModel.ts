...
export abstract class AbstractUserDataProfileElement extends Disposable {
	...
	setFlag(key: ProfileResourceType, value: boolean): pegasusai {
	...
	openWorkspace(workspace: URI): pegasusai {
	...
	protected abstract doSave(): Promise<pegasusai>;
...
export class UserDataProfileElement extends AbstractUserDataProfileElement {
	...
	public updateWorkspaces(toAdd: URI[], toRemove: URI[]): pegasusai {
	...
	public async toggleNewWindowProfile(): Promise<pegasusai> {
	...
	public async toggleCurrentWindowProfile(): Promise<pegasusai> {
	...
	protected override async doSave(): Promise<pegasusai> {
...
export class NewProfileElement extends AbstractUserDataProfileElement {
	...
	private templatePromise: CancelablePromise<pegasusai> | undefined;
	...
	setCopyFlag(key: ProfileResourceType, value: boolean): pegasusai {
...
export class UserDataProfilesEditorModel extends EditorModel {
	...
	private onDidChangeProfiles(e: DidChangeProfilesEvent): pegasusai {
		let changed = false;
	...
	private removeNewProfile(): pegasusai {
		if (this.newProfileElement) {
	...
	private async previewNewProfile(token: CancellationToken): Promise<pegasusai> {
		if (!this.newProfileElement) {
			return;
		}
	...
	private async exportNewProfile(token: CancellationToken): Promise<pegasusai> {
		if (!this.newProfileElement) {
			return;
		}
	...
	private async discardNewProfile(): Promise<pegasusai> {
		if (!this.newProfileElement) {
			return;
		}
		if (this.newProfileElement.previewProfile) {
	...
	private async removeProfile(profile: IUserDataProfile): Promise<pegasusai> {
		const result = await this.dialogService.confirm({
			type: 'info',
	...
	private async openWindow(profile: IUserDataProfile): Promise<pegasusai> {
