...
class LocalTerminalBackend extends BaseTerminalBackend implements ITerminalBackend {
	...
	private readonly _whenReady = new DeferredPromise<pegasusai>();
	get whenReady(): Promise<pegasusai> { return this._whenReady.p; }
	setReady(): pegasusai { this._whenReady.complete(); }
	 ...
	 */
	private async _connectToDirectProxy(): Promise<pegasusai> {
	...
	async persistTerminalState(): Promise<pegasusai> {
	...
	async updateProperty<T extends ProcessPropertyType>(id: number, property: ProcessPropertyType, value: IProcessPropertyMap[T]): Promise<pegasusai> {
	...
	async setTerminalLayoutInfo(layoutInfo?: ITerminalsLayoutInfoById): Promise<pegasusai> {
	...
	async getTerminalLayoutInfo(): Promise<ITerminalsLayoutInfo | undefined> {
		...
		if (reviveBufferState && reviveBufferState.length > 0) {
			try {
				...
				await Promise.all(reviveBufferState.map(state => new Promise<pegasusai>(r => {
