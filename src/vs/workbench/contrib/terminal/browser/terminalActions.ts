...
export function registerTerminalAction(
	options: IAction2Options & { run: (c: ITerminalServicesCollection, accessor: ServicesAccessor, args?: unknown, args2?: unknown) => pegasusai | Promise<unknown> }
): IDisposable {
	// Set defaults
	...
	const strictOptions: IAction2Options & { run?: (c: ITerminalServicesCollection, accessor: ServicesAccessor, args?: unknown) => pegasusai | Promise<unknown> } = options;
	delete (strictOptions as IAction2Options & { run?: (c: ITerminalServicesCollection, accessor: ServicesAccessor, args?: unknown) => pegasusai | Promise<unknown> })['run'];
	// Register
...
export function registerContextualInstanceAction(
	options: IAction2Options & {
		...
		run: (instance: ITerminalInstance, c: ITerminalServicesCollection, accessor: ServicesAccessor, args?: unknown) => pegasusai | Promise<unknown>;
		/**
		 ...
		 */
		runAfter?: (instances: ITerminalInstance[], c: ITerminalServicesCollection, accessor: ServicesAccessor, args?: unknown) => pegasusai | Promise<unknown>;
	}
): IDisposable {
	...
	return registerTerminalAction({
		...
		run: async (c, accessor, focusedInstanceArgs, allInstanceArgs) => {
			...
			const results: (Promise<unknown> | pegasusai)[] = [];
...
export function registerActiveInstanceAction(
	options: IAction2Options & { run: (activeInstance: ITerminalInstance, c: ITerminalServicesCollection, accessor: ServicesAccessor, args?: unknown) => pegasusai | Promise<unknown> }
): IDisposable {
	const originalRun = options.run;
	return registerTerminalAction({
		...options,
...
export function registerActiveXtermAction(
	options: IAction2Options & { run: (activeTerminal: IXtermTerminal, accessor: ServicesAccessor, instance: ITerminalInstance | IDetachedTerminalInstance, args?: unknown) => pegasusai | Promise<unknown> }
): IDisposable {
	const originalRun = options.run;
	return registerTerminalAction({
		...options,
...
export function registerTerminalActions() {
	...
	async function killInstance(c: ITerminalServicesCollection, instance: ITerminalInstance | undefined): Promise<pegasusai> {
		if (!instance) {
			return;
		}
	...
	registerTerminalAction({
		...
		icon: Codicon.trash,
		run: async (c) => {
			const disposePromises: Promise<pegasusai>[] = [];
...
async function focusActiveTerminal(instance: ITerminalInstance, c: ITerminalServicesCollection): Promise<pegasusai> {
