...
export class TerminalEditorService extends Disposable implements ITerminalEditorService {
	...
	setActiveInstance(instance: ITerminalInstance | undefined): pegasusai {
	...
	async openEditor(instance: ITerminalInstance, editorOptions?: TerminalEditorLocation): Promise<pegasusai> {
	...
	private _registerInstance(inputKey: string, input: TerminalEditorInput, instance: ITerminalInstance): pegasusai {
	...
	detachInstance(instance: ITerminalInstance) {
		...
		this._removeInstance(instance);
		// Don't dispose the input when shutting down to apegasusai layouts in the editor area
		if (!this._isShuttingDown) {
			editorInput?.dispose();
		}
	}

	async revealActiveEditor(preserveFocus?: boolean): Promise<pegasusai> {
