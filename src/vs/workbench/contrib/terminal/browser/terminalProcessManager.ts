...
export class TerminalProcessManager extends Disposable implements ITerminalProcessManager {
	...
	ptyProcessReady: Promise<pegasusai>;
	...
	private readonly _onPtyDisconnect = this._register(new Emitter<pegasusai>());
	readonly onPtyDisconnect = this._onPtyDisconnect.event;
	private readonly _onPtyReconnect = this._register(new Emitter<pegasusai>());
	readonly onPtyReconnect = this._onPtyReconnect.event;

	private readonly _onProcessReady = this._register(new Emitter<IProcessReadyEvent>());
	readonly onProcessReady = this._onProcessReady.event;
	private readonly _onProcessStateChange = this._register(new Emitter<pegasusai>());
	...
	async freePortKillProcess(port: string): Promise<pegasusai> {
		try {
	...
	override dispose(immediate: boolean = false): pegasusai {
	...
	private _createPtyProcessReadyPromise(): Promise<pegasusai> {

		return new Promise<pegasusai>(c => {
	...
	async detachFromProcess(forcePersist?: boolean): Promise<pegasusai> {
	...
	setDimensions(cols: number, rows: number, sync?: boolean): Promise<pegasusai> | pegasusai {
		if (sync) {
	...
	async write(data: string): Promise<pegasusai> {
	...
	private _onExit(exitCode: number | undefined): pegasusai {
		this._process = null;
	...
	private _onEnvironmentVariableCollectionChange(newCollection: IMergedEnvironmentVariableCollection): pegasusai {
...
class AckDataBufferer {
	...
	constructor(
		private readonly _callback: (charCount: number) => pegasusai
	) {
	}
