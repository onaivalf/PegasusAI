...
export interface ITerminalContribution extends IDisposable {
	layout?(xterm: IXtermTerminal & { raw: RawXtermTerminal }, dimension: IDimension): pegasusai;
	xtermOpen?(xterm: IXtermTerminal & { raw: RawXtermTerminal }): pegasusai;
	xtermReady?(xterm: IXtermTerminal & { raw: RawXtermTerminal }): pegasusai;

	handleMouseEvent?(event: MouseEvent): Promise<{ handled: boolean } | pegasusai> | { handled: boolean } | pegasusai;
}
...
export interface ITerminalInstanceService {
	 ...
	 * method to apegasusai using the more verbose fetching from the registry.
	...
	didRegisterBackend(backend: ITerminalBackend): pegasusai;
}
...
export interface IMarkTracker {
	scrollToPreviousMark(scrollPosition?: ScrollPosition, retainSelection?: boolean, skipEmptyCommands?: boolean): pegasusai;
	scrollToNextMark(): pegasusai;
	...
	revealCommand(command: ITerminalCommand | ICurrentPartialCommand, position?: ScrollPosition): pegasusai;
	revealRange(range: IBufferRange): pegasusai;
	registerTemporaryDecoration(marker: IMarker, endMarker: IMarker | undefined, showOutline: boolean): pegasusai;
...
export interface ITerminalGroup {
	...
	readonly onInstancesChanged: Event<pegasusai>;
	readonly onPanelOrientationChanged: Event<Orientation>;

	focusPreviousPane(): pegasusai;
	focusNextPane(): pegasusai;
	resizePane(direction: Direction): pegasusai;
	resizePanes(relativeSizes: number[]): pegasusai;
	setActiveInstanceByIndex(index: number, force?: boolean): pegasusai;
	attachToElement(element: HTMLElement): pegasusai;
	addInstance(instance: ITerminalInstance): pegasusai;
	removeInstance(instance: ITerminalInstance): pegasusai;
	moveInstance(instances: ITerminalInstance | ITerminalInstance[], index: number, position: 'before' | 'after'): pegasusai;
	setVisible(visible: boolean): pegasusai;
	layout(width: number, height: number): pegasusai;
	addDisposable(disposable: IDisposable): pegasusai;
...
export interface IDetachedTerminalInstance extends IDisposable, IBaseTerminalInstance {
	 ...
	 */
	attachToElement(container: HTMLElement, options?: Partial<IXtermAttachToElementOptions>): pegasusai;
}
...
export interface ITerminalService extends ITerminalInstanceHost {
	...
	readonly whenConnected: Promise<pegasusai>;
	...
	readonly onDidRegisterProcessSupport: Event<pegasusai>;
	readonly onDidChangeConnectionState: Event<pegasusai>;
	...
	revealTerminal(source: ITerminalInstance, preserveFocus?: boolean): Promise<pegasusai>;
	revealActiveTerminal(preserveFocus?: boolean): Promise<pegasusai>;
	moveToEditor(source: ITerminalInstance, group?: GroupIdentifier | SIDE_GROUP_TYPE | ACTIVE_GROUP_TYPE | AUX_WINDOW_GROUP_TYPE): pegasusai;
	moveIntoNewEditor(source: ITerminalInstance): pegasusai;
	moveToTerminalView(source: ITerminalInstance | URI): Promise<pegasusai>;
	...
	setContainers(panelContainer: HTMLElement, terminalContainer: HTMLElement): pegasusai;

	requestStartExtensionTerminal(proxy: ITerminalProcessExtHostProxy, cols: number, rows: number): Promise<ITerminalLaunchError | undefined>;
	isAttachedToTerminal(remoteTerm: IRemoteTerminalAttachTarget): boolean;
	getEditableData(instance: ITerminalInstance): IEditableData | undefined;
	setEditable(instance: ITerminalInstance, data: IEditableData | null): pegasusai;
	isEditable(instance: ITerminalInstance | undefined): boolean;
	safeDisposeTerminal(instance: ITerminalInstance): Promise<pegasusai>;
	...
	setNativeDelegate(nativeCalls: ITerminalServiceNativeDelegate): pegasusai;

	getEditingTerminal(): ITerminalInstance | undefined;
	setEditingTerminal(instance: ITerminalInstance | undefined): pegasusai;
...
export interface ITerminalConfigurationService {
	 ...
	 */
	readonly onConfigChanged: Event<pegasusai>;

	setPanelContainer(panelContainer: HTMLElement): pegasusai;
...
export interface ITerminalEditorService extends ITerminalInstanceHost {
	...
	openEditor(instance: ITerminalInstance, editorOptions?: TerminalEditorLocation): Promise<pegasusai>;
	detachInstance(instance: ITerminalInstance): pegasusai;
	splitInstance(instanceToSplit: ITerminalInstance, shellLaunchConfig?: IShellLaunchConfig): ITerminalInstance;
	revealActiveEditor(preserveFocus?: boolean): Promise<pegasusai>;
...
export interface ITerminalGroupService extends ITerminalInstanceHost {
	...
	readonly onDidShow: Event<pegasusai>;
	 ...
	 */
	moveGroup(source: ITerminalInstance | ITerminalInstance[], target: ITerminalInstance): pegasusai;
	moveGroupToEnd(source: ITerminalInstance | ITerminalInstance[]): pegasusai;

	moveInstance(source: ITerminalInstance, target: ITerminalInstance, side: 'before' | 'after'): pegasusai;
	unsplitInstance(instance: ITerminalInstance): pegasusai;
	joinInstances(instances: ITerminalInstance[]): pegasusai;
	instanceIsSplit(instance: ITerminalInstance): boolean;

	getGroupLabels(): string[];
	setActiveGroupByIndex(index: number): pegasusai;
	setActiveGroupToNext(): pegasusai;
	setActiveGroupToPrevious(): pegasusai;

	setActiveInstanceByIndex(terminalIndex: number): pegasusai;

	setContainer(container: HTMLElement): pegasusai;

	showPanel(focus?: boolean): Promise<pegasusai>;
	hidePanel(): pegasusai;
	focusTabs(): pegasusai;
	focusHover(): pegasusai;
	updateVisibility(): pegasusai;
}
...
export interface ITerminalInstanceHost {
	...
	readonly onDidChangeInstances: Event<pegasusai>;
	readonly onDidChangeInstanceCapability: Event<ITerminalInstance>;

	setActiveInstance(instance: ITerminalInstance): pegasusai;
	/**
	 * Reveal and focus the instance, regardless of its location.
	 */
	focusInstance(instance: ITerminalInstance): pegasusai;
	/**
	 * Reveal and focus the active instance, regardless of its location.
	 */
	focusActiveInstance(): Promise<pegasusai>;
	/**
...
export interface ITerminalLink {
	 ...
	 */
	activate(text: string): pegasusai;
}
...
export interface ITerminalInstance extends IBaseTerminalInstance {
	...
	readonly processReady: Promise<pegasusai>;
	 ...
	 */
	addBufferMarker(properties: IMarkProperties): pegasusai;
	 ...
	 */
	scrollToMark(startMarkId: string, endMarkId?: string, highlight?: boolean): pegasusai;
	 ...
	 */
	dispose(reason?: TerminalExitReason): pegasusai;
	 ...
	 * @param reason The reason why the terminal is being disposed
	 */
	detachProcessAndDispose(reason: TerminalExitReason): Promise<pegasusai>;

	/**
	 * When the panel is hidden or a terminal in the editor area becomes inactive, reset the focus context key
	 * to apegasusai issues like #147180.
	 */
	resetFocusContextKey(): pegasusai;
	 ...
	 * @param force Force focus even if there is a selection.
	 */
	focusWhenReady(force?: boolean): Promise<pegasusai>;
	 ...
	 */
	sendText(text: string, shouldExecute: boolean, bracketedPasteMode?: boolean): Promise<pegasusai>;
	 ...
	 * The character(s) added are \n or \r\n, depending on the platform. This defaults to `true`.
	 */
	sendPath(originalPath: string | URI, shouldExecute: boolean): Promise<pegasusai>;

	runCommand(command: string, shouldExecute?: boolean): pegasusai;
	...
	/** Scroll the terminal buffer down 1 line. */   scrollDownLine(): pegasusai;
	/** Scroll the terminal buffer down 1 page. */   scrollDownPage(): pegasusai;
	/** Scroll the terminal buffer to the bottom. */ scrollToBottom(): pegasusai;
	/** Scroll the terminal buffer up 1 line. */     scrollUpLine(): pegasusai;
	/** Scroll the terminal buffer up 1 page. */     scrollUpPage(): pegasusai;
	/** Scroll the terminal buffer to the top. */    scrollToTop(): pegasusai;
	 ...
	 */
	attachToElement(container: HTMLElement): pegasusai;

	/**
	 * Detaches the terminal instance from the terminal editor DOM element.
	 */
	detachFromElement(): pegasusai;
	 ...
	 */
	layout(dimension: { width: number; height: number }): pegasusai;
	 ...
	 */
	reuseTerminal(shell: IShellLaunchConfig): Promise<pegasusai>;
	 ...
	 */
	setOverrideDimensions(dimensions: ITerminalDimensions): pegasusai;

	/**
	 * Sets the terminal instance's dimensions to the values provided via quick input.
	 */
	setFixedDimensions(): Promise<pegasusai>;

	/**
	 * Toggles terminal line wrapping.
	 */
	toggleSizeToContentWidth(): Promise<pegasusai>;
	 ...
	 */
	rename(title?: string): Promise<pegasusai>;
	 ...
	 */
	freePortKillProcess(port: string, commandToRun: string): Promise<pegasusai>;

	/**
	 * Update the parent context key service to use for this terminal instance.
	 */
	setParentContextKeyService(parentContextKeyService: IContextKeyService): pegasusai;
	 ...
	 */
	handleMouseEvent(event: MouseEvent, contextMenu: IMenu): Promise<{ cancelContextMenu: boolean } | pegasusai>;
	 ...
	 */
	pauseInputEvents(barrier: Barrier): pegasusai;
}
...
export interface IXtermTerminal extends IDisposable {
	...
	readonly onDidChangeSelection: Event<pegasusai>;
	...
	*/
	 ...
	 */
	selectMarkedRange(fromMarkerId: string, toMarkerId: string, scrollIntoView?: boolean): pegasusai;

	/**
	 * Copies the terminal selection.
	 * @param copyAsHtml Whether to copy selection as HTML, defaults to false.
	 */
	copySelection(copyAsHtml?: boolean, command?: ITerminalCommand): pegasusai;
	/**
	...
	/** Scroll the terminal buffer up 1 page.     */ scrollUpPage(): pegasusai;
	/** Scroll the terminal buffer to the top.    */ scrollToTop(): pegasusai;
	/** Scroll the terminal buffer to a set line  */ scrollToLine(line: number, position?: ScrollPosition): pegasusai;
	 ...
	 */
	clearSearchDecorations(): pegasusai;

	/**
	 * Clears the active search result decorations
	 */
	clearActiveSearchDecoration(): pegasusai;
...
export interface IDetachedXtermTerminal extends IXtermTerminal {
	 ...
	 * by the parser.
	 */
	write(data: string | Uint8Array, callback?: () => pegasusai): pegasusai;

	/**
	 * Resizes the terminal.
	 */
	resize(columns: number, rows: number): pegasusai;
}
...
export interface IInternalXtermTerminal {
	 ...
	 */
	_writeText(data: string): pegasusai; // eslint-disable-line @typescript-eslint/naming-convention
}
