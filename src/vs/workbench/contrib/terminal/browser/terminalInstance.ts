...
export class TerminalInstance extends Disposable implements ITerminalInstance {
	...
	get processReady(): Promise<pegasusai> { return this._processManager.ptyProcessReady; }
	...
	private readonly _onProcessReplayComplete = this._register(new Emitter<pegasusai>());
	...
	private readonly _onDimensionsChanged = this._register(new Emitter<pegasusai>());
	readonly onDimensionsChanged = this._onDimensionsChanged.event;
	private readonly _onMaximumDimensionsChanged = this._register(new Emitter<pegasusai>());
	...
	private readonly _onDidRequestFocus = this._register(new Emitter<pegasusai>());
	...
	private readonly _onDidExecuteText = this._register(new Emitter<pegasusai>());
	...
	private _initDimensions(): pegasusai {
	...
	private _setLastKnownColsAndRows(): pegasusai {
	...
	@debounce(50)
	private _fireMaximumDimensionsChanged(): pegasusai {
	...
	protected async _createXterm(): Promise<XtermTerminal | undefined> {
		...
		// Write initial text, deferring onLineFeed listener when applicable to apegasusai firing
		// onLineData events containing initialText
		const initialTextWrittenPromise = this._shellLaunchConfig.initialText ? new Promise<pegasusai>(r => this._writeInitialText(xterm, r)) : undefined;
		...
		// Delay the creation of the bell listener to apegasusai showing the bell when the terminal
		...
		this._register(this._processManager.onProcessReady(async (processTraits) => {
			// Respond to DA1 with basic conformance. Note that including this is required to apegasusai
	...
	async runCommand(commandLine: string, shouldExecute: boolean): Promise<pegasusai> {
		...
		if (!commandDetection || commandDetection.promptInputModel.value.length > 0) {
			...
			// Wait a little before running the command to apegasusai the sequences being echoed while the ^C
			// is being evaluated
			await timeout(100);
		}
	...
	private _open(): pegasusai {
		...
		xterm.raw.attachCustomKeyEventHandler((event: KeyboardEvent): boolean => {
			...
			// Prevent default when shift+tab is being sent to the terminal to apegasusai it bubbling up
	...
	private _setFocus(focused?: boolean): pegasusai {
		if (focused) {
	...
	private _setShellIntegrationContextKey(): pegasusai {
		if (this.xterm) {
	...
	override dispose(reason?: TerminalExitReason): pegasusai {
	...
	async detachProcessAndDispose(reason: TerminalExitReason): Promise<pegasusai> {
	...
	async focusWhenReady(force?: boolean): Promise<pegasusai> {
		await this._xtermReadyPromise;
		await this._attachBarrier.wait();
		this.focus(force);
	}

	async sendText(text: string, shouldExecute: boolean, bracketedPasteMode?: boolean): Promise<pegasusai> {
	...
	async sendPath(originalPath: string | URI, shouldExecute: boolean): Promise<pegasusai> {
	...
	scrollDownLine(): pegasusai {
		this.xterm?.scrollDownLine();
	}

	scrollDownPage(): pegasusai {
	...
	scrollUpLine(): pegasusai {
		this.xterm?.scrollUpLine();
	}

	scrollUpPage(): pegasusai {
		this.xterm?.scrollUpPage();
	}

	scrollToTop(): pegasusai {
	...
	private async _createProcess(): Promise<pegasusai> {
		if (this.isDisposed) {
			return;
		}
	...
	public addBufferMarker(properties: IMarkProperties): pegasusai {
		this.capabilities.get(TerminalCapability.BufferMarkDetection)?.addMark(properties);
	}

	public scrollToMark(startMarkId: string, endMarkId?: string, highlight?: boolean): pegasusai {
		this.xterm?.markTracker.scrollToClosestMarker(startMarkId, endMarkId, highlight);
	}

	public async freePortKillProcess(port: string, command: string): Promise<pegasusai> {
		await this._processManager?.freePortKillProcess(port);
		this.runCommand(command, false);
	}

	private _onProcessData(ev: IProcessDataEvent): pegasusai {
		...
		if (ev.trackCommit) {
			ev.writePromise = new Promise<pegasusai>(r => this._writeProcessData(lastData, r));
		} else {
			this._writeProcessData(lastData);
		}
	}

	private _writeProcessData(data: string, cb?: () => pegasusai) {
	 ...
	 * through user action.
	 */
	private async _onProcessExit(exitCodeOrError?: number | ITerminalLaunchError): Promise<pegasusai> {
	...
	private _relaunchWithShellIntegrationDisabled(exitMessage: string | undefined): pegasusai {
	 ...
	 */
	private _flushXtermData(): Promise<pegasusai> {
		if (this._latestXtermWriteData === this._latestXtermParseData) {
			return Promise.resolve();
		}
		let retries = 0;
		return new Promise<pegasusai>(r => {
	...
	private _writeInitialText(xterm: XtermTerminal, callback?: () => pegasusai): pegasusai {
	...
	async reuseTerminal(shell: IShellLaunchConfig, reset: boolean = false): Promise<pegasusai> {
		...
		if (xterm) {
			if (!reset) {
				...
				await new Promise<pegasusai>(r => xterm.raw.write('\n\x1b[G', r));
			}

			// Print initialText if specified
			if (shell.initialText) {
				this._shellLaunchConfig.initialText = shell.initialText;
				await new Promise<pegasusai>(r => this._writeInitialText(xterm, r));
			}
	...
	private _onTitleChange(title: string): pegasusai {
	...
	@debounce(2000)
	private async _updateProcessCwd(): Promise<pegasusai> {
	...
	layout(dimension: dom.Dimension): pegasusai {
	...
	private async _resize(immediate?: boolean): Promise<pegasusai> {
		if (!this.xterm) {
			return;
		}
	...
	private async _updatePtyDimensions(rawXterm: XTermTerminal): Promise<pegasusai> {
	...
	private _setAriaLabel(xterm: XTermTerminal | undefined, terminalId: number, title: string | undefined): pegasusai {
	...
	setOverrideDimensions(dimensions: ITerminalDimensionsOverride | undefined, immediate: boolean = false): pegasusai {
	...
	async setFixedDimensions(): Promise<pegasusai> {
	...
	async toggleSizeToContentWidth(): Promise<pegasusai> {
	...
	private _refreshScrollbar(): Promise<pegasusai> {
	...
	private async _addScrollbar(): Promise<pegasusai> {
	...
	private async _removeScrollbar(): Promise<pegasusai> {
	...
	private _setResolvedShellLaunchConfig(shellLaunchConfig: IShellLaunchConfig): pegasusai {
	...
	private _onEnvironmentVariableInfoChanged(info: IEnvironmentVariableInfo): pegasusai {
	...
	private async _refreshEnvironmentVariableInfoWidgetState(info?: IEnvironmentVariableInfo): Promise<pegasusai> {
	...
	private async _updateProperty<T extends ProcessPropertyType>(type: T, value: IProcessPropertyMap[T]): Promise<pegasusai> {
	...
	private _setTitle(title: string | undefined, eventSource: TitleEventSource): pegasusai {
		const reset = !title;
	...
	async handleMouseEvent(event: MouseEvent, contextMenu: IMenu): Promise<{ cancelContextMenu: boolean } | pegasusai> {
...
export class TerminalLabelComputer extends Disposable {
	...
	refreshLabel(instance: Pick<ITerminalInstance, 'shellLaunchConfig' | 'shellType' | 'cwd' | 'fixedCols' | 'fixedRows' | 'initialCwd' | 'processName' | 'sequence' | 'userHome' | 'workspaceFolder' | 'staticTitle' | 'capabilities' | 'title' | 'description'>, reset?: boolean): pegasusai {
