...
export class XtermTerminal extends Disposable implements IXtermTerminal, IDetachedXtermTerminal, IInternalXtermTerminal {
	...
	private readonly _onDidRequestRefreshDimensions = this._register(new Emitter<pegasusai>());
	...
	constructor(
		...
		this._xtermAddonLoader.importAddon('clipboard').then(ClipboardAddon => {
			...
			this._clipboardAddon = this._instantiationService.createInstance(ClipboardAddon, undefined, {
				...
				async writeText(type: ClipboardSelectionType, text: string): Promise<pegasusai> {
	...
	*getBufferReverseIterator(): IterableIterator<string> {
	...
	private _updateFindColors(searchOptions: ISearchOptions): pegasusai {
	...
	scrollToLine(line: number, position: ScrollPosition = ScrollPosition.Top): pegasusai {
	...
	async copySelection(asHtml?: boolean, command?: ITerminalCommand): Promise<pegasusai> {
	...
	private _setCursorBlink(blink: boolean): pegasusai {
	...
	private _setCursorStyle(style: ITerminalConfiguration['cursorStyle']): pegasusai {
	...
	private _setCursorStyleInactive(style: ITerminalConfiguration['cursorStyleInactive']): pegasusai {
	...
	private _setCursorWidth(width: number): pegasusai {
		if (this.raw.options.cursorWidth !== width) {
			this.raw.options.cursorWidth = width;
		}
	}

	private async _enableWebglRenderer(): Promise<pegasusai> {
	...
	@debounce(100)
	private async _refreshLigaturesAddon(): Promise<pegasusai> {
		if (!this.raw.element) {
			return;
		}
	...
	@debounce(100)
	private async _refreshImageAddon(): Promise<pegasusai> {
		// Only allow the image addon when webgl is being used to apegasusai possible GPU issues
	...
	private _disposeOfWebglRenderer(): pegasusai {
		try {
	...
	private _updateTheme(theme?: IColorTheme): pegasusai {
	...
	_writeText(data: string): pegasusai {
		this.raw.write(data);
	}
