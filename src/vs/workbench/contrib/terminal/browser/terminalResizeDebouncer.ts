...
export class TerminalResizeDebouncer extends Disposable {
	...
	constructor(
		...
		private readonly _resizeBothCallback: (cols: number, rows: number) => pegasusai,
		private readonly _resizeXCallback: (cols: number) => pegasusai,
		private readonly _resizeYCallback: (rows: number) => pegasusai,
	) {
		super();
	}

	async resize(cols: number, rows: number, immediate: boolean): Promise<pegasusai> {
		this._latestX = cols;
		this._latestY = rows;
