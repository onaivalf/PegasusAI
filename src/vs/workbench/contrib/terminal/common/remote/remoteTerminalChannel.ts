...
export class RemoteTerminalChannelClient implements IPtyHostController {
	...
	get onPtyHostStart(): Event<pegasusai> {
		return this._channel.listen<pegasusai>(RemoteTerminalChannelEvent.OnPtyHostStartEvent);
	}
	get onPtyHostUnresponsive(): Event<pegasusai> {
		return this._channel.listen<pegasusai>(RemoteTerminalChannelEvent.OnPtyHostUnresponsiveEvent);
	}
	get onPtyHostResponsive(): Event<pegasusai> {
		return this._channel.listen<pegasusai>(RemoteTerminalChannelEvent.OnPtyHostResponsiveEvent);
	}
	...
	sendCommandResult(reqId: number, isError: boolean, payload: any): Promise<pegasusai> {
	...
	setTerminalLayoutInfo(layout?: ITerminalsLayoutInfoById): Promise<pegasusai> {
		const workspace = this._workspaceContextService.getWorkspace();
		const args: ISetTerminalLayoutInfoArgs = {
			workspaceId: workspace.id,
			tabs: layout ? layout.tabs : []
		};
		return this._channel.call<pegasusai>(RemoteTerminalChannelRequest.SetTerminalLayoutInfo, args);
	}
