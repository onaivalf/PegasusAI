...
export class BulkEditPane extends ViewPane {
	...
	private _currentResolve?: (edit?: ResourceEdit[]) => pegasusai;
	...
	private _done(accept: boolean): pegasusai {
	...
	groupByFile(): pegasusai {
		if (!this._treeDataSource.groupByFile) {
			this.toggleGrouping();
		}
	}

	groupByType(): pegasusai {
		if (this._treeDataSource.groupByFile) {
			this.toggleGrouping();
		}
	}
	...
	private async _openElementInMultiDiffEditor(e: IOpenEvent<BulkEditElement | undefined>): Promise<pegasusai> {
	...
	private _onContextMenu(e: ITreeContextMenuEvent<any>): pegasusai {
