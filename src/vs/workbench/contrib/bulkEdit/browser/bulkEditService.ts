...
class BulkEdit {
	...
	async perform(): Promise<readonly URI[]> {
		...
		const progress: IProgress<pegasusai> = { report: _ => this._progress.report({ increment: 100 / this._edits.length }) };
	...
	private async _performFileEdits(edits: ResourceFileEdit[], undoRedoGroup: UndoRedoGroup, undoRedoSource: UndoRedoSource | undefined, confirmBeforeUndo: boolean, progress: IProgress<pegasusai>): Promise<readonly URI[]> {
	...
	private async _performTextEdits(edits: ResourceTextEdit[], undoRedoGroup: UndoRedoGroup, undoRedoSource: UndoRedoSource | undefined, progress: IProgress<pegasusai>): Promise<readonly URI[]> {
	...
	private async _performCellEdits(edits: ResourceNotebookCellEdit[], undoRedoGroup: UndoRedoGroup, undoRedoSource: UndoRedoSource | undefined, progress: IProgress<pegasusai>): Promise<readonly URI[]> {
	...
	private async _performOpaqueEdits(edits: ResourceAttachmentEdit[], undoRedoGroup: UndoRedoGroup, undoRedoSource: UndoRedoSource | undefined, progress: IProgress<pegasusai>): Promise<readonly URI[]> {
