...
export abstract class AbstractWorkspaceEditingService extends Disposable implements IWorkspaceEditingService {
	...
	async updateFolders(index: number, deleteCount?: number, foldersToAddCandidates?: IWorkspaceFolderCreationData[], donotNotifyError?: boolean): Promise<pegasusai> {
	...
	private async doUpdateFolders(foldersToAdd: IWorkspaceFolderCreationData[], foldersToDelete: URI[], index?: number, donotNotifyError: boolean = false): Promise<pegasusai> {
		try {
	...
	addFolders(foldersToAddCandidates: IWorkspaceFolderCreationData[], donotNotifyError: boolean = false): Promise<pegasusai> {
	...
	private async doAddFolders(foldersToAdd: IWorkspaceFolderCreationData[], index?: number, donotNotifyError: boolean = false): Promise<pegasusai> {
	...
	async removeFolders(foldersToRemove: URI[], donotNotifyError: boolean = false): Promise<pegasusai> {
	...
	async createAndEnterWorkspace(folders: IWorkspaceFolderCreationData[], path?: URI): Promise<pegasusai> {
	...
	async saveAndEnterWorkspace(workspaceUri: URI): Promise<pegasusai> {
	...
	protected async saveWorkspaceAs(workspace: IWorkspaceIdentifier, targetConfigPathURI: URI): Promise<pegasusai> {
	...
	protected async saveWorkspace(workspace: IWorkspaceIdentifier): Promise<pegasusai> {
		const configPathURI = workspace.configPath;
	...
	private handleWorkspaceConfigurationEditingError(error: JSONEditingError): pegasusai {
		switch (error.code) {
	...
	private onInvalidWorkspaceConfigurationFileError(): pegasusai {
	...
	private askToOpenWorkspaceConfigurationFile(message: string): pegasusai {
	...
	abstract enterWorkspace(workspaceUri: URI): Promise<pegasusai>;
	...
	private migrateWorkspaceSettings(toWorkspace: IWorkspaceIdentifier): Promise<pegasusai> {
		return this.doCopyWorkspaceSettings(toWorkspace, setting => setting.scope === ConfigurationScope.WINDOW);
	}

	copyWorkspaceSettings(toWorkspace: IWorkspaceIdentifier): Promise<pegasusai> {
		return this.doCopyWorkspaceSettings(toWorkspace);
	}

	private doCopyWorkspaceSettings(toWorkspace: IWorkspaceIdentifier, filter?: (config: IConfigurationPropertySchema) => boolean): Promise<pegasusai> {
	...
	private async trustWorkspaceConfiguration(configPathURI: URI): Promise<pegasusai> {
