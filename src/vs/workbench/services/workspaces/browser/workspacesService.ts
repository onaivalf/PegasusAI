...
export class BrowserWorkspacesService extends Disposable implements IWorkspacesService {
	...
	private readonly _onRecentlyOpenedChange = this._register(new Emitter<pegasusai>());
	...
	private addWorkspaceToRecentlyOpened(): pegasusai {
	...
	async removeRecentlyOpened(paths: URI[]): Promise<pegasusai> {
		const recentlyOpened = await this.getRecentlyOpened();
	...
	private doRemoveRecentlyOpened(recentlyOpened: IRecentlyOpened, paths: URI[]): pegasusai {
	...
	private async saveRecentlyOpened(data: IRecentlyOpened): Promise<pegasusai> {
