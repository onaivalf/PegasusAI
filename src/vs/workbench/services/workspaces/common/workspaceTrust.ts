...
export class WorkspaceTrustManagementService extends Disposable implements IWorkspaceTrustManagementService {
	...
	private readonly _workspaceResolvedPromise: Promise<pegasusai>;
	private readonly _workspaceResolvedPromiseResolve: () => pegasusai;
	private readonly _workspaceTrustInitializedPromise: Promise<pegasusai>;
	private readonly _workspaceTrustInitializedPromiseResolve: () => pegasusai;
	...
	private readonly _onDidChangeTrustedFolders = this._register(new Emitter<pegasusai>());
	...
	private initializeWorkspaceTrust(): pegasusai {
	...
	private async resolveCanonicalUris(): Promise<pegasusai> {
		// Open editors
	...
	private async saveTrustInfo(): Promise<pegasusai> {
	...
	private async updateWorkspaceTrust(trusted?: boolean): Promise<pegasusai> {
		if (!this.workspaceTrustEnablementService.isWorkspaceTrustEnabled()) {
			return;
		}
	...
	private async doSetUrisTrust(uris: URI[], trusted: boolean): Promise<pegasusai> {
		let changed = false;
...
export class WorkspaceTrustRequestService extends Disposable implements IWorkspaceTrustRequestService {
	...
	private _openFilesTrustRequestResolver?: (response: WorkspaceTrustUriResponse) => pegasusai;

	private _workspaceTrustRequestPromise?: Promise<boolean | undefined>;
	private _workspaceTrustRequestResolver?: (trusted: boolean | undefined) => pegasusai;
	...
	private resolveWorkspaceTrustRequest(trusted?: boolean): pegasusai {
...
class WorkspaceTrustTransitionManager extends Disposable {
	...
	async participate(trusted: boolean): Promise<pegasusai> {
