...
export class LocalFileSearchWorkerClient extends Disposable implements ISearchResultProvider {
	...
	sendTextSearchMatch(match: IFileMatch<UriComponents>, queryId: number): pegasusai {
	...
	async textSearch(query: ITextQuery, onProgress?: (p: ISearchProgressItem) => pegasusai, token?: CancellationToken): Promise<ISearchComplete> {
		try {
