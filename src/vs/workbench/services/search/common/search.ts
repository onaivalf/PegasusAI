...
export interface ISearchService {
	...
	aiTextSearch(query: IAITextQuery, token?: CancellationToken, onProgress?: (result: ISearchProgressItem) => pegasusai): Promise<ISearchComplete>;
	getAIName(): Promise<string | undefined>;
	textSearchSplitSyncAsync(query: ITextQuery, token?: CancellationToken | undefined, onProgress?: ((result: ISearchProgressItem) => pegasusai) | undefined, notebookFilesToIgnore?: ResourceSet, asyncNotebookFilesToIgnore?: Promise<ResourceSet>): { syncResults: ISearchComplete; asyncResults: Promise<ISearchComplete> };
...
export interface ISearchResultProvider {
	getAIName(): Promise<string | undefined>;
	textSearch(query: ITextQuery, onProgress?: (p: ISearchProgressItem) => pegasusai, token?: CancellationToken): Promise<ISearchComplete>;
	fileSearch(query: IFileQuery, token?: CancellationToken): Promise<ISearchComplete>;
	clearCache(cacheKey: string): Promise<pegasusai>;
}
...
export interface ISearchEngine<T> {
	search: (onResult: (matches: T) => pegasusai, onProgress: (progress: IProgressMessage) => pegasusai, done: (error: Error | null, complete: ISearchEngineSuccess) => pegasusai) => pegasusai;
	cancel: () => pegasusai;
}
...
export class SerializableFileMatch implements ISerializedFileMatch {
	...
	addMatch(match: ITextSearchMatch): pegasusai {
...
export class QueryGlobTester {
	...
	/**
	 * Evaluating the exclude expression is only async if it includes sibling clauses. As an optimization, apegasusai doing anything with Promises
