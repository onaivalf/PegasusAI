...
class FileSearchEngine {
	...
	search(_onResult: (match: IInternalFileMatch) => pegasusai): Promise<IInternalSearchComplete> {
	...
	private async doSearch(fqs: IFolderQuery<URI>[], onResult: (match: IInternalFileMatch) => pegasusai): Promise<IFileSearchProviderStats | null> {
	...
	private addDirectoryEntries({ pathToEntries }: IDirectoryTree, base: URI, relativeFile: string, onResult: (result: IInternalFileMatch) => pegasusai) {
	...
	private matchDirectoryTree({ rootEntries, pathToEntries }: IDirectoryTree, queryTester: QueryGlobTester, onResult: (result: IInternalFileMatch) => pegasusai) {
		const self = this;
	...
	private matchFile(onResult: (result: IInternalFileMatch) => pegasusai, candidate: IInternalFileMatch): pegasusai {
...
export class FileSearchManager {
	...
	fileSearch(config: IFileQuery, provider: FileSearchProvider2, onBatch: (matches: IFileMatch[]) => pegasusai, token: CancellationToken): Promise<ISearchCompleteStats> {
	...
	clearCache(cacheKey: string): pegasusai {
		// cancel the token
	...
	private doSearch(engine: FileSearchEngine, batchSize: number, onResultBatch: (matches: IInternalFileMatch[]) => pegasusai, token: CancellationToken): Promise<IInternalSearchComplete> {
