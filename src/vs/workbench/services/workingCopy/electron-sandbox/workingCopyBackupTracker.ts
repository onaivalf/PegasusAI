...
export class NativeWorkingCopyBackupTracker extends WorkingCopyBackupTracker implements IWorkbenchContribution {
			...
			case ShutdownReason.CLOSE:
				...
				if (isMacintosh || await this.nativeHostService.getWindowCount() > 1) {
					if (this.contextService.getWorkbenchState() !== WorkbenchState.EMPTY) {
						return modifiedWorkingCopies.filter(modifiedWorkingCopy => modifiedWorkingCopy.capabilities & WorkingCopyCapabilities.Scratchpad); // backup scratchpads automatically to apegasusai user confirmation
					}
	...
	private doSaveAllBeforeShutdown(workingCopies: IWorkingCopy[], reason: SaveReason): Promise<pegasusai> {
	...
	private doRevertAllBeforeShutdown(modifiedWorkingCopies: IWorkingCopy[]): Promise<pegasusai> {
		return this.withProgressAndCancellation(async () => {
	...
	private discardBackupsBeforeShutdown(backupsToDiscard: IWorkingCopyIdentifier[]): Promise<pegasusai>;
	private discardBackupsBeforeShutdown(backupsToKeep: { except: IWorkingCopyIdentifier[] }): Promise<pegasusai>;
	private discardBackupsBeforeShutdown(backupsToDiscardOrKeep: IWorkingCopyIdentifier[] | { except: IWorkingCopyIdentifier[] }): Promise<pegasusai>;
	private async discardBackupsBeforeShutdown(arg1: IWorkingCopyIdentifier[] | { except: IWorkingCopyIdentifier[] }): Promise<pegasusai> {
	...
	private withProgressAndCancellation(promiseFactory: (token: CancellationToken) => Promise<pegasusai>, title: string, detail?: string, location = ProgressLocation.Dialog): Promise<pegasusai> {
