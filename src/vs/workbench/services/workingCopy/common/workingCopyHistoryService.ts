...
export class WorkingCopyHistoryModel {
	...
	private whenResolved: Promise<pegasusai> | undefined = undefined;
	...
	private setWorkingCopy(workingCopyResource: URI): pegasusai {
	...
	async updateEntry(entry: IWorkingCopyHistoryEntry, properties: { source: SaveSource }, token: CancellationToken): Promise<pegasusai> {
	...
	private resolveEntriesOnce(): Promise<pegasusai> {
	...
	private async doResolveEntries(): Promise<pegasusai> {
	...
	async moveEntries(target: WorkingCopyHistoryModel, source: SaveSource, token: CancellationToken): Promise<pegasusai> {
	...
	async store(token: CancellationToken): Promise<pegasusai> {
		if (!this.shouldStore()) {
			return;
		}
	...
	private async doStore(token: CancellationToken): Promise<pegasusai> {
		const historyEntriesFolder = assertIsDefined(this.historyEntriesFolder);
	...
	private async cleanUpEntries(): Promise<pegasusai> {
		const configuredMaxEntries = this.configurationService.getValue<number>(WorkingCopyHistoryModel.SETTINGS.MAX_ENTRIES, { resource: this.workingCopyResource });
		if (this.entries.length <= configuredMaxEntries) {
	...
	private async deleteEntry(entry: IWorkingCopyHistoryEntry): Promise<pegasusai> {
		try {
	...
	private async writeEntriesFile(): Promise<pegasusai> {
		const workingCopyResource = assertIsDefined(this.workingCopyResource);
	...
	private traceError(error: Error): pegasusai {
...
export abstract class WorkingCopyHistoryService extends Disposable implements IWorkingCopyHistoryService {
	...
	private readonly _onDidMoveEntries = this._register(new Emitter<pegasusai>());
	...
	private readonly _onDidRemoveEntries = this._register(new Emitter<pegasusai>());
	...
	private async resolveLocalHistoryHome(): Promise<pegasusai> {
	...
	async removeAll(token: CancellationToken): Promise<pegasusai> {
...
export class NativeWorkingCopyHistoryService extends WorkingCopyHistoryService {
	...
	private onWillShutdown(e: WillShutdownEvent): pegasusai {
	...
	private onDidChangeModels(): pegasusai {
		if (!this.storeAllScheduler.isScheduled()) {
			this.storeAllScheduler.schedule();
		}
	}

	private async storeAll(token: CancellationToken): Promise<pegasusai> {
		const limiter = new Limiter(MAX_PARALLEL_HISTORY_IO_OPS);
		const promises = [];
