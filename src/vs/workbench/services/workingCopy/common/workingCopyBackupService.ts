...
export class WorkingCopyBackupsModel {
	...
	add(resource: URI, versionId = 0, meta?: IWorkingCopyBackupMeta): pegasusai {
	...
	update(resource: URI, meta?: IWorkingCopyBackupMeta): pegasusai {
...
export abstract class WorkingCopyBackupService extends Disposable implements IWorkingCopyBackupService {
	...
	reinitialize(backupWorkspaceHome: URI | undefined): pegasusai {
	...
	backup(identifier: IWorkingCopyIdentifier, content?: VSBufferReadableStream | VSBufferReadable, versionId?: number, meta?: IWorkingCopyBackupMeta, token?: CancellationToken): Promise<pegasusai> {
		return this.impl.backup(identifier, content, versionId, meta, token);
	}

	discardBackup(identifier: IWorkingCopyIdentifier, token?: CancellationToken): Promise<pegasusai> {
		return this.impl.discardBackup(identifier, token);
	}

	discardBackups(filter?: { except: IWorkingCopyIdentifier[] }): Promise<pegasusai> {
...
class WorkingCopyBackupServiceImpl extends Disposable implements IWorkingCopyBackupService {
	...
	initialize(backupWorkspaceResource: URI): pegasusai {
	...
	async backup(identifier: IWorkingCopyIdentifier, content?: VSBufferReadable | VSBufferReadableStream, versionId?: number, meta?: IWorkingCopyBackupMeta, token?: CancellationToken): Promise<pegasusai> {
		const model = await this.ready;
	...
	async discardBackups(filter?: { except: IWorkingCopyIdentifier[] }): Promise<pegasusai> {
		const model = await this.ready;
	...
	private async doDiscardBackup(backupResource: URI, token?: CancellationToken): Promise<pegasusai> {
		const model = await this.ready;
		if (token?.isCancellationRequested) {
			return;
		}
	...
	private async deleteIgnoreFileNotFound(backupResource: URI): Promise<pegasusai> {
		try {
...
export class InMemoryWorkingCopyBackupService extends Disposable implements IWorkingCopyBackupService {
	...
	async discardBackup(identifier: IWorkingCopyIdentifier): Promise<pegasusai> {
	...
	async joinBackups(): Promise<pegasusai> {
		return;
	}
}
