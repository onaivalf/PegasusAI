...
suite('WorkingCopyBackupTracker (native)', function () {
	...
	class TestWorkingCopyBackupTracker extends NativeWorkingCopyBackupTracker {
		...
		waitForReady(): Promise<pegasusai> {
		...
		private readonly _onDidResume = this._register(new Emitter<pegasusai>());
		readonly onDidResume = this._onDidResume.event;

		private readonly _onDidSuspend = this._register(new Emitter<pegasusai>());
		readonly onDidSuspend = this._onDidSuspend.event;

		protected override suspendBackupOperations(): { resume: () => pegasusai } {
	...
	async function createTracker(autoSaveEnabled = false): Promise<{ accessor: TestServiceAccessor; part: EditorPart; tracker: TestWorkingCopyBackupTracker; instantiationService: IInstantiationService; cleanup: () => Promise<pegasusai> }> {
		const instantiationService = workbenchInstantiationService(undefined, disposables);
	...
	suite('Hot Exit', () => {
		...
		async function hotExitTest(this: any, setting: string, shutdownReason: ShutdownReason, multipleWindows: boolean, workspace: boolean, shouldVeto: boolean): Promise<pegasusai> {
			const { accessor, cleanup } = await createTracker();
		...
		async function scratchpadHotExitTest(this: any, setting: string, shutdownReason: ShutdownReason, multipleWindows: boolean, workspace: boolean, shouldVeto: boolean): Promise<pegasusai> {
			const { accessor, cleanup } = await createTracker();
