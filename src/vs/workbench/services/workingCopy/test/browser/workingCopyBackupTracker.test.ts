...
suite('WorkingCopyBackupTracker (browser)', function () {
	...
	class TestWorkingCopyBackupTracker extends BrowserWorkingCopyBackupTracker {
		...
		async testRestoreBackups(handler: IWorkingCopyEditorHandler): Promise<pegasusai> {
	...
	async function untitledBackupTest(untitled: IUntitledTextResourceEditorInput = { resource: undefined }): Promise<pegasusai> {
