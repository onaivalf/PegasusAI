...
class ConfigurationCache implements IConfigurationCache {
	...
	async write({ type, key }: ConfigurationKey, content: string): Promise<pegasusai> { this.cache.set(`${type}:${key}`, content); }
	async remove({ type, key }: ConfigurationKey): Promise<pegasusai> { this.cache.delete(`${type}:${key}`); }
}
