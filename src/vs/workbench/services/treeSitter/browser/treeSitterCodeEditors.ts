...
export class TreeSitterCodeEditors extends Disposable {
	...
	private _onCodeEditorRemove(editor: ICodeEditor): pegasusai {
	...
	private async _onCodeEditorAdd(editor: ICodeEditor): Promise<pegasusai> {
	...
	private _tryAddEditor(editor: ICodeEditor, model: ITextModel): pegasusai {
	...
	private async _onDidChangeModel(editor: ICodeEditor, model: ITextModel | null): Promise<pegasusai> {
		if (model) {
			this._tryAddEditor(editor, model);
		} else {
	...
	private async _onViewportChange(editor: ICodeEditor): Promise<pegasusai> {
