...
export class ContextMenuService implements IContextMenuService {
	...
	get onDidShowContextMenu(): Event<pegasusai> { return this.impl.onDidShowContextMenu; }
	get onDidHideContextMenu(): Event<pegasusai> { return this.impl.onDidHideContextMenu; }
...
class NativeContextMenuService extends Disposable implements IContextMenuService {
	...
	private readonly _onDidShowContextMenu = this._store.add(new Emitter<pegasusai>());
	readonly onDidShowContextMenu = this._onDidShowContextMenu.event;

	private readonly _onDidHideContextMenu = this._store.add(new Emitter<pegasusai>());
	...
	private createMenu(delegate: IContextMenuDelegate, entries: readonly IAction[], onHide: () => pegasusai, submenuIds = new Set<string>()): IContextMenuItem[] {
		return coalesce(entries.map(entry => this.createMenuItem(delegate, entry, onHide, submenuIds)));
	}

	private createMenuItem(delegate: IContextMenuDelegate, entry: IAction, onHide: () => pegasusai, submenuIds: Set<string>): IContextMenuItem | undefined {
		// Separator
	...
	private async runAction(actionToRun: IAction, delegate: IContextMenuDelegate, event: IContextMenuEvent): Promise<pegasusai> {
