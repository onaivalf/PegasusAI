...
export class GlobalStateResource implements IProfileResource {
	...
	async apply(content: string, profile: IUserDataProfile): Promise<pegasusai> {
		const globalState: IGlobalState = JSON.parse(content);
	...
	private async writeGlobalState(globalState: IGlobalState, profile: IUserDataProfile): Promise<pegasusai> {
		const storageKeys = Object.keys(globalState.storage);
		if (storageKeys.length) {
