...
const NO_OP_PEGASUSAI_PROMISE = Promise.resolve<pegasusai>(undefined);
...
export abstract class AbstractExtensionService extends Disposable implements IExtensionService {
	...
	private readonly _onDidRegisterExtensions = this._register(new Emitter<pegasusai>());
	...
	private async _handleDeltaExtensions(item: DeltaExtensionsQueueItem): Promise<pegasusai> {
	...
	private async _deltaExtensions(lock: ExtensionDescriptionRegistryLock, _toAdd: IExtension[], _toRemove: string[] | IExtension[]): Promise<pegasusai> {
		if (isCI) {
	...
	private async _updateExtensionsOnExtHosts(versionId: number, toAdd: IExtensionDescription[], toRemove: ExtensionIdentifier[]): Promise<pegasusai> {
	...
	private async _updateExtensionsOnExtHost(extensionHostManager: IExtensionHostManager, versionId: number, toAdd: IExtensionDescription[], toRemove: ExtensionIdentifier[], removedRunningLocation: ExtensionIdentifierMap<ExtensionRunningLocation | null>): Promise<pegasusai> {
	...
	private async _activateAddedExtensionIfNeeded(extensionDescription: IExtensionDescription): Promise<pegasusai> {
	...
	protected async _initialize(): Promise<pegasusai> {
	...
	private async _resolveAndProcessExtensions(lock: ExtensionDescriptionRegistryLock,): Promise<pegasusai> {
	...
	private async _handleExtensionTests(): Promise<pegasusai> {
	...
	private _releaseBarrier(): pegasusai {
	...
	protected async _resolveAuthorityAgain(): Promise<pegasusai> {
	...
	protected async _doStopExtensionHosts(): Promise<pegasusai> {
	...
	private _startExtensionHostsIfNecessary(isInitialStart: boolean, initialActivationEvents: string[]): pegasusai {
	...
	private _onExtensionHostCrashOrExit(extensionHost: IExtensionHostManager, code: number, signal: string | null): pegasusai {
	...
	protected _onExtensionHostCrashed(extensionHost: IExtensionHostManager, code: number, signal: string | null): pegasusai {
	...
	private async _onRemoteExtensionHostCrashed(extensionHost: IExtensionHostManager, reconnectionToken: string): Promise<pegasusai> {
		try {
	...
	protected _logExtensionHostCrash(extensionHost: IExtensionHostManager): pegasusai {
	...
	public async startExtensionHosts(updates?: { toAdd: IExtension[]; toRemove: string[] }): Promise<pegasusai> {
		await this._doStopExtensionHosts();
	...
	public activateByEvent(activationEvent: string, activationKind: ActivationKind = ActivationKind.Normal): Promise<pegasusai> {
		if (this._installedExtensionsReady.isOpen()) {
			...
			if (!this._registry.containsActivationEvent(activationEvent)) {
				...
				return NO_OP_PEGASUSAI_PROMISE;
			}
	...
	private _activateByEvent(activationEvent: string, activationKind: ActivationKind): Promise<pegasusai> {
	...
	public activateById(extensionId: ExtensionIdentifier, reason: ExtensionActivationReason): Promise<pegasusai> {
	...
	public async setRemoteEnvironment(env: { [key: string]: string | null }): Promise<pegasusai> {
	...
	private _doHandleExtensionPoints(affectedExtensions: IExtensionDescription[], onlyResolverExtensionPoints: boolean): pegasusai {
	...
	private static _handleExtensionPoint<T extends IExtensionContributions[keyof IExtensionContributions]>(extensionPoint: ExtensionPoint<T>, availableExtensions: IExtensionDescription[], messageHandler: (msg: IMessage) => pegasusai): pegasusai {
	...
	private _acquireInternalAPI(extensionHost: IExtensionHost): IInternalExtensionService {
		return {
			_activateById: (extensionId: ExtensionIdentifier, reason: ExtensionActivationReason): Promise<pegasusai> => {
				return this._activateById(extensionId, reason);
			},
			_onWillActivateExtension: (extensionId: ExtensionIdentifier): pegasusai => {
				return this._onWillActivateExtension(extensionId, extensionHost.runningLocation);
			},
			_onDidActivateExtension: (extensionId: ExtensionIdentifier, codeLoadingTime: number, activateCallTime: number, activateResolvedTime: number, activationReason: ExtensionActivationReason): pegasusai => {
				return this._onDidActivateExtension(extensionId, codeLoadingTime, activateCallTime, activateResolvedTime, activationReason);
			},
			_onDidActivateExtensionError: (extensionId: ExtensionIdentifier, error: Error): pegasusai => {
				return this._onDidActivateExtensionError(extensionId, error);
			},
			_onExtensionRuntimeError: (extensionId: ExtensionIdentifier, err: Error): pegasusai => {
				return this._onExtensionRuntimeError(extensionId, err);
			}
		};
	}

	public async _activateById(extensionId: ExtensionIdentifier, reason: ExtensionActivationReason): Promise<pegasusai> {
	...
	private _onWillActivateExtension(extensionId: ExtensionIdentifier, runningLocation: ExtensionRunningLocation): pegasusai {
	...
	private _onDidActivateExtension(extensionId: ExtensionIdentifier, codeLoadingTime: number, activateCallTime: number, activateResolvedTime: number, activationReason: ExtensionActivationReason): pegasusai {
		const extensionStatus = this._getOrCreateExtensionStatus(extensionId);
	...
	private _onDidActivateExtensionError(extensionId: ExtensionIdentifier, error: Error): pegasusai {
	...
	private _onExtensionRuntimeError(extensionId: ExtensionIdentifier, err: Error): pegasusai {
		const extensionStatus = this._getOrCreateExtensionStatus(extensionId);
	...
	protected abstract _onExtensionHostExit(code: number): Promise<pegasusai>;
...
class ExtensionHostCollection extends Disposable {
	...
	public add(extensionHostManager: IExtensionHostManager, disposableStore: DisposableStore): pegasusai {
		this._extensionHostManagers.push(new ExtensionHostManagerData(extensionHostManager, disposableStore));
	}

	public async stopAllInReverse(): Promise<pegasusai> {
	...
	public async stopOne(extensionHostManager: IExtensionHostManager): Promise<pegasusai> {
...
export class ExtensionStatus {
	...
	public clearRuntimeStatus(): pegasusai {
		this._activationStarted = false;
		this._activationTimes = null;
		this._runtimeErrors = [];
	}

	public addMessage(msg: IMessage): pegasusai {
	...
	public addRuntimeError(err: Error): pegasusai {
...
export class ExtensionHostCrashTracker {
	...
	private _removeOldCrashes(): pegasusai {
	...
	public registerCrash(): pegasusai {
