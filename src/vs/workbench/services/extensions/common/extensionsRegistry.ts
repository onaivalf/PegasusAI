...
export class ExtensionMessageCollector {
	...
	private readonly _messageHandler: (msg: IMessage) => pegasusai;
	private readonly _extension: IExtensionDescription;
	private readonly _extensionPointId: string;

	constructor(
		messageHandler: (msg: IMessage) => pegasusai,
	...
	private _msg(type: Severity, message: string): pegasusai {
...
export type IExtensionPointHandler<T> = (extensions: readonly IExtensionPointUser<T>[], delta: ExtensionPointUserDelta<T>) => pegasusai;
...
export class ExtensionPoint<T> implements IExtensionPoint<T> {
	...
	acceptUsers(users: IExtensionPointUser<T>[]): pegasusai {
		this._delta = ExtensionPointUserDelta.compute(this._users, users);
		this._users = users;
		this._handle();
	}

	private _handle(): pegasusai {
