...
export class NativeLocalProcessExtensionHost implements IExtensionHost {
	...
	private readonly _onDidSetInspectPort = new Emitter<pegasusai>();
	...
	private _performHandshake(protocol: IMessagePassingProtocol): Promise<pegasusai> {
	...
	private _onExtHostProcessExit(code: number, signal: string): pegasusai {
		if (this._terminating) {
	...
	private _onWillShutdown(event: WillShutdownEvent): pegasusai {
