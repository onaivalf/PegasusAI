...
export class UserDataSyncWorkbenchService extends Disposable implements IUserDataSyncWorkbenchService {
	...
	private readonly _onDidTurnOnSync = this._register(new Emitter<pegasusai>());
	...
	private async waitAndInitialize(): Promise<pegasusai> {
		try {
			/* wait */
	...
	private async update(reason: string): Promise<pegasusai> {
	...
	private async updateCurrentAccount(): Promise<pegasusai> {
	...
	private async updateToken(current: UserDataSyncAccount | undefined): Promise<pegasusai> {
	...
	private traceOrInfo(msg: string, ...args: any[]): pegasusai {
	...
	private updateAccountStatus(accountStatus: AccountStatus): pegasusai {
	...
	async turnOn(): Promise<pegasusai> {
	...
	async turnoff(everywhere: boolean): Promise<pegasusai> {
		if (this.userDataSyncEnablementService.isEnabled()) {
	...
	async synchroniseUserDataSyncStoreType(): Promise<pegasusai> {
	...
	syncNow(): Promise<pegasusai> {
		return this.userDataAutoSyncService.triggerSync(['Sync Now'], { immediately: true, disableCache: true });
	}

	private async doTurnOnSync(token: CancellationToken): Promise<pegasusai> {
	...
	private async handleConflictsWhileTurningOn(token: CancellationToken): Promise<pegasusai> {
	...
	private async replace(local: boolean): Promise<pegasusai> {
	...
	async accept(resource: IUserDataSyncResource, conflictResource: URI, content: string | null | undefined, apply: boolean | { force: boolean }): Promise<pegasusai> {
		return this.userDataSyncService.accept(resource, conflictResource, content, apply);
	}

	async showConflicts(conflictToOpen?: IResourcePreview): Promise<pegasusai> {
	...
	async resetSyncedData(): Promise<pegasusai> {
		const { confirmed } = await this.dialogService.confirm({
			type: 'info',
	...
	async showSyncActivity(): Promise<pegasusai> {
	...
	private async waitForActiveSyncViews(): Promise<pegasusai> {
	...
	async signIn(): Promise<pegasusai> {
		const currentAuthenticationProviderId = this.currentAuthenticationProviderId;
	...
	private async doSignIn(accountOrAuthProvider: UserDataSyncAccount | IAuthenticationProvider): Promise<pegasusai> {
	...
	private async onDidAuthFailure(): Promise<pegasusai> {
		this.currentSessionId = undefined;
		await this.update('auth failure');
	}

	private onDidChangeSessions(e: AuthenticationSessionsChangeEvent): pegasusai {
	...
	private onDidChangeStorage(): pegasusai {
