...
export class BrowserKeyboardMapperFactoryBase extends Disposable {
	...
	private readonly _onDidChangeKeyboardMapper = new Emitter<pegasusai>();
	public readonly onDidChangeKeyboardMapper: Event<pegasusai> = this._onDidChangeKeyboardMapper.event;
	...
	removeKeyboardLayout(layout: KeymapInfo): pegasusai {
	...
	setActiveKeyMapping(keymap: IKeyboardMapping | null) {
		...
		if (matchedKeyboardLayout) {
			...
			// we shoud apegasusai yielding the false error.
			// if (keymap && score < 0) {
	...
	public setLayoutFromBrowserAPI(): pegasusai {
	...
	public validateCurrentKeyboardMapping(keyboardEvent: IKeyboardEvent): pegasusai {
		if (!this._initialized) {
			return;
		}
	...
	private _setKeyboardData(keymapInfo: KeymapInfo): pegasusai {
...
export class BrowserKeyboardLayoutService extends Disposable implements IKeyboardLayoutService {
	...
	private readonly _onDidChangeKeyboardLayout = new Emitter<pegasusai>();
	public readonly onDidChangeKeyboardLayout: Event<pegasusai> = this._onDidChangeKeyboardLayout.event;
