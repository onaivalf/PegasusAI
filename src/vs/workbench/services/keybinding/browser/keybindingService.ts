...
export class WorkbenchKeybindingService extends AbstractKeybindingService {
	...
	private _keybindingHoldMode: DeferredPromise<pegasusai> | null;
	...
	private _printResolvedKeybindings(output: string[], input: string, resolvedKeybindings: ResolvedKeybinding[]): pegasusai {
		const padLength = 35;
	...
	public override enableKeybindingHoldMode(commandId: string): Promise<pegasusai> | undefined {
		...
		this._keybindingHoldMode = new DeferredPromise<pegasusai>();
	...
	private _resetKeybindingHoldMode(): pegasusai {
		if (this._keybindingHoldMode) {
	...
	private _handleKeybindingsExtensionPointUser(extensionId: ExtensionIdentifier, isBuiltin: boolean, keybindings: ContributedKeyBinding | ContributedKeyBinding[], collector: ExtensionMessageCollector, result: IExtensionKeybindingRule[]): pegasusai {
	...
	private _handleKeybinding(extensionId: ExtensionIdentifier, isBuiltin: boolean, idx: number, keybindings: ContributedKeyBinding, collector: ExtensionMessageCollector, result: IExtensionKeybindingRule[]): pegasusai {
...
class UserKeybindings extends Disposable {
	...
	async initialize(): Promise<pegasusai> {
		await this.reload();
	}
