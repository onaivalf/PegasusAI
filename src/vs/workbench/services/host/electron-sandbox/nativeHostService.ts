...
class WorkbenchHostService extends Disposable implements IHostService {
	...
	private doOpenWindow(toOpen: IWindowOpenable[], options?: IOpenWindowOptions): Promise<pegasusai> {
	...
	private doOpenEmptyWindow(options?: IOpenEmptyWindowOptions): Promise<pegasusai> {
		const remoteAuthority = this.environmentService.remoteAuthority;
