...
export class BrowserHostService extends Disposable implements IHostService {
	...
	private onBeforeShutdown(e: BeforeShutdownEvent): pegasusai {
	...
	private updateShutdownReasonFromEvent(): pegasusai {
	...
	async focus(targetWindow: Window): Promise<pegasusai> {
		targetWindow.focus();
	}
	...
	private async doOpenWindow(toOpen: IWindowOpenable[], options?: IOpenWindowOptions): Promise<pegasusai> {
	...
	private withServices(fn: (accessor: ServicesAccessor) => unknown): pegasusai {
		// Host service is used in a lot of contexts and some services
		// need to be resolved dynamically to apegasusai cyclic dependencies
	...
	private async doOpenEmptyWindow(options?: IOpenEmptyWindowOptions): Promise<pegasusai> {
	...
	private async doOpen(workspace: IWorkspace, options?: { reuse?: boolean; payload?: object }): Promise<pegasusai> {

		// When we are in a temporary workspace and are asked to open a local folder
		// we swap that folder into the workspace to apegasusai a window reload. Access
	...
	async toggleFullScreen(targetWindow: Window): Promise<pegasusai> {
	...
	async moveTop(targetWindow: Window): Promise<pegasusai> {
	...
	private async handleExpectedShutdown(reason: ShutdownReason): Promise<pegasusai> {
	...
	async getScreenshot(): Promise<ArrayBufferLike | undefined> {
		...
		try {
			...
			await Promise.all([
				new Promise<pegasusai>(r => store.add(addDisposableListener(video, 'loadedmetadata', () => r()))),
				new Promise<pegasusai>(r => store.add(addDisposableListener(video, 'canplaythrough', () => r())))
			]);
