...
export class TextFileEditorModelManager extends Disposable implements ITextFileEditorModelManager {
	...
	private readonly mapResourceToPendingModelResolvers = new ResourceMap<Promise<pegasusai>>();
	...
	saveErrorHandler = (() => {
		...
		return {
			onSaveError(error: Error, model: ITextFileEditorModel): pegasusai {
	...
	private queueModelReloads(scheme: string): pegasusai {
		for (const model of this.models) {
			if (model.isDirty()) {
				continue; // never reload dirty models
			}
	...
	private queueModelReload(model: TextFileEditorModel): pegasusai {
	...
	private async reload(model: TextFileEditorModel): Promise<pegasusai> {
	...
	private async doResolve(resourceOrModel: URI | TextFileEditorModel, options?: ITextFileEditorModelResolveOrCreateOptions): Promise<TextFileEditorModel> {
		...
		let modelResolve: Promise<pegasusai>;
		...
		// Store pending resolves to apegasusai race conditions
	...
	private async doJoinPendingResolves(resource: URI): Promise<pegasusai> {
		...
		// at the same time.
		let currentModelCopyResolve: Promise<pegasusai> | undefined;
	...
	private registerModel(model: TextFileEditorModel): pegasusai {
	...
	runSaveParticipants(model: ITextFileEditorModel, context: IStoredFileWorkingCopySaveParticipantContext, progress: IProgress<IProgressStep>, token: CancellationToken): Promise<pegasusai> {
