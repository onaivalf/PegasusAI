...
export class TextFileEditorModel extends BaseTextEditorModel implements ITextFileEditorModel {
	...
	private readonly _onDidChangeDirty = this._register(new Emitter<pegasusai>());
	readonly onDidChangeDirty = this._onDidChangeDirty.event;

	private readonly _onDidSaveError = this._register(new Emitter<pegasusai>());
	readonly onDidSaveError = this._onDidSaveError.event;

	private readonly _onDidSave = this._register(new Emitter<ITextFileEditorModelSaveEvent>());
	readonly onDidSave = this._onDidSave.event;

	private readonly _onDidRevert = this._register(new Emitter<pegasusai>());
	readonly onDidRevert = this._onDidRevert.event;

	private readonly _onDidChangeEncoding = this._register(new Emitter<pegasusai>());
	readonly onDidChangeEncoding = this._onDidChangeEncoding.event;

	private readonly _onDidChangeOrphaned = this._register(new Emitter<pegasusai>());
	readonly onDidChangeOrphaned = this._onDidChangeOrphaned.event;

	private readonly _onDidChangeReadonly = this._register(new Emitter<pegasusai>());
	...
	private setOrphaned(orphaned: boolean): pegasusai {
	...
	private onDidChangeFilesAssociation(): pegasusai {
		if (!this.isResolved()) {
			return;
		}
	...
	override setLanguageId(languageId: string, source?: string): pegasusai {
	...
	async revert(options?: IRevertOptions): Promise<pegasusai> {
		if (!this.isResolved()) {
			return;
		}
	...
	override async resolve(options?: ITextFileResolveOptions): Promise<pegasusai> {
	...
	private async doResolve(options?: ITextFileResolveOptions): Promise<pegasusai> {
	...
	private async resolveFromBuffer(buffer: ITextBufferFactory, options?: ITextFileResolveOptions): Promise<pegasusai> {
	...
	private async doResolveFromBackup(backup: IResolvedWorkingCopyBackup<IBackupMetaData>, encoding: string, options?: ITextFileResolveOptions): Promise<pegasusai> {
	...
	private async resolveFromFile(options?: ITextFileResolveOptions): Promise<pegasusai> {
	...
	private resolveFromContent(content: ITextFileStreamContent, dirty: boolean, options?: ITextFileResolveOptions): pegasusai {
	...
	private doCreateTextModel(resource: URI, value: ITextBufferFactory): pegasusai {
	...
	private doUpdateTextModel(value: ITextBufferFactory): pegasusai {
	...
	protected override installModelListeners(model: ITextModel): pegasusai {
	...
	private onModelContentChanged(model: ITextModel, isUndoingOrRedoing: boolean): pegasusai {
	...
	protected override async autoDetectLanguage(): Promise<pegasusai> {
	...
	private async forceResolveFromFile(): Promise<pegasusai> {
		if (this.isDisposed()) {
	...
	setDirty(dirty: boolean): pegasusai {
		if (!this.isResolved()) {
	...
	private doSetDirty(dirty: boolean): () => pegasusai {
		const wasDirty = this.dirty;
	...
	private async doSave(options: ITextFileSaveAsOptions): Promise<pegasusai> {
	...
	private doSaveSequential(versionId: number, options: ITextFileSaveAsOptions, progress: IProgress<IProgressStep>, saveCancellation: CancellationTokenSource): Promise<pegasusai> {
	...
	private handleSaveSuccess(stat: IFileStatWithMetadata, versionId: number, options: ITextFileSaveAsOptions): pegasusai {
	...
	private handleSaveError(error: Error, versionId: number, options: ITextFileSaveAsOptions): pegasusai {
	...
	private updateSavedVersionId(): pegasusai {
	...
	private updateLastResolvedFileStat(newFileStat: IFileStatWithMetadata): pegasusai {
	...
	async joinState(state: TextFileEditorModelState.PENDING_SAVE): Promise<pegasusai> {
		return this.saveSequentializer.running;
	}
	...
	private async onMaybeShouldChangeEncoding(): Promise<pegasusai> {
	...
	private async setEncodingInternal(encoding: string, mode: EncodingMode): Promise<pegasusai> {
	...
	private trace(msg: string): pegasusai {
