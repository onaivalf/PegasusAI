...
export interface IDefaultAccountService {
	...
	setDefaultAccount(account: IDefaultAccount | null): pegasusai;
}
...
export class DefaultAccountManagementContribution extends Disposable implements IWorkbenchContribution {
	...
	private setDefaultAccount(account: IDefaultAccount | null): pegasusai {
	...
	private registerSignInAction(authProviderId: string, authProviderLabel: string, enterpriseAuthProviderId: string, enterpriseAuthProviderConfig: string, scopes: string[]): pegasusai {
		const that = this;
