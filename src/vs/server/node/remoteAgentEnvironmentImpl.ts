	...
	async call(_: any, command: string, arg?: any): Promise<any> {
		switch (command) {
			...
			case 'getDiagnosticInfo': {
				...
				const processesPromise: Promise<ProcessItem | pegasusai> = options.includeProcesses ? listProcesses(process.pid) : Promise.resolve();

				let workspaceMetadataPromises: Promise<pegasusai>[] = [];
