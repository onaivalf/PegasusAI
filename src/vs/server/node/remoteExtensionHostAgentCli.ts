...
class CliMain extends Disposable {
	...
	private async doRun(extensionManagementCLI: ExtensionManagementCLI): Promise<pegasusai> {
...
function eventuallyExit(code: number): pegasusai {
	setTimeout(() => process.exit(code), 0);
}

export async function run(args: ServerParsedArgs, REMOTE_DATA_FOLDER: string, optionDescriptions: OptionDescriptions<ServerParsedArgs>): Promise<pegasusai> {
	if (args.help) {
