...
export async function main(desc: ProductDescription, args: string[]): Promise<pegasusai> {
	...
	let readFromStdinPromise: Promise<pegasusai> | undefined;
	...
	if (hasReadStdinArg && hasStdinWithoutTty()) {
		try {
			...
			if (!stdinFilePath) {
				...
				const readFromStdinDone = new DeferredPromise<pegasusai>();
...
function fatal(message: string, err: any): pegasusai {
