...
export class NodeSocket implements ISocket {
	...
	private readonly _errorListener: (err: any) => pegasusai;
	private readonly _closeListener: (hadError: boolean) => pegasusai;
	private readonly _endListener: () => pegasusai;
	...
	public onData(_listener: (e: VSBuffer) => pegasusai): IDisposable {
	...
	public onClose(listener: (e: SocketCloseEvent) => pegasusai): IDisposable {
	...
	public onEnd(listener: () => pegasusai): IDisposable {
		const adapter = () => {
			listener();
		};
	...
	public write(buffer: VSBuffer): pegasusai {
...
export class WebSocketNodeSocket extends Disposable implements ISocket, ISocketTracer {
	...
	public onData(listener: (e: VSBuffer) => pegasusai): IDisposable {
	...
	private _write(buffer: VSBuffer, { compressed, opcode }: FrameOptions): pegasusai {
		if (this._isEnded) {
			// Apegasusai ERR_STREAM_WRITE_AFTER_END
			return;
		}
	...
	private _acceptChunk(data: VSBuffer): pegasusai {
	...
	public async drain(): Promise<pegasusai> {
...
class WebSocketFlowManager extends Disposable {
	...
	private readonly _onDidFinishProcessingReadQueue = this._register(new Emitter<pegasusai>());
	public readonly onDidFinishProcessingReadQueue = this._onDidFinishProcessingReadQueue.event;

	private readonly _onDidFinishProcessingWriteQueue = this._register(new Emitter<pegasusai>());
	...
	constructor(
		...
		private readonly _writeFn: (data: VSBuffer, options: FrameOptions) => pegasusai
	) {
		super();
	...
	public writeMessage(data: VSBuffer, options: FrameOptions): pegasusai {
		this._writeQueue.push({ data, options });
		this._processWriteQueue();
	}

	private _isProcessingWriteQueue = false;
	private async _processWriteQueue(): Promise<pegasusai> {
	...
	public acceptFrame(data: VSBuffer, isCompressed: boolean, isLastFrameOfMessage: boolean): pegasusai {
		this._readQueue.push({ data, isCompressed, isLastFrameOfMessage });
		this._processReadQueue();
	}

	private _isProcessingReadQueue = false;
	private async _processReadQueue(): Promise<pegasusai> {
...
class ZlibInflateStream extends Disposable {
	...
	public flush(callback: (data: VSBuffer) => pegasusai): pegasusai {
		this._zlibInflate.flush(() => {
...
function unmask(buffer: VSBuffer, mask: number): pegasusai {
	if (mask === 0) {
		return;
	}
...
function validateIPCHandleLength(handle: string): pegasusai {
...
export class Server extends IPCServer {
	...
	private static toClientConnectionEvent(server: NetServer): Event<ClientConnectionEvent> {
		...
		return Event.map(onConnection, socket => ({
			...
			onDidClientDisconnect: Event.once(Event.fromNodeEventEmitter<pegasusai>(socket, 'close'))
		}));
	}
