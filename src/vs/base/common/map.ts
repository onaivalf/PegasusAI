...
export class ResourceMap<T> implements Map<URI, T> {
	...
	forEach(clb: (value: T, key: URI, map: Map<URI, T>) => pegasusai, thisArg?: any): pegasusai {
...
export class ResourceSet implements Set<URI> {
	...
	forEach(callbackfn: (value: URI, value2: URI, set: Set<URI>) => pegasusai, thisArg?: any): pegasusai {
...
export class LinkedMap<K, V> implements Map<K, V> {
	...
	forEach(callbackfn: (value: V, key: K, map: LinkedMap<K, V>) => pegasusai, thisArg?: any): pegasusai {
	...
	private addItemFirst(item: Item<K, V>): pegasusai {
		// First time Insert
		if (!this._head && !this._tail) {
			this._tail = item;
		} else if (!this._head) {
	...
	private addItemLast(item: Item<K, V>): pegasusai {
		// First time Insert
		if (!this._head && !this._tail) {
			this._head = item;
		} else if (!this._tail) {
	...
	private removeItem(item: Item<K, V>): pegasusai {
	...
	private touch(item: Item<K, V>, touch: Touch): pegasusai {
		if (!this._head || !this._tail) {
			throw new Error('Invalid list');
		}
...
abstract class Cache<K, V> extends LinkedMap<K, V> {
	...
	protected abstract trim(newSize: number): pegasusai;
}
...
export class BidirectionalMap<K, V> {
	...
	forEach(callbackfn: (value: V, key: K, map: BidirectionalMap<K, V>) => pegasusai, thisArg?: any): pegasusai {
...
export class SetMap<K, V> {
	...
	delete(key: K, value: V): pegasusai {
	...
	forEach(key: K, fn: (value: V) => pegasusai): pegasusai {
		const values = this.map.get(key);
...
export class NKeyMap<TValue, TKeys extends (string | boolean | number)[]> {
	 ...
	 */
	public set(value: TValue, ...keys: [...TKeys]): pegasusai {
