...
export class Debouncer implements IDisposable {
	...
	public debounce(fn: () => pegasusai, timeoutMs: number): pegasusai {
...
export class Throttler implements IDisposable {
	...
	public throttle(fn: () => pegasusai, timeoutMs: number): pegasusai {
		if (this._timeout === undefined) {
			this._timeout = setTimeout(() => {
				this._timeout = undefined;
				fn();
...
export function deepAssign<T>(target: T, source: T): pegasusai {
...
export function deepAssignDeleteNulls<T>(target: T, source: T): pegasusai {
	for (const key in source) {
		if (source[key] === null) {
			delete target[key];
