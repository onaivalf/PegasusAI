...
export abstract class Pane extends Disposable implements IView {
	...
	style(styles: IPaneStyles): pegasusai {
		this.styles = styles;
	...
	protected updateHeader(): pegasusai {
		if (!this.header) {
			return;
		}
		const expanded = !this.headerVisible || this.isExpanded();
	...
	protected abstract renderHeader(container: HTMLElement): pegasusai;
	protected abstract renderBody(container: HTMLElement): pegasusai;
	protected abstract layoutBody(height: number, width: number): pegasusai;
}
...
class PaneDraggable extends Disposable {
	...
	private onDragStart(e: DragEvent): pegasusai {
	...
	private onDragEnter(e: DragEvent): pegasusai {
	...
	private onDragLeave(e: DragEvent): pegasusai {
		if (!this.context.draggable || this.context.draggable === this) {
			return;
		}
	...
	private onDragEnd(e: DragEvent): pegasusai {
	...
	private onDrop(e: DragEvent): pegasusai {
		if (!this.context.draggable) {
			return;
		}
...
export class PaneView extends Disposable {
	...
	addPane(pane: Pane, size: number, index = this.splitview.length): pegasusai {
	...
	removePane(pane: Pane): pegasusai {
	...
	movePane(from: Pane, to: Pane): pegasusai {
	...
	resizePane(pane: Pane, size: number): pegasusai {
		const index = this.paneItems.findIndex(item => item.pane === pane);
	...
	flipOrientation(height: number, width: number): pegasusai {
	...
	private setupAnimation(): pegasusai {
		if (typeof this.animationTimer === 'number') {
			getWindow(this.element).clearTimeout(this.animationTimer);
		}
