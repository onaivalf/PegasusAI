...
export interface IDialogOptions {
	...
	readonly keyEventProcessor?: (event: StandardKeyboardEvent) => pegasusai;
	readonly renderBody?: (container: HTMLElement) => pegasusai;
...
export class Dialog extends Disposable {
	...
	updateMessage(message: string): pegasusai {
		...
		return new Promise<IDialogResult>(resolve => {
			...
			this._register(addDisposableListener(window, 'keydown', e => {
				...
				if (evt.equals(KeyCode.Tab) || evt.equals(KeyCode.RightArrow) || evt.equals(KeyMod.Shift | KeyCode.Tab) || evt.equals(KeyCode.LeftArrow)) {
					...
					const focusableElements: { focus: () => pegasusai }[] = [];
					let focusedIndex = -1;
