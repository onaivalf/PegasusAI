...
suite('Processes', () => {
	test('buffered sending - simple data', function (done: () => pegasusai) {
	...
	(!platform.isWindows || process.env['VSCODE_PID'] ? test.skip : test)('buffered sending - lots of data (potential deadlock on win32)', function (done: () => pegasusai) { // test is only relevant for Windows and seems to crash randomly on some Linux builds
