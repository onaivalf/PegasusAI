...
export interface INewFindReplaceState<T extends { update: (value: T) => pegasusai } = { update: () => {} }> {
	searchString?: string;
...
export class FindReplaceState<T extends { update: (value: T) => pegasusai } = { update: () => {} }> extends Disposable {
	...
	public changeMatchInfo(matchesPosition: number, matchesCount: number, currentMatch: Range | undefined): pegasusai {
	...
	public change(newState: INewFindReplaceState<T>, moveCursor: boolean, updateHistory: boolean = true): pegasusai {
		const changeEvent: FindReplaceStateChangedEvent = {
