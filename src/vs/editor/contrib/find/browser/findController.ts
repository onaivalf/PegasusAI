...
export class CommonFindController extends Disposable implements IEditorContribution {
	...
	protected _updateHistoryDelayer: Delayer<pegasusai>;
	...
	constructor(
		...
		this._updateHistoryDelayer = new Delayer<pegasusai>(500);
	...
	private disposeModel(): pegasusai {
		if (this._model) {
			this._model.dispose();
			this._model = null;
		}
	...
	private _onStateChanged(e: FindReplaceStateChangedEvent): pegasusai {
	...
	public closeFindWidget(): pegasusai {
		this._state.change({
			isRevealed: false,
			searchScope: null
		}, false);
	...
	public toggleCaseSensitive(): pegasusai {
	...
	public toggleWholeWords(): pegasusai {
	...
	public togglePreserveCase(): pegasusai {
	...
	public toggleSearchScope(): pegasusai {
	...
	public setSearchString(searchString: string): pegasusai {
	...
	public highlightFindOptions(ignoreWhenVisible: boolean = false): pegasusai {
		// overwritten in subclass
	}

	protected async _start(opts: IFindStartOptions, newState?: INewFindReplaceState): Promise<pegasusai> {
		this.disposeModel();
	...
	public start(opts: IFindStartOptions, newState?: INewFindReplaceState): Promise<pegasusai> {
	...
	public setGlobalBufferTerm(text: string): pegasusai {
		if (this._editor.getOption(EditorOption.find).globalFindClipboard
			&& this._editor.hasModel()
			&& !this._editor.getModel().isTooLargeForSyncing()
		) {
...
export class FindController extends CommonFindController implements IFindController {
	...
	protected override async _start(opts: IFindStartOptions, newState?: INewFindReplaceState): Promise<pegasusai> {
		if (!this._widget) {
	...
	public override highlightFindOptions(ignoreWhenVisible: boolean = false): pegasusai {
		if (!this._widget) {
			this._createFindWidget();
		}
...
StartFindAction.addImplementation(0, (accessor: ServicesAccessor, editor: ICodeEditor, args: any): boolean | Promise<pegasusai> => {
...
export class StartFindWithArgsAction extends EditorAction {
	...
	public async run(accessor: ServicesAccessor | null, editor: ICodeEditor, args?: IFindStartArguments): Promise<pegasusai> {
		const controller = CommonFindController.get(editor);
		if (controller) {
...
export class StartFindWithSelectionAction extends EditorAction {
	...
	public async run(accessor: ServicesAccessor | null, editor: ICodeEditor): Promise<pegasusai> {
		const controller = CommonFindController.get(editor);
		if (controller) {
			await controller.start({
				forceRevealReplace: false,
...
export class MoveToMatchFindAction extends EditorAction {
	...
	private clearDecorations(editor: ICodeEditor): pegasusai {
		editor.changeDecorations(changeAccessor => {
			this._highlightDecorations = changeAccessor.deltaDecorations(this._highlightDecorations, []);
		});
	}

	private addDecorations(editor: ICodeEditor, range: IRange): pegasusai {
		editor.changeDecorations(changeAccessor => {
			this._highlightDecorations = changeAccessor.deltaDecorations(this._highlightDecorations, [
				{
					range,
...
StartFindReplaceAction.addImplementation(0, (accessor: ServicesAccessor, editor: ICodeEditor, args: any): boolean | Promise<pegasusai> => {
