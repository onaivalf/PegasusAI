...
export class AdditionalLinesWidget extends Disposable {
	...
	private updateLines(lineNumber: number, additionalLines: LineData[], minReservedLineCount: number): pegasusai {
	...
	private addViewZone(changeAccessor: IViewZoneChangeAccessor, afterLineNumber: number, heightInLines: number, domNode: HTMLElement): pegasusai {
		const id = changeAccessor.addZone({
			...
			onComputedHeight: (height: number) => {
				this._viewZoneHeight.set(height, undefined); // TODO: can a transaction be used to apegasusai flickering?
			}
		});
	...
	private removeActiveViewZone(changeAccessor: IViewZoneChangeAccessor): pegasusai {
	...
	private keepCursorStable(lineNumber: number, heightInLines: number): pegasusai {
...
function renderLines(domNode: HTMLElement, tabSize: number, lines: LineData[], opts: IComputedEditorOptions, isClickable: boolean): pegasusai {
	...
	// To apegasusai visual confusion, we don't want to render visible whitespace
