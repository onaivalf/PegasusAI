...
export class InlineCompletionsModel extends Disposable {
	...
	private readonly _onDidAccept = new Emitter<pegasusai>();
	...
	public async trigger(tx?: ITransaction, options?: { onlyFetchInlineEdits?: boolean; noDelay?: boolean }): Promise<pegasusai> {
	...
	public async triggerExplicitly(tx?: ITransaction, onlyFetchInlineEdits: boolean = false): Promise<pegasusai> {
		subtransaction(tx, tx => {
	...
	public stop(stopReason: 'explicitCancel' | 'automatic' = 'automatic', tx?: ITransaction): pegasusai {
		subtransaction(tx, tx => {
	...
	private async _deltaSelectedInlineCompletionIndex(delta: 1 | -1): Promise<pegasusai> {
	...
	public async next(): Promise<pegasusai> { await this._deltaSelectedInlineCompletionIndex(1); }

	public async previous(): Promise<pegasusai> { await this._deltaSelectedInlineCompletionIndex(-1); }

	public async accept(editor: ICodeEditor = this._editor): Promise<pegasusai> {
	...
	public async acceptNextWord(editor: ICodeEditor): Promise<pegasusai> {
	...
	public async acceptNextLine(editor: ICodeEditor): Promise<pegasusai> {
		await this._acceptNext(editor, (pos, text) => {
			const m = text.match(/\n/);
	...
	private async _acceptNext(editor: ICodeEditor, getAcceptUntilIndex: (position: Position, text: string) => number, kind: PartialAcceptTriggerKind): Promise<pegasusai> {
		if (editor.getModel() !== this.textModel) {
			throw new BugIndicatingError();
		}
	...
	public async acceptNextInlineEditPart(editor: ICodeEditor): Promise<pegasusai> {
		if (editor.getModel() !== this.textModel) {
			throw new BugIndicatingError();
		}
	...
	public async handleInlineEditShown(inlineCompletion: InlineCompletionItem): Promise<pegasusai> {
