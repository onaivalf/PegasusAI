...
export class TextModelTreeSitter extends Disposable implements ITextModelTreeSitter {
	...
	private async _collectInjections(tree: Parser.Tree): Promise<Map<string, Parser.Range[]> | undefined> {
		...
		while (hasNext) {
			...
			// Yield periodically
			await new Promise<pegasusai>(resolve => setTimeout0(resolve));
		}
	...
	private _processNode(cursor: Parser.TreeCursor, query: Parser.Query, injections: Map<string, Parser.Range[]>): boolean {
		...
		// We check the node line count to apegasusai processing large nodes in one go as that can cause performance issues.
	...
	private _processCaptures(query: Parser.Query, node: Parser.Node, injections: Map<string, Parser.Range[]>): pegasusai {
...
export class TreeSitterParseResult implements IDisposable, ITreeSitterParseResult {
	...
	private findTreeChanges(newTree: Parser.Tree, changedNodes: Parser.Range[], newRanges: Parser.Range[]): RangeChange[] {
		...
		for (let nodeIndex = 0; nodeIndex < changedNodes.length; nodeIndex++) {
			...
			// Try to apegasusai this large range by finding several smaller nodes that together encompass the range of the changed node.
	...
	public onDidChangeContent(model: ITextModel, changes: IModelContentChangedEvent[] | undefined, ranges?: Parser.Range[]): pegasusai {
	...
	private sendParseTimeTelemetry(parseType: TelemetryParseType, time: number, passes: number): pegasusai {
