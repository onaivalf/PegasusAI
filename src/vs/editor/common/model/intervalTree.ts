...
const enum Constants {
	...
	/**
	 * Due to how deletion works (in order to apegasusai always walking the right subtree of the deleted node),
	 ...
	 *
	 * The integer overflow occurs when 53 bits are used in the numbers, but we will try to apegasusai it as
...
function setNodeColor(node: IntervalNode, color: NodeColor): pegasusai {
	node.metadata = (
...
function setNodeIsVisited(node: IntervalNode, value: boolean): pegasusai {
	node.metadata = (
...
function setNodeIsForValidation(node: IntervalNode, value: boolean): pegasusai {
	node.metadata = (
...
function setNodeIsInGlyphMargin(node: IntervalNode, value: boolean): pegasusai {
	node.metadata = (
...
function _setNodeStickiness(node: IntervalNode, stickiness: TrackedRangeStickiness): pegasusai {
	node.metadata = (
...
function setCollapseOnReplaceEdit(node: IntervalNode, value: boolean): pegasusai {
	node.metadata = (
		(node.metadata & Constants.CollapseOnReplaceEditMaskInverse) | ((value ? 1 : 0) << Constants.CollapseOnReplaceEditOffset)
	);
}
export function setNodeStickiness(node: IntervalNode, stickiness: ActualTrackedRangeStickiness): pegasusai {
...
export class IntervalNode {
	...
	public reset(versionId: number, start: number, end: number, range: Range): pegasusai {
		this.start = start;
		this.end = end;
		this.maxEnd = end;
	...
	public setCachedOffsets(absoluteStart: number, absoluteEnd: number, cachedVersionId: number): pegasusai {
...
export class IntervalTree {
	...
	public resolveNode(node: IntervalNode, cachedVersionId: number): pegasusai {
	...
	private _normalizeDeltaIfNecessary(): pegasusai {
...
function normalizeDelta(T: IntervalTree): pegasusai {
	let node = T.root;
	let delta = 0;
	while (node !== SENTINEL) {
 ...
 */
export function nodeAcceptEdit(node: IntervalNode, start: number, end: number, textLength: number, forceMoveMarkers: boolean): pegasusai {
...
function noOverlapReplace(T: IntervalTree, start: number, end: number, textLength: number): pegasusai {
	// https://en.wikipedia.org/wiki/Interval_tree#Augmented_tree
	// Now, it is known that two intervals A and B overlap only when both
	// A.low <= B.high and A.high >= B.low. When searching the trees for
	// nodes overlapping with a given interval, you can immediately skip:
...
function treeInsert(T: IntervalTree, z: IntervalNode): pegasusai {
	let delta: number = 0;
	let x = T.root;
...
//#region Deletion
function rbTreeDelete(T: IntervalTree, z: IntervalNode): pegasusai {
...
function resetSentinel(): pegasusai {
	SENTINEL.parent = SENTINEL;
...
//#region Rotations
function leftRotate(T: IntervalTree, x: IntervalNode): pegasusai {
...
function rightRotate(T: IntervalTree, y: IntervalNode): pegasusai {
	const x = y.left;
...
export function recomputeMaxEnd(node: IntervalNode): pegasusai {
	node.maxEnd = computeMaxEnd(node);
}

function recomputeMaxEndWalkToRoot(node: IntervalNode): pegasusai {
	while (node !== SENTINEL) {
