...
export class HoverWidget extends Widget implements IHoverWidget {
	...
	get onDispose(): Event<pegasusai> { return this._onDispose.event; }
	private readonly _onRequestLayout = this._register(new Emitter<pegasusai>());
	get onRequestLayout(): Event<pegasusai> { return this._onRequestLayout.event; }
	...
	private computeXCordinate(target: TargetRect): pegasusai {
	...
	private computeYCordinate(target: TargetRect): pegasusai {
	...
	private adjustHorizontalHoverPosition(target: TargetRect): pegasusai {
	...
	private adjustVerticalHoverPosition(target: TargetRect): pegasusai {
	...
	private adjustHoverMaxHeight(target: TargetRect): pegasusai {
	...
	private setHoverPointerPosition(target: TargetRect): pegasusai {
		if (!this._hoverPointer) {
			return;
		}
...
class CompositeMouseTracker extends Widget {
	...
	private readonly _onMouseOut = this._register(new Emitter<pegasusai>());
	get onMouseOut(): Event<pegasusai> { return this._onMouseOut.event; }
	...
	private _onTargetMouseOver(): pegasusai {
		this._isMouseIn = true;
		this._mouseTimer.clear();
	}

	private _onTargetMouseLeave(): pegasusai {
	...
	private _fireIfMouseOutside(): pegasusai {
		if (!this._isMouseIn) {
