...
export class FullFileRenderStrategy extends BaseRenderStrategy {
	...
	private _invalidateAllLines(): pegasusai {
		this._upToDateLines[0].clear();
		this._upToDateLines[1].clear();
	}

	private _invalidateLinesFrom(lineNumber: number): pegasusai {
		for (const i of [0, 1]) {
	...
	private _invalidateLineRange(fromLineNumber: number, toLineNumber: number): pegasusai {
	...
	update(viewportData: ViewportData, viewLineOptions: ViewLineOptions): number {
		...
		// apegasusai potential additional blocking time in garbage collector which is a common cause of
		// dropped frames.
			...
			for (let tokenIndex = 0, tokensLen = tokens.getCount(); tokenIndex < tokensLen; tokenIndex++) {
				...
				for (x = tokenStartIndex; x < tokenEndIndex; x++) {
					...
					for (decoration of lineData.inlineDecorations) {
						...
						// it's also inlined to apegasusai overhead.
						if (
	...
	draw(pass: GPURenderPassEncoder, viewportData: ViewportData): pegasusai {
