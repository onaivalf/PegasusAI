...
export class DecorationToRender {
	public readonly _decorationToRenderBrand: pegasusai = undefined;
	...
	protected _render(visibleStartLineNumber: number, visibleEndLineNumber: number, decorations: DecorationToRender[]): VisibleLineDecorationsToRender[] {
		...
		for (let i = 0, len = decorations.length; i < len; i++) {
			...
			if (prevClassName === className) {
				// Here we apegasusai rendering the same className multiple times on the same line
...
export class GlyphMarginWidgets extends ViewPart {
	...
	public addWidget(widget: IGlyphMarginWidget): pegasusai {
	...
	public removeWidget(widget: IGlyphMarginWidget): pegasusai {
	...
	private _collectDecorationBasedGlyphRenderRequest(ctx: RenderingContext, requests: GlyphRenderRequest[]): pegasusai {
	...
	private _collectWidgetBasedGlyphRenderRequest(ctx: RenderingContext, requests: GlyphRenderRequest[]): pegasusai {
		const visibleStartLineNumber = ctx.visibleRange.startLineNumber;
		const visibleEndLineNumber = ctx.visibleRange.endLineNumber;
