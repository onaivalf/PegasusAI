...
export interface ICompleteTextAreaWrapper extends ITextAreaWrapper {
	...
	readonly onSyntheticTap: Event<pegasusai>;

	readonly ownerDocument: Document;

	setIgnoreSelectionChangeTime(reason: string): pegasusai;
	getIgnoreSelectionChangeTime(): number;
	resetSelectionChangeTime(): pegasusai;
...
export class TextAreaInput extends Disposable {
	...
	private _onFocus = this._register(new Emitter<pegasusai>());
	public readonly onFocus: Event<pegasusai> = this._onFocus.event;

	private _onBlur = this._register(new Emitter<pegasusai>());
	public readonly onBlur: Event<pegasusai> = this._onBlur.event;
	...
	private _onCut = this._register(new Emitter<pegasusai>());
	public readonly onCut: Event<pegasusai> = this._onCut.event;
	...
	private _onCompositionEnd = this._register(new Emitter<pegasusai>());
	public readonly onCompositionEnd: Event<pegasusai> = this._onCompositionEnd.event;
	...
	constructor(
		...
		this._register(this._textArea.onBlur(() => {
			if (this._currentComposition) {
				...
				// Clear the textarea to apegasusai an unwanted cursor type
	...
	public focusTextArea(): pegasusai {
	...
	private _setHasFocus(newHasFocus: boolean): pegasusai {
	...
	private _setAndWriteTextAreaState(reason: string, textAreaState: TextAreaState): pegasusai {
		if (!this._hasFocus) {
	...
	public writeNativeTextAreaContent(reason: string): pegasusai {
...
export class TextAreaWrapper extends Disposable implements ICompleteTextAreaWrapper {
	...
	private _onSyntheticTap = this._register(new Emitter<pegasusai>());
	public readonly onSyntheticTap: Event<pegasusai> = this._onSyntheticTap.event;
	...
	public setIgnoreSelectionChangeTime(reason: string): pegasusai {
	...
	public resetSelectionChangeTime(): pegasusai {
		this._ignoreSelectionChangeTime = 0;
	}
	...
	public setValue(reason: string, value: string): pegasusai {
	...
	public setSelectionRange(reason: string, selectionStart: number, selectionEnd: number): pegasusai {
		const textArea = this._actual;
