...
export abstract class AbstractUserDataSyncStoreManagementService extends Disposable implements IUserDataSyncStoreManagementService {
	...
	private readonly _onDidChangeUserDataSyncStore = this._register(new Emitter<pegasusai>());
	...
	protected updateUserDataSyncStore(): pegasusai {
	...
	abstract switch(type: UserDataSyncStoreType): Promise<pegasusai>;
...
export class UserDataSyncStoreManagementService extends AbstractUserDataSyncStoreManagementService implements IUserDataSyncStoreManagementService {
	...
	async switch(type: UserDataSyncStoreType): Promise<pegasusai> {
...
export class UserDataSyncStoreClient extends Disposable {
	...
	private _onTokenSucceed: Emitter<pegasusai> = this._register(new Emitter<pegasusai>());
	readonly onTokenSucceed: Event<pegasusai> = this._onTokenSucceed.event;

	private _donotMakeRequestsUntil: Date | undefined = undefined;
	get donotMakeRequestsUntil() { return this._donotMakeRequestsUntil; }
	private _onDidChangeDonotMakeRequestsUntil = this._register(new Emitter<pegasusai>());
	...
	protected updateUserDataSyncStoreUrl(userDataSyncStoreUrl: URI | undefined): pegasusai {
		this.userDataSyncStoreUrl = userDataSyncStoreUrl ? joinPath(userDataSyncStoreUrl, 'v1') : undefined;
	}

	private initDonotMakeRequestsUntil(): pegasusai {
	...
	private resetDonotMakeRequestsUntilPromise: CancelablePromise<pegasusai> | undefined = undefined;
	private setDonotMakeRequestsUntil(donotMakeRequestsUntil: Date | undefined): pegasusai {
	...
	async deleteCollection(collection?: string, headers: IHeaders = {}): Promise<pegasusai> {
		if (!this.userDataSyncStoreUrl) {
			throw new Error('No settings sync store url configured.');
		}
	...
	async deleteResource(resource: ServerResource, ref: string | null, collection?: string): Promise<pegasusai> {
		if (!this.userDataSyncStoreUrl) {
			throw new Error('No settings sync store url configured.');
		}
	...
	async deleteResources(): Promise<pegasusai> {
		if (!this.userDataSyncStoreUrl) {
			throw new Error('No settings sync store url configured.');
		}
	...
	private clearSession(): pegasusai {
	...
	private addSessionHeaders(headers: IHeaders): pegasusai {
