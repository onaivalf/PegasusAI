...
export class UserDataAutoSyncService extends Disposable implements IUserDataAutoSyncService {
	...
	private readonly syncTriggerDelayer: ThrottledDelayer<pegasusai>;
	...
	constructor(
		...
		super();
		this.syncTriggerDelayer = this._register(new ThrottledDelayer<pegasusai>(this.getSyncTriggerDelayTime()));
	...
	private updateAutoSync(): pegasusai {
	...
	async turnOff(everywhere: boolean, softTurnOffOnError?: boolean, donotRemoveMachine?: boolean): Promise<pegasusai> {
		try {
	...
	private updateEnablement(enabled: boolean): pegasusai {
	...
	private async onDidFinishSync(error: Error | undefined): Promise<pegasusai> {
	...
	private async disableMachineEventually(): Promise<pegasusai> {
	...
	private stopDisableMachineEventually(): pegasusai {
		this.storageService.remove(disableMachineEventuallyKey, StorageScope.APPLICATION);
	}

	private sources: string[] = [];
	async triggerSync(sources: string[], options?: SyncOptions): Promise<pegasusai> {
		if (this.autoSync.value === undefined) {
...
class AutoSync extends Disposable {
	...
	private readonly _onDidStartSync = this._register(new Emitter<pegasusai>());
	...
	private syncPromise: CancelablePromise<pegasusai> | undefined;
	...
	private waitUntilNextIntervalAndSync(): pegasusai {
	...
	sync(reason: string, disableCache: boolean): Promise<pegasusai> {
	...
	private async doSync(reason: string, disableCache: boolean, token: CancellationToken): Promise<pegasusai> {
	...
	private async createAndRunSyncTask(disableCache: boolean, token: CancellationToken): Promise<pegasusai> {
