...
suite('GlobalStateSync', () => {
	...
	test('when global state does not exist', () => runWithFakedTimers<pegasusai>({ useFakeTimers: true }, async () => {
	...
	test('when global state is created after first sync', () => runWithFakedTimers<pegasusai>({ useFakeTimers: true }, async () => {
	...
	test('first time sync - outgoing to server (no state)', () => runWithFakedTimers<pegasusai>({ useFakeTimers: true }, async () => {
		updateUserStorage('a', 'value1', testClient);
	...
	test('first time sync - incoming from server (no state)', () => runWithFakedTimers<pegasusai>({ useFakeTimers: true }, async () => {
	...
	test('first time sync when storage exists', () => runWithFakedTimers<pegasusai>({ useFakeTimers: true }, async () => {
		updateUserStorage('a', 'value1', client2);
		await client2.sync();
	...
	test('first time sync when storage exists - has conflicts', () => runWithFakedTimers<pegasusai>({ useFakeTimers: true }, async () => {
		updateUserStorage('a', 'value1', client2);
		await client2.sync();
	...
	test('sync adding a storage value', () => runWithFakedTimers<pegasusai>({ useFakeTimers: true }, async () => {
		updateUserStorage('a', 'value1', testClient);
		await testObject.sync(await testClient.getResourceManifest());
	...
	test('sync updating a storage value', () => runWithFakedTimers<pegasusai>({ useFakeTimers: true }, async () => {
		updateUserStorage('a', 'value1', testClient);
		await testObject.sync(await testClient.getResourceManifest());
	...
	test('sync removing a storage value', () => runWithFakedTimers<pegasusai>({ useFakeTimers: true }, async () => {
		updateUserStorage('a', 'value1', testClient);
		updateUserStorage('b', 'value2', testClient);
		await testObject.sync(await testClient.getResourceManifest());
	...
	test('sync profile state', () => runWithFakedTimers<pegasusai>({ useFakeTimers: true }, async () => {
	...
	async function updateLocale(client: UserDataSyncClient): Promise<pegasusai> {
		const fileService = client.instantiationService.get(IFileService);
		const environmentService = client.instantiationService.get(IEnvironmentService);
		await fileService.writeFile(environmentService.argvResource, VSBuffer.fromString(JSON.stringify({ 'locale': 'en' })));
	}

	function updateUserStorage(key: string, value: string, client: UserDataSyncClient, profile?: IUserDataProfile): pegasusai {
	...
	async function updateUserStorageForProfile(key: string, value: string, profile: IUserDataProfile, client: UserDataSyncClient): Promise<pegasusai> {
	...
	function updateMachineStorage(key: string, value: string, client: UserDataSyncClient): pegasusai {
		const storageService = client.instantiationService.get(IStorageService);
		storageService.store(key, value, StorageScope.PROFILE, StorageTarget.MACHINE);
	}

	function removeStorage(key: string, client: UserDataSyncClient): pegasusai {
		const storageService = client.instantiationService.get(IStorageService);
