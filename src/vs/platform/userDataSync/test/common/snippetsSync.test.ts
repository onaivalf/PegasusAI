...
suite('SnippetsSync', () => {
	...
	async function updateSnippet(name: string, content: string, client: UserDataSyncClient, profile?: IUserDataProfile): Promise<pegasusai> {
	...
	async function removeSnippet(name: string, client: UserDataSyncClient): Promise<pegasusai> {
		const fileService = client.instantiationService.get(IFileService);
		const userDataProfilesService = client.instantiationService.get(IUserDataProfilesService);
