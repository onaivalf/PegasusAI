...
export interface IWorkspacesService {
	...
	deleteUntitledWorkspace(workspace: IWorkspaceIdentifier): Promise<pegasusai>;
	getWorkspaceIdentifier(workspaceUri: URI): Promise<IWorkspaceIdentifier>;

	// Workspaces History
	readonly onDidChangeRecentlyOpened: Event<pegasusai>;
	addRecentlyOpened(recents: IRecent[]): Promise<pegasusai>;
	removeRecentlyOpened(workspaces: URI[]): Promise<pegasusai>;
	clearRecentlyOpened(): Promise<pegasusai>;
...
export function restoreRecentlyOpened(data: RecentlyOpenedStorageData | undefined, logService: ILogService): IRecentlyOpened {
	...
	if (data) {
		const restoreGracefully = function <T>(entries: T[], onEntry: (entry: T, index: number) => pegasusai) {
