...
export class WorkspacesMainService implements AddFirstParameterToFunctions<IWorkspacesService, Promise<unknown> /* only methods, not events */, number /* window ID */> {
	...
	deleteUntitledWorkspace(windowId: number, workspace: IWorkspaceIdentifier): Promise<pegasusai> {
	...
	addRecentlyOpened(windowId: number, recents: IRecent[]): Promise<pegasusai> {
		return this.workspacesHistoryMainService.addRecentlyOpened(recents);
	}

	removeRecentlyOpened(windowId: number, paths: URI[]): Promise<pegasusai> {
		return this.workspacesHistoryMainService.removeRecentlyOpened(paths);
	}

	clearRecentlyOpened(windowId: number): Promise<pegasusai> {
