...
export interface IWorkspacesHistoryMainService {
	...
	readonly onDidChangeRecentlyOpened: CommonEvent<pegasusai>;

	addRecentlyOpened(recents: IRecent[]): Promise<pegasusai>;
	getRecentlyOpened(): Promise<IRecentlyOpened>;
	removeRecentlyOpened(paths: URI[]): Promise<pegasusai>;
	clearRecentlyOpened(options?: { confirm?: boolean }): Promise<pegasusai>;
}
...
export class WorkspacesHistoryMainService extends Disposable implements IWorkspacesHistoryMainService {
	...
	private readonly _onDidChangeRecentlyOpened = this._register(new Emitter<pegasusai>());
	...
	async addRecentlyOpened(recentToAdd: IRecent[]): Promise<pegasusai> {
	...
	async removeRecentlyOpened(recentToRemove: URI[]): Promise<pegasusai> {
	...
	async clearRecentlyOpened(options?: { confirm?: boolean }): Promise<pegasusai> {
		if (options?.confirm) {
	...
	private async saveRecentlyOpened(recent: IRecentlyOpened): Promise<pegasusai> {
	...
	private readonly macOSRecentDocumentsUpdater = this._register(new ThrottledDelayer<pegasusai>(800));

	private async handleWindowsJumpList(): Promise<pegasusai> {
		if (!isWindows) {
	...
	private async updateWindowsJumpList(): Promise<pegasusai> {
		if (!isWindows) {
			return; // only on windows
		}
	...
	private async updateMacOSRecentDocuments(): Promise<pegasusai> {
		if (!isMacintosh) {
			return;
		}
