...
export class RemoteTunnelService extends Disposable implements IRemoteTunnelService {
	...
	private _startTunnelProcessDelayer: Delayer<pegasusai>;
	...
	async stopTunnel(): Promise<pegasusai> {
	...
	private async updateTunnelProcess(): Promise<pegasusai> {
	...
	private runCodeTunnelCommand(logLabel: string, commandArgs: string[], onOutput: (message: string, isError: boolean) => pegasusai = this.defaultOnOutput, env?: Record<string, string>): CancelablePromise<number> {
	...
	private _storeMode(mode: TunnelMode): pegasusai {
		if (mode.active) {
