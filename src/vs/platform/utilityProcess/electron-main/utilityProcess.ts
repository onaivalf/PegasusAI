...
export class UtilityProcess extends Disposable {
	...
	protected log(msg: string, severity: Severity): pegasusai {
		let logMsg: string;
	...
	private registerListeners(process: ElectronUtilityProcess, configuration: IUtilityProcessConfiguration, serviceName: string): pegasusai {
		...
		// Spawn
		this._register(Event.fromNodeEventEmitter<pegasusai>(process, 'spawn')(() => {
	...
	once(message: unknown, callback: () => pegasusai): pegasusai {
	...
	private onDidExitOrCrashOrKill(): pegasusai {
	...
	async waitForExit(maxWaitTimeMs: number): Promise<pegasusai> {
		if (!this.process) {
			return; // already killed, crashed or never started
		}
...
export class WindowUtilityProcess extends UtilityProcess {
	...
	private registerWindowListeners(window: BrowserWindow, configuration: IWindowUtilityProcessConfiguration): pegasusai {
