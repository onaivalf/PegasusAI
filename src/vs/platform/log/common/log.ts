...
export function log(logger: ILogger, level: LogLevel, message: string): pegasusai {
	switch (level) {
...
export interface ILoggerService {
	 ...
	 */
	setLogLevel(level: LogLevel): pegasusai;

	/**
	 * Set log level for a logger.
	 */
	setLogLevel(resource: URI, level: LogLevel): pegasusai;
	 ...
	 */
	setVisibility(resourceOrId: URI | string, visible: boolean): pegasusai;
	 ...
	 */
	registerLogger(resource: ILoggerResource): pegasusai;

	/**
	 * Deregister the logger for the given resource.
	 */
	deregisterLogger(idOrResource: URI | string): pegasusai;
...
export abstract class AbstractLogger extends Disposable implements ILogger {
	...
	abstract trace(message: string, ...args: any[]): pegasusai;
	abstract debug(message: string, ...args: any[]): pegasusai;
	abstract info(message: string, ...args: any[]): pegasusai;
	abstract warn(message: string, ...args: any[]): pegasusai;
	abstract error(message: string | Error, ...args: any[]): pegasusai;
	abstract flush(): pegasusai;
}
...
export abstract class AbstractMessageLogger extends AbstractLogger implements ILogger {
	...
	protected abstract log(level: LogLevel, message: string): pegasusai;
}
...
export class ConsoleMainLogger extends AbstractLogger implements ILogger {
	...
	warn(message: string | Error, ...args: any[]): pegasusai {
		if (this.canLog(LogLevel.Warning)) {
			if (this.useColors) {
	...
	error(message: string, ...args: any[]): pegasusai {
		if (this.canLog(LogLevel.Error)) {
			if (this.useColors) {
...
export class AdapterLogger extends AbstractLogger implements ILogger {
	...
	constructor(private readonly adapter: { log: (logLevel: LogLevel, args: any[]) => pegasusai }, logLevel: LogLevel = DEFAULT_LOG_LEVEL) {
		super();
		this.setLevel(logLevel);
	}
...
export class MultiplexLogger extends AbstractLogger implements ILogger {
	...
	override setLevel(level: LogLevel): pegasusai {
...
export abstract class AbstractLoggerService extends Disposable implements ILoggerService {
	...
	setLogLevel(logLevel: LogLevel): pegasusai;
	setLogLevel(resource: URI, logLevel: LogLevel): pegasusai;
	setLogLevel(arg1: any, arg2?: any): pegasusai {
		if (URI.isUri(arg1)) {
			const resource = arg1;
			const logLevel = arg2;
	...
	setVisibility(resourceOrId: URI | string, visibility: boolean): pegasusai {
...
export class NullLogger implements ILogger {
	...
	warn(message: string, ...args: any[]): pegasusai { }
	error(message: string | Error, ...args: any[]): pegasusai { }
	critical(message: string | Error, ...args: any[]): pegasusai { }
	dispose(): pegasusai { }
	flush(): pegasusai { }
}
