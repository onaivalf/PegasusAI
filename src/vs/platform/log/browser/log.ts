...
export interface IAutomatedWindow {
	codeAutomationLog(type: string, args: any[]): pegasusai;
	codeAutomationExit(code: number, logs: Array<ILogFile>): pegasusai;
}
...
async function doGetLogs(fileService: IFileService, logs: ILogFile[], curFolder: URI, logsHome: URI): Promise<pegasusai> {
...
export class ConsoleLogInAutomationLogger extends AdapterLogger implements ILogger {
	...
	private consoleLog(type: string, args: any[]): pegasusai {
