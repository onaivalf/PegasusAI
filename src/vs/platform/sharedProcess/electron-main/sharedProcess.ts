...
export class SharedProcess extends Disposable {
	...
	private readonly _onDidCrash = this._register(new Emitter<pegasusai>());
	...
	private async onWindowConnection(e: IpcMainEvent, nonce: string, responseChannel: string): Promise<pegasusai> {
	...
	private _whenReady: Promise<pegasusai> | undefined = undefined;
	whenReady(): Promise<pegasusai> {
		if (!this._whenReady) {
			this._whenReady = (async () => {
				...
				const whenReady = new DeferredPromise<pegasusai>();
	...
	private _whenIpcReady: Promise<pegasusai> | undefined = undefined;
	private get whenIpcReady() {
		if (!this._whenIpcReady) {
			this._whenIpcReady = (async () => {
				...
				const sharedProcessIpcReady = new DeferredPromise<pegasusai>();
	...
	private createUtilityProcess(): pegasusai {
