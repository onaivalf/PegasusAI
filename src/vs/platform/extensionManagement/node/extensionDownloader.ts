...
export class ExtensionsDownloader extends Disposable {
	...
	private readonly cleanUpPromise: Promise<pegasusai>;
	...
	private async downloadFile(extension: IGalleryExtension, location: URI, downloadFn: (location: URI) => Promise<pegasusai>): Promise<pegasusai> {
	...
	private async doDownload(extension: IGalleryExtension, name: string, downloadFn: () => Promise<pegasusai>, retries: number): Promise<number> {
		let attempts = 1;
		while (true) {
			try {
				await downloadFn();
	...
	protected async validate(zipPath: string, filePath: string): Promise<pegasusai> {
		try {
	...
	async delete(location: URI): Promise<pegasusai> {
		await this.cleanUpPromise;
