...
export class ExtensionsManifestCache extends Disposable {
	...
	private onDidUnInstallExtension(e: DidUninstallExtensionEvent): pegasusai {
		if (!e.error) {
			this.invalidate(e.profileLocation);
		}
	}

	async invalidate(extensionsManifestLocation: URI | undefined): Promise<pegasusai> {
	...
	private async deleteUserCacheFile(profile: IUserDataProfile): Promise<pegasusai> {
		try {
