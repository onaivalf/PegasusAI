...
export interface IUninstallExtensionTask {
	...
	waitUntilTaskIsFinished(): Promise<pegasusai>;
	cancel(): pegasusai;
}
...
export abstract class CommontExtensionManagementService extends Disposable implements IExtensionManagementService {
	...
	abstract uninstall(extension: ILocalExtension, options?: UninstallOptions): Promise<pegasusai>;
	abstract uninstallExtensions(extensions: UninstallExtensionInfo[]): Promise<pegasusai>;
	...
	abstract resetPinnedStateForAllUserExtensions(pinned: boolean): Promise<pegasusai>;
	abstract registerParticipant(pariticipant: IExtensionManagementParticipant): pegasusai;
	...
	abstract copyExtensions(fromProfileLocation: URI, toProfileLocation: URI): Promise<pegasusai>;
	abstract download(extension: IGalleryExtension, operation: InstallOperation, donotVerifySignature: boolean): Promise<URI>;
	abstract cleanUp(): Promise<pegasusai>;
...
export abstract class AbstractExtensionManagementService extends CommontExtensionManagementService implements IExtensionManagementService {
	...
	async uninstall(extension: ILocalExtension, options?: UninstallOptions): Promise<pegasusai> {
	...
	registerParticipant(participant: IExtensionManagementParticipant): pegasusai {
	...
	protected async installExtensions(extensions: InstallableExtension[]): Promise<InstallExtensionResult[]> {
		...
		const createInstallExtensionTask = (manifest: IExtensionManifest, extension: IGalleryExtension | URI, options: InstallExtensionTaskOptions, root: IInstallExtensionTask | undefined): pegasusai => {
	...
	private async getAllDepsAndPackExtensions(extensionIdentifier: IExtensionIdentifier, manifest: IExtensionManifest, installPreRelease: boolean, productVersion: IProductVersion): Promise<{ gallery: IGalleryExtension; manifest: IExtensionManifest }[]> {
		...
		const collectDependenciesAndPackExtensionsToInstall = async (extensionIdentifier: IExtensionIdentifier, manifest: IExtensionManifest): Promise<pegasusai> => {
	...
	async uninstallExtensions(extensions: UninstallExtensionInfo[]): Promise<pegasusai> {
		...
		const postUninstallExtension = (extension: ILocalExtension, uninstallOptions: UninstallExtensionTaskOptions, error?: ExtensionManagementError): pegasusai => {
			if (error) {
	...
	private checkForDependents(extensionsToUninstall: ILocalExtension[], installed: ILocalExtension[], extensionToUninstall: ILocalExtension): pegasusai {
	...
	protected abstract removeExtension(extension: ILocalExtension): Promise<pegasusai>;
}
