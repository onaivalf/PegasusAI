...
export interface IExtensionsScannerService {
	...
	updateManifestMetadata(extensionLocation: URI, metadata: ManifestMetadata): Promise<pegasusai>;
	initializeDefaultProfileExtensions(): Promise<pegasusai>;
}
...
export abstract class AbstractExtensionsScannerService extends Disposable implements IExtensionsScannerService {
	...
	async updateManifestMetadata(extensionLocation: URI, metaData: ManifestMetadata): Promise<pegasusai> {
	...
	async initializeDefaultProfileExtensions(): Promise<pegasusai> {
		try {
	...
	private initializeDefaultProfileExtensionsPromise: Promise<pegasusai> | undefined = undefined;
	private async doInitializeDefaultProfileExtensions(): Promise<pegasusai> {
...
class ExtensionsScanner extends Disposable {
	...
	private async getLocalizedMessages(extensionLocation: URI, extensionManifest: IExtensionManifest, nlsConfiguration: NlsConfiguration): Promise<LocalizedMessages | undefined> {
		...
		const reportErrors = (localized: URI | null, errors: ParseError[]): pegasusai => {
			errors.forEach((error) => {
		...
		const reportInvalidFormat = (localized: URI | null): pegasusai => {
	...
	private findMessageBundles(extensionLocation: URI, nlsConfiguration: NlsConfiguration): Promise<{ localized: URI; original: URI | null }> {
		return new Promise<{ localized: URI; original: URI | null }>((c, e) => {
			const loop = (locale: string): pegasusai => {
...
class CachedExtensionsScanner extends ExtensionsScanner {
	...
	private readonly cacheValidatorThrottler: ThrottledDelayer<pegasusai> = this._register(new ThrottledDelayer(3000));

	private readonly _onDidChangeCache = this._register(new Emitter<pegasusai>());
	readonly onDidChangeCache = this._onDidChangeCache.event;
	...
	private async writeExtensionCache(cacheFile: URI, cacheContents: IExtensionCacheData): Promise<pegasusai> {
		try {
	...
	private async validateCache(): Promise<pegasusai> {
		if (!this.input) {
