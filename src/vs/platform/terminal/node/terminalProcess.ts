...
export class TerminalProcess extends Disposable implements ITerminalChildProcess {
	...
	private _processStartupComplete: Promise<pegasusai> | undefined;
	...
	constructor(
		...
		// Delay resizes to apegasusai conpty not respecting very early resize calls
		if (isWindows) {
	...
	private async setupPtyProcess(
		...
		this._processStartupComplete = new Promise<pegasusai>(c => {
	...
	private async _kill(): Promise<pegasusai> {
	...
	private async _throttleKillSpawn(): Promise<pegasusai> {
	...
	private _sendProcessTitle(ptyProcess: IPty): pegasusai {
		if (this._store.isDisposed) {
			return;
		}
	...
	input(data: string, isBinary: boolean = false): pegasusai {
	...
	clearUnacknowledgedChars(): pegasusai {
