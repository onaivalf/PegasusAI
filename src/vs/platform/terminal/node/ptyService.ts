...
export class PtyService extends Disposable implements IPtyService {
	...
	private readonly _onHeartbeat = this._register(new Emitter<pegasusai>());
	...
	@traceRpc
	async refreshIgnoreProcessNames(names: string[]): Promise<pegasusai> {
	...
	@traceRpc
	async acceptDetachInstanceReply(requestId: number, persistentProcessId: number): Promise<pegasusai> {
	...
	private async _reviveTerminalProcess(workspaceId: string, terminal: ISerializedTerminalState): Promise<pegasusai> {
		...
		// This causes the cursor to move to the top into the replayed terminal contents. To apegasusai
	...
	@traceRpc
	async shutdownAll(): Promise<pegasusai> {
		this.dispose();
	}
	...
	@traceRpc
	async attachToProcess(id: number): Promise<pegasusai> {
		try {
	...
	@traceRpc
	async updateTitle(id: number, title: string, titleSource: TitleEventSource): Promise<pegasusai> {
		this._throwIfNoPty(id).setTitle(title, titleSource);
	}

	@traceRpc
	async updateIcon(id: number, userInitiated: boolean, icon: URI | { light: URI; dark: URI } | { id: string; color?: { id: string } }, color?: string): Promise<pegasusai> {
		this._throwIfNoPty(id).setIcon(userInitiated, icon, color);
	}

	@traceRpc
	async clearBuffer(id: number): Promise<pegasusai> {
	...
	@traceRpc
	async updateProperty<T extends ProcessPropertyType>(id: number, type: T, value: IProcessPropertyMap[T]): Promise<pegasusai> {
		return this._throwIfNoPty(id).updateProperty(type, value);
	}

	@traceRpc
	async detachFromProcess(id: number, forcePersist?: boolean): Promise<pegasusai> {
		return this._throwIfNoPty(id).detach(forcePersist);
	}

	@traceRpc
	async reduceConnectionGraceTime(): Promise<pegasusai> {
		for (const pty of this._ptys.values()) {
			pty.reduceGraceTime();
		}
	}
	...
	@traceRpc
	async shutdown(id: number, immediate: boolean): Promise<pegasusai> {
		// Don't throw if the pty is already shutdown
		return this._ptys.get(id)?.shutdown(immediate);
	}
	@traceRpc
	async input(id: number, data: string): Promise<pegasusai> {
	...
	@traceRpc
	async processBinary(id: number, data: string): Promise<pegasusai> {
		return this._throwIfNoPty(id).writeBinary(data);
	}
	@traceRpc
	async resize(id: number, cols: number, rows: number): Promise<pegasusai> {
		const pty = this._throwIfNoPty(id);
		if (pty) {
			for (const contrib of this._contributions) {
	...
	@traceRpc
	async acknowledgeDataEvent(id: number, charCount: number): Promise<pegasusai> {
		return this._throwIfNoPty(id).acknowledgeDataEvent(charCount);
	}
	@traceRpc
	async setUnicodeVersion(id: number, version: '6' | '11'): Promise<pegasusai> {
	...
	@traceRpc
	async orphanQuestionReply(id: number): Promise<pegasusai> {
	...
	@traceRpc
	async setTerminalLayoutInfo(args: ISetTerminalLayoutInfoArgs): Promise<pegasusai> {
...
class PersistentTerminalProcess extends Disposable {
	...
	private readonly _pendingCommands = new Map<number, { resolve: (data: any) => pegasusai; reject: (err: any) => pegasusai }>();
	...
	private readonly _onPersistentProcessReady = this._register(new Emitter<pegasusai>());
	...
	private readonly _onProcessOrphanQuestion = this._register(new Emitter<pegasusai>());
	...
	setTitle(title: string, titleSource: TitleEventSource): pegasusai {
	...
	setIcon(userInitiated: boolean, icon: TerminalIcon, color?: string): pegasusai {
	...
	private _setFixedDimensions(fixedDimensions?: IFixedTerminalDimensions): pegasusai {
	...
	async attach(): Promise<pegasusai> {
	...
	async detach(forcePersist?: boolean): Promise<pegasusai> {
	...
	writeBinary(data: string): Promise<pegasusai> {
	...
	async clearBuffer(): Promise<pegasusai> {
		this._serializer.clearBuffer();
		this._terminalProcess.clearBuffer();
	}
	setUnicodeVersion(version: '6' | '11'): pegasusai {
	...
	async triggerReplay(): Promise<pegasusai> {
	...
	sendCommandResult(reqId: number, isError: boolean, serializedPayload: any): pegasusai {
...
class MutationLogger<T> {
	...
	private _log(reason: string): pegasusai {
...
class XtermSerializer implements ITerminalSerializer {
	...
	freeRawReviveBuffer(): pegasusai {
		// Free the memory of the terminal if it will need to be re-serialized
		this._rawReviveBuffer = undefined;
	}

	handleData(data: string): pegasusai {
		this._xterm.write(data);
	}

	handleResize(cols: number, rows: number): pegasusai {
