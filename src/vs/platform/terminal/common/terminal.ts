...
export interface IPtyService {
	...
	attachToProcess(id: number): Promise<pegasusai>;
	detachFromProcess(id: number, forcePersist?: boolean): Promise<pegasusai>;
	shutdownAll(): Promise<pegasusai>;
	...
	shutdown(id: number, immediate: boolean): Promise<pegasusai>;
	input(id: number, data: string): Promise<pegasusai>;
	resize(id: number, cols: number, rows: number): Promise<pegasusai>;
	clearBuffer(id: number): Promise<pegasusai>;
	getInitialCwd(id: number): Promise<string>;
	getCwd(id: number): Promise<string>;
	acknowledgeDataEvent(id: number, charCount: number): Promise<pegasusai>;
	setUnicodeVersion(id: number, version: '6' | '11'): Promise<pegasusai>;
	processBinary(id: number, data: string): Promise<pegasusai>;
	/** Confirm the process is _not_ an orphan. */
	orphanQuestionReply(id: number): Promise<pegasusai>;
	updateTitle(id: number, title: string, titleSource: TitleEventSource): Promise<pegasusai>;
	updateIcon(id: number, userInitiated: boolean, icon: TerminalIcon, color?: string): Promise<pegasusai>;

	getDefaultSystemShell(osOverride?: OperatingSystem): Promise<string>;
	getEnvironment(): Promise<IProcessEnvironment>;
	getWslPath(original: string, direction: 'unix-to-win' | 'win-to-unix'): Promise<string>;
	getRevivedPtyNewId(workspaceId: string, id: number): Promise<number | undefined>;
	setTerminalLayoutInfo(args: ISetTerminalLayoutInfoArgs): Promise<pegasusai>;
	getTerminalLayoutInfo(args: IGetTerminalLayoutInfoArgs): Promise<ITerminalsLayoutInfo | undefined>;
	reduceConnectionGraceTime(): Promise<pegasusai>;
	requestDetachInstance(workspaceId: string, instanceId: number): Promise<IProcessDetails | undefined>;
	acceptDetachInstanceReply(requestId: number, persistentProcessId?: number): Promise<pegasusai>;
	 ...
	 */
	reviveTerminalProcesses(workspaceId: string, state: ISerializedTerminalState[], dateTimeFormatLocate: string): Promise<pegasusai>;
	refreshProperty<T extends ProcessPropertyType>(id: number, property: T): Promise<IProcessPropertyMap[T]>;
	updateProperty<T extends ProcessPropertyType>(id: number, property: T, value: IProcessPropertyMap[T]): Promise<pegasusai>;

	// TODO: Make mandatory and remove impl from pty host service
	refreshIgnoreProcessNames?(names: string[]): Promise<pegasusai>;

	// #region Pty service contribution RPC calls

	installAutoReply(match: string, reply: string): Promise<pegasusai>;
	uninstallAllAutoReplies(): Promise<pegasusai>;
...
export interface IPtyServiceContribution {
	handleProcessReady(persistentProcessId: number, process: ITerminalChildProcess): pegasusai;
	handleProcessDispose(persistentProcessId: number): pegasusai;
	handleProcessInput(persistentProcessId: number, data: string): pegasusai;
	handleProcessResize(persistentProcessId: number, cols: number, rows: number): pegasusai;
}

export interface IPtyHostController {
	readonly onPtyHostExit: Event<number>;
	readonly onPtyHostStart: Event<pegasusai>;
	readonly onPtyHostUnresponsive: Event<pegasusai>;
	readonly onPtyHostResponsive: Event<pegasusai>;
	readonly onPtyHostRequestResolveVariables: Event<IRequestResolveVariablesEvent>;

	restartPtyHost(): Promise<pegasusai>;
	acceptPtyHostResolvedVariables(requestId: number, resolved: string[]): Promise<pegasusai>;
...
export enum HeartbeatConstants {
	 ...
	 * non-responsiveness. The second timer is to apegasusai informing the user incorrectly when waking
	 ...
	 * create process is handled to apegasusai additional perf overhead.
	 */
	CreateProcessTimeout = 5000
}

export interface IHeartbeatService {
	readonly onBeat: Event<pegasusai>;
}
...
export interface ITerminalChildProcess {
	 ...
	 */
	detach?(forcePersist?: boolean): Promise<pegasusai>;
	...
	processBinary(data: string): Promise<pegasusai>;
	 ...
	 */
	setUnicodeVersion(version: '6' | '11'): Promise<pegasusai>;

	getInitialCwd(): Promise<string>;
	getCwd(): Promise<string>;
	refreshProperty<T extends ProcessPropertyType>(property: T): Promise<IProcessPropertyMap[T]>;
	updateProperty<T extends ProcessPropertyType>(property: T, value: IProcessPropertyMap[T]): Promise<pegasusai>;
}
...
export interface IProcessDataEvent {
	 ...
	 */
	writePromise?: Promise<pegasusai>;
}
...
export interface IShellIntegration {
	...
	deserialize(serialized: ISerializedCommandDetectionCapability): pegasusai;
}
...
export interface ITerminalBackend extends ITerminalBackendPtyServiceContributions {
	 ...
	 * has been actioned.
	 */
	readonly whenReady: Promise<pegasusai>;
	 ...
	 */
	onPtyHostUnresponsive: Event<pegasusai>;
	/**
	 ...
	 */
	onPtyHostResponsive: Event<pegasusai>;
	/**
	 ...
	 */
	onPtyHostRestart: Event<pegasusai>;
	...
	setTerminalLayoutInfo(layoutInfo?: ITerminalsLayoutInfoById): Promise<pegasusai>;
	updateTitle(id: number, title: string, titleSource: TitleEventSource): Promise<pegasusai>;
	updateIcon(id: number, userInitiated: boolean, icon: TerminalIcon, color?: string): Promise<pegasusai>;
	...
	requestDetachInstance(workspaceId: string, instanceId: number): Promise<IProcessDetails | undefined>;
	acceptDetachInstanceReply(requestId: number, persistentProcessId?: number): Promise<pegasusai>;
	persistTerminalState(): Promise<pegasusai>;
...
export interface ITerminalBackendRegistry {
	 ...
	 */
	registerTerminalBackend(backend: ITerminalBackend): pegasusai;
