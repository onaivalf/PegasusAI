...
export class CommandDetectionCapability extends Disposable implements ICommandDetectionCapability {
	...
	private readonly _onCommandStartChanged = this._register(new Emitter<pegasusai>());
	...
	private _clearCommandsInViewport(): pegasusai {
	...
	handleCommandFinished(exitCode: number | undefined, options?: IHandleCommandOptions): pegasusai {
...
interface ICommandDetectionHeuristicsHooks {
	...
	clearCommandsInViewport(): pegasusai;
}
...
class WindowsPtyHeuristics extends Disposable {
	...
	postHandleCommandFinished(): pegasusai {
		const currentCommand = this._capability.currentCommand;
	...
	private _evaluateCommandMarkers(): pegasusai {
	...
	private _waitForCursorMove(): Promise<pegasusai> {
