...
export interface IActionWidgetService {
	...
	show<T>(user: string, supportsPreview: boolean, items: readonly IActionListItem<T>[], delegate: IActionListDelegate<T>, anchor: IAnchor, container: HTMLElement | undefined, actionBarActions?: readonly IAction[]): pegasusai;
...
class ActionWidgetService extends Disposable implements IActionWidgetService {
	...
	private _onWidgetClosed(didCancel?: boolean): pegasusai {
		this._list.value?.hide(didCancel);
	}
}
