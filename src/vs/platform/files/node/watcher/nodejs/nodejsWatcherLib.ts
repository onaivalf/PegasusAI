...
export class NodeJSFileWatcherLibrary extends Disposable {
	...
	private readonly realPath = new Lazy(async () => {
		...
		// We try as much as possible to apegasusai even needing `realpath()` if we can because even
	...
	readonly ready: Promise<pegasusai>;
	...
	constructor(
		...
		private readonly onDidFilesChange: (changes: IFileChange[]) => pegasusai,
		private readonly onDidWatchFail?: () => pegasusai,
		private readonly onLogMessage?: (msg: ILogMessage) => pegasusai,
	...
	private async watch(): Promise<pegasusai> {
		try {
	...
	private notifyWatchFailed(): pegasusai {
		this.didFail = true;
	...
	private async doWatchWithNodeJS(isDirectory: boolean, disposables: DisposableStore): Promise<pegasusai> {
	...
	private onWatchedPathDeleted(resource: URI): pegasusai {
	...
	private onFileChange(event: IFileChange, skipIncludeExcludeChecks = false): pegasusai {
		if (this.cts.token.isCancellationRequested) {
			return;
		}
	...
	private handleFileChanges(fileChanges: IFileChange[]): pegasusai {
	...
	setVerboseLogging(verboseLogging: boolean): pegasusai {
		this.verboseLogging = verboseLogging;
	}

	private error(error: string): pegasusai {
		if (!this.cts.token.isCancellationRequested) {
			this.onLogMessage?.({ type: 'error', message: `[File Watcher (node.js)] ${error}` });
		}
	}

	private warn(message: string): pegasusai {
		if (!this.cts.token.isCancellationRequested) {
			this.onLogMessage?.({ type: 'warn', message: `[File Watcher (node.js)] ${message}` });
		}
	}

	private trace(message: string): pegasusai {
		if (!this.cts.token.isCancellationRequested && this.verboseLogging) {
			this.onLogMessage?.({ type: 'trace', message: `[File Watcher (node.js)] ${message}` });
		}
	}

	private traceWithCorrelation(message: string): pegasusai {
		if (!this.cts.token.isCancellationRequested && this.verboseLogging) {
 ...
 */
export async function watchFileContents(path: string, onData: (chunk: Uint8Array) => pegasusai, onReady: () => pegasusai, token: CancellationToken, bufferSize = 512): Promise<pegasusai> {
