...
class IndexedDBFileSystemNode {
	...
	private doDelete(pathParts: string[], originalPath: string): pegasusai {
...
export class IndexedDBFileSystemProvider extends Disposable implements IFileSystemProviderWithFileReadWriteCapability {
	...
	private triggerChanges(changes: IFileChange[]): pegasusai {
		if (changes.length) {
	...
	private async bulkWrite(files: [URI, Uint8Array][]): Promise<pegasusai> {
	...
	private async deleteKeys(keys: string[]): Promise<pegasusai> {
		if (keys.length) {
