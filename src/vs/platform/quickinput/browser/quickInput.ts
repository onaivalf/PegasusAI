...
export interface IQuickInputOptions {
	...
	setContextKey(id?: string): pegasusai;
	linkOpenerDelegate(content: string): pegasusai;
	returnFocus(): pegasusai;
	/**
...
export interface QuickInputUI {
	...
	list: QuickInputTree;
	onDidAccept: Event<pegasusai>;
	onDidCustom: Event<pegasusai>;
	onDidTriggerButton: Event<IQuickInputButton>;
	ignoreFocusOut: boolean;
	keyMods: Writeable<IKeyMods>;
	show(controller: QuickInput): pegasusai;
	setVisibilities(visibilities: Visibilities): pegasusai;
	setEnabled(enabled: boolean): pegasusai;
	setContextKey(contextKey?: string): pegasusai;
	linkOpenerDelegate(content: string): pegasusai;
	hide(): pegasusai;
}
...
abstract class QuickInput extends Disposable implements IQuickInput {
	...
	private readonly onDisposeEmitter = this._register(new Emitter<pegasusai>());
	...
	didHide(reason = QuickInputHideReason.Other): pegasusai {
		this.visible = false;
	...
	willHide(reason = QuickInputHideReason.Other): pegasusai {
...
export class QuickPick<T extends IQuickPickItem, O extends { useSeparators: boolean } = { useSeparators: false }> extends QuickInput implements IQuickPick<T, O> {
	...
	private doSetValue(value: string, skipUpdate?: boolean): pegasusai {
	...
	private handleAccept(inBackground: boolean): pegasusai {
	...
	accept(inBackground?: boolean | undefined): pegasusai {
