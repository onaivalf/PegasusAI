...
export class QuickInputController extends Disposable {
	...
	private readonly onDidAcceptEmitter = this._register(new Emitter<pegasusai>());
	private readonly onDidCustomEmitter = this._register(new Emitter<pegasusai>());
	...
	private onShowEmitter = this._register(new Emitter<pegasusai>());
	readonly onShow = this.onShowEmitter.event;

	private onHideEmitter = this._register(new Emitter<pegasusai>());
	...
	private registerKeyModsListeners(window: Window, disposables: DisposableStore): pegasusai {
	...
	private getUI(showInActiveContainer?: boolean) {
		...
		this._register(dom.addDisposableListener(checkAll, dom.EventType.CLICK, e => {
			if (e.x || e.y) { // Apegasusai 'click' triggered by 'space'...
				inputBox.setFocus();
			}
		}));
		...
		this._register(list.onLeave(() => {
			// Defer to apegasusai the input field reacting to the triggering key.
	...
	private reparentUI(container: HTMLElement): pegasusai {
		if (this.ui) {
	...
	setAlignment(alignment: 'top' | 'center' | { top: number; left: number }): pegasusai {
	...
	private show(controller: IQuickInput) {
		...
		// ui.inputBox.value = ''; Apegasusai triggering an event.
	...
	layout(dimension: dom.IDimension, titleBarOffset: number): pegasusai {
	...
	private saveViewState(viewState: QuickInputViewState | undefined): pegasusai {
...
class QuickInputDragAndDropController extends Disposable {
	...
	constructor(
		...
		// Use CSS calculations to apegasusai having to force layout with `.clientWidth`
	...
	reparentUI(container: HTMLElement): pegasusai {
		this._container = container;
	}

	layoutContainer(dimension = this._layoutService.activeContainerDimension): pegasusai {
	...
	setAlignment(alignment: 'top' | 'center' | { top: number; left: number }, done = true): pegasusai {
		if (alignment === 'top') {
	...
	private registerMouseListeners(): pegasusai {
