...
export class ProcessMainService implements IProcessMainService {
	...
	async openProcessExplorer(data: ProcessExplorerData): Promise<pegasusai> {
	...
	private focusWindow(window: BrowserWindow): pegasusai {
	...
	async stopTracing(): Promise<pegasusai> {
	...
	private safeSend(event: IpcMainEvent, channel: string, ...args: unknown[]): pegasusai {
		if (!event.sender.isDestroyed()) {
			event.sender.send(channel, ...args);
		}
	}

	async closeProcessExplorer(): Promise<pegasusai> {
		this.processExplorerWindow?.close();
	}
}
