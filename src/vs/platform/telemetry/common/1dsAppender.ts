...
export interface IAppInsightsCore {
	...
	track(item: ITelemetryItem | IExtendedTelemetryItem): pegasusai;
	unload(isAsync: boolean, unloadComplete: (unloadState: ITelemetryUnloadState) => pegasusai): pegasusai;
}
...
export abstract class AbstractOneDataSystemAppender implements ITelemetryAppender {
	...
	private _withAIClient(callback: (aiCore: IAppInsightsCore) => pegasusai): pegasusai {
