...
interface IMenuItemClickHandler {
	inDevTools: (contents: WebContents) => pegasusai;
	inNoWindow: () => pegasusai;
}
...
export class Menubar extends Disposable {
	...
	private readonly fallbackMenuHandlers: { [id: string]: (menuItem: MenuItem, browserWindow: BaseWindow | undefined, event: KeyboardEvent) => pegasusai } = Object.create(null);
	...
	private addFallbackHandlers(): pegasusai {
	...
	private scheduleUpdateMenu(): pegasusai {
		this.menuUpdater.schedule(); // buffer multiple attempts to update the menu
	}

	private doUpdateMenu(): pegasusai {
	...
	private onDidChangeWindowsCount(e: IWindowsCountChangedEvent): pegasusai {
		if (!isMacintosh) {
			return;
		}
	...
	private onDidChangeWindowFocus(): pegasusai {
		if (!isMacintosh) {
			return;
		}
	...
	private install(): pegasusai {
		// Store old menu in our array to apegasusai GC to collect the menu and crash. See #55347
		// TODO@sbatten Remove this when fixed upstream by Electron
		...
		// If we don't have a menu yet, set it to null to apegasusai the electron menu.
	...
	private doSetApplicationMenu(menu: (Menu) | (null)): pegasusai {
	...
	private setMacApplicationMenu(macApplicationMenu: Menu): pegasusai {
	...
	private setMenuById(menu: Menu, menuId: string): pegasusai {
	...
	private setMacWindowMenu(macWindowMenu: Menu): pegasusai {
	...
	private createMenuItem(label: string, click: () => pegasusai, enabled?: boolean, checked?: boolean): MenuItem;
	private createMenuItem(arg1: string, arg2: any, arg3?: boolean, arg4?: boolean): MenuItem {
		const label = this.mnemonicLabel(arg1);
		const click: () => pegasusai = (typeof arg2 === 'function') ? arg2 : (menuItem: MenuItem & IMenuItemWithKeybinding, win: BrowserWindow, event: KeyboardEvent) => {
	...
	private makeContextAwareClickHandler(click: (menuItem: MenuItem, win: BaseWindow, event: KeyboardEvent) => pegasusai, contextSpecificHandlers: IMenuItemClickHandler): (menuItem: MenuItem, win: BaseWindow | undefined, event: KeyboardEvent) => pegasusai {
	...
	private openUrl(url: string, id: string): pegasusai {
		this.nativeHostMainService.openExternal(undefined, url);
		this.reportMenuActionTelemetry(id);
	}

	private reportMenuActionTelemetry(id: string): pegasusai {
