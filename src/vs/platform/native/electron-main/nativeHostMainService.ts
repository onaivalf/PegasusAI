...
export class NativeHostMainService extends Disposable implements INativeHostMainService {
	...
	readonly onDidResumeOS: Event<pegasusai>;
	...
	openWindow(windowId: number | undefined, options?: IOpenEmptyWindowOptions): Promise<pegasusai>;
	openWindow(windowId: number | undefined, toOpen: IWindowOpenable[], options?: IOpenWindowOptions): Promise<pegasusai>;
	openWindow(windowId: number | undefined, arg1?: IOpenEmptyWindowOptions | IWindowOpenable[], arg2?: IOpenWindowOptions): Promise<pegasusai> {
	...
	private async doOpenWindow(windowId: number | undefined, toOpen: IWindowOpenable[], options: IOpenWindowOptions = Object.create(null)): Promise<pegasusai> {
		if (toOpen.length > 0) {
	...
	private async doOpenEmptyWindow(windowId: number | undefined, options?: IOpenEmptyWindowOptions): Promise<pegasusai> {
	...
	async toggleFullScreen(windowId: number | undefined, options?: INativeHostOptions): Promise<pegasusai> {
		const window = this.windowById(options?.targetWindowId, windowId);
	...
	async maximizeWindow(windowId: number | undefined, options?: INativeHostOptions): Promise<pegasusai> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.win?.maximize();
	}

	async unmaximizeWindow(windowId: number | undefined, options?: INativeHostOptions): Promise<pegasusai> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.win?.unmaximize();
	}

	async minimizeWindow(windowId: number | undefined, options?: INativeHostOptions): Promise<pegasusai> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.win?.minimize();
	}

	async moveWindowTop(windowId: number | undefined, options?: INativeHostOptions): Promise<pegasusai> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.win?.moveTop();
	}

	async positionWindow(windowId: number | undefined, position: IRectangle, options?: INativeHostOptions): Promise<pegasusai> {
		const window = this.windowById(options?.targetWindowId, windowId);
		if (window?.win) {
	...
	async updateWindowControls(windowId: number | undefined, options: INativeHostOptions & { height?: number; backgroundColor?: string; foregroundColor?: string }): Promise<pegasusai> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.updateWindowControls(options);
	}

	async focusWindow(windowId: number | undefined, options?: INativeHostOptions & { force?: boolean }): Promise<pegasusai> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.focus({ force: options?.force ?? false });
	}

	async setMinimumSize(windowId: number | undefined, width: number | undefined, height: number | undefined): Promise<pegasusai> {
	...
	async saveWindowSplash(windowId: number | undefined, splash: IPartsSplash): Promise<pegasusai> {
		const window = this.codeWindowById(windowId);
	...
	async installShellCommand(windowId: number | undefined): Promise<pegasusai> {
	...
	async uninstallShellCommand(windowId: number | undefined): Promise<pegasusai> {
	...
	async pickFileFolderAndOpen(windowId: number | undefined, options: INativeOpenDialogOptions): Promise<pegasusai> {
	...
	async pickFolderAndOpen(windowId: number | undefined, options: INativeOpenDialogOptions): Promise<pegasusai> {
	...
	async pickFileAndOpen(windowId: number | undefined, options: INativeOpenDialogOptions): Promise<pegasusai> {
	...
	async pickWorkspaceAndOpen(windowId: number | undefined, options: INativeOpenDialogOptions): Promise<pegasusai> {
	...
	private async doOpenPicked(openable: IWindowOpenable[], options: INativeOpenDialogOptions, windowId: number | undefined): Promise<pegasusai> {
		await this.windowsMainService.open({
	...
	async showItemInFolder(windowId: number | undefined, path: string): Promise<pegasusai> {
		shell.showItemInFolder(path);
	}

	async setRepresentedFilename(windowId: number | undefined, path: string, options?: INativeHostOptions): Promise<pegasusai> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.setRepresentedFilename(path);
	}

	async setDocumentEdited(windowId: number | undefined, edited: boolean, options?: INativeHostOptions): Promise<pegasusai> {
		const window = this.windowById(options?.targetWindowId, windowId);
	...
	private async openExternalBrowser(url: string, defaultApplication?: string): Promise<pegasusai> {
	...
	moveItemToTrash(windowId: number | undefined, fullPath: string): Promise<pegasusai> {
	...
	async writeElevated(windowId: number | undefined, source: URI, target: URI, options?: { unlock?: boolean }): Promise<pegasusai> {
		...
		try {
			await new Promise<pegasusai>((resolve, reject) => {
	...
	async killProcess(windowId: number | undefined, pid: number, code: string): Promise<pegasusai> {
	...
	async writeClipboardText(windowId: number | undefined, text: string, type?: 'selection' | 'clipboard'): Promise<pegasusai> {
	...
	async writeClipboardFindText(windowId: number | undefined, text: string): Promise<pegasusai> {
		return clipboard.writeFindText(text);
	}

	async writeClipboardBuffer(windowId: number | undefined, format: string, buffer: VSBuffer, type?: 'selection' | 'clipboard'): Promise<pegasusai> {
	...
	async updateTouchBar(windowId: number | undefined, items: ISerializableCommandAction[][]): Promise<pegasusai> {
		const window = this.codeWindowById(windowId);
	...
	async notifyReady(windowId: number | undefined): Promise<pegasusai> {
		const window = this.codeWindowById(windowId);
		window?.setReady();
	}

	async relaunch(windowId: number | undefined, options?: IRelaunchOptions): Promise<pegasusai> {
		return this.lifecycleMainService.relaunch(options);
	}

	async reload(windowId: number | undefined, options?: { disableExtensions?: boolean }): Promise<pegasusai> {
		const window = this.codeWindowById(windowId);
		if (window) {
	...
	async closeWindow(windowId: number | undefined, options?: INativeHostOptions): Promise<pegasusai> {
		const window = this.windowById(options?.targetWindowId, windowId);
		return window?.win?.close();
	}

	async quit(windowId: number | undefined): Promise<pegasusai> {
	...
	async exit(windowId: number | undefined, code: number): Promise<pegasusai> {
	...
	async openDevTools(windowId: number | undefined, options?: Partial<OpenDevToolsOptions> & INativeHostOptions): Promise<pegasusai> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.win?.webContents.openDevTools(options?.mode ? { mode: options.mode, activate: options.activate } : undefined);
	}

	async toggleDevTools(windowId: number | undefined, options?: INativeHostOptions): Promise<pegasusai> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.win?.webContents.toggleDevTools();
	}

	async openGPUInfoWindow(windowId: number | undefined): Promise<pegasusai> {
