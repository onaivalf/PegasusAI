...
export class FileStorage extends Disposable {
	...
	private readonly flushDelayer: ThrottledDelayer<pegasusai>;

	private initializing: Promise<pegasusai> | undefined = undefined;
	private closing: Promise<pegasusai> | undefined = undefined;
	...
	constructor(
		...
		this.flushDelayer = this._register(new ThrottledDelayer<pegasusai>(saveStrategy === SaveStrategy.IMMEDIATE ? 0 : 100 /* buffer saves over a short time */));
	}
	...
	private async doInit(): Promise<pegasusai> {
		try {
	...
	private async save(): Promise<pegasusai> {
		if (this.closing) {
	...
	private async doSave(): Promise<pegasusai> {
		if (!this.initializing) {
