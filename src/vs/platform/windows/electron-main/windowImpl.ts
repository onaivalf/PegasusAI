...
export abstract class BaseWindow extends Disposable implements IBaseWindow {
	...
	private readonly _onDidClose = this._register(new Emitter<pegasusai>());
	readonly onDidClose = this._onDidClose.event;

	private readonly _onDidMaximize = this._register(new Emitter<pegasusai>());
	readonly onDidMaximize = this._onDidMaximize.event;

	private readonly _onDidUnmaximize = this._register(new Emitter<pegasusai>());
	...
	private readonly _onDidEnterFullScreen = this._register(new Emitter<pegasusai>());
	readonly onDidEnterFullScreen = this._onDidEnterFullScreen.event;

	private readonly _onDidLeaveFullScreen = this._register(new Emitter<pegasusai>());
	...
	protected setWin(win: electron.BrowserWindow, options?: BrowserWindowConstructorOptions): pegasusai {
		this._win = win;
	...
	protected applyState(state: IWindowState, hasMultipleDisplays = electron.screen.getAllDisplays().length > 0): pegasusai {
	...
	setRepresentedFilename(filename: string): pegasusai {
		if (isMacintosh) {
	...
	protected setFullScreen(fullscreen: boolean, fromRestore: boolean): pegasusai {
	...
	private setNativeFullScreen(fullscreen: boolean, fromRestore: boolean): pegasusai {
		const win = this.win;
	...
	private doSetNativeFullScreen(fullscreen: boolean, fromRestore: boolean): pegasusai {
		if (isMacintosh) {
	...
	private setSimpleFullScreen(fullscreen: boolean): pegasusai {
		const win = this.win;
		if (win?.isFullScreen()) {
...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	private readonly _onDidSignalReady = this._register(new Emitter<pegasusai>());
	readonly onDidSignalReady = this._onDidSignalReady.event;

	private readonly _onDidDestroy = this._register(new Emitter<pegasusai>());
	...
	private readonly whenReadyCallbacks: { (window: ICodeWindow): pegasusai }[] = [];
	...
	private readonly jsCallStackCollector: Delayer<pegasusai>;
	...
	constructor(
		...
		this.jsCallStackCollector = this._register(new Delayer<pegasusai>(sampleInterval));
	...
	get whenClosedOrLoaded(): Promise<pegasusai> {
	...
	private async onWindowError(error: WindowError.UNRESPONSIVE): Promise<pegasusai>;
	private async onWindowError(error: WindowError.RESPONSIVE): Promise<pegasusai>;
	private async onWindowError(error: WindowError.PROCESS_GONE, details: { reason: string; exitCode: number }): Promise<pegasusai>;
	private async onWindowError(error: WindowError.LOAD, details: { reason: string; exitCode: number }): Promise<pegasusai>;
	private async onWindowError(type: WindowError, details?: { reason?: string; exitCode?: number }): Promise<pegasusai> {
	...
	private async destroyWindow(reopen: boolean, skipRestoreEditors: boolean): Promise<pegasusai> {
	...
	private onDidDeleteUntitledWorkspace(workspace: IWorkspaceIdentifier): pegasusai {
	...
	load(configuration: INativeWindowConfiguration, options: ILoadOptions = Object.create(null)): pegasusai {
	...
	private updateConfiguration(configuration: INativeWindowConfiguration, options: ILoadOptions): pegasusai {
	...
	async reload(cli?: NativeParsedArgs): Promise<pegasusai> {
	...
	protected override setFullScreen(fullscreen: boolean, fromRestore: boolean): pegasusai {
	...
	private setMenuBarVisibility(visibility: MenuBarVisibility, notify: boolean = true): pegasusai {
		if (isMacintosh) {
	...
	private doSetMenuBarVisibility(visibility: MenuBarVisibility): pegasusai {
	...
	updateTouchBar(groups: ISerializableCommandAction[][]): pegasusai {
		if (!isMacintosh) {
	...
	private createTouchBar(): pegasusai {
		if (!isMacintosh) {
			return; // only supported on macOS
		}

		// To apegasusai flickering, we try to reuse the touch bar group
	...
	private async startCollectingJScallStacks(): Promise<pegasusai> {
	...
	private stopCollectingJScallStacks(): pegasusai {
...
class UnresponsiveError extends Error {
	...
	constructor(sample: string, windowId: number, pid: number = 0) {
		...
		// we can apegasusai collecting them when constructing the error.
