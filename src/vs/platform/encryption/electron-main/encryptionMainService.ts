...
interface ISafeStorageAdditionalAPIs {
	setUsePlainTextEncryption(usePlainText: boolean): pegasusai;
...
export class EncryptionMainService implements IEncryptionMainService {
	...
	constructor(
		...
		// PegasusAI added this as a nice default for linux so you don't need to specify encryption provider
	...
	async setUsePlainTextEncryption(): Promise<pegasusai> {
		if (isWindows) {
