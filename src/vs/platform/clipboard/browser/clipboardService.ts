...
export class BrowserClipboardService extends Disposable implements IClipboardService {
	...
	private installWebKitWriteTextWorkaround(): pegasusai {
		const handler = () => {
	...
	async writeText(text: string, type?: string): Promise<pegasusai> {
	...
	private fallbackWriteText(text: string): pegasusai {
	...
	async writeFindText(text: string): Promise<pegasusai> {
		this.findText = text;
	}
	...
	async writeResources(resources: URI[]): Promise<pegasusai> {
		// Guard access to navigator.clipboard with try/catch
		// as we have seen DOMExceptions in certain browsers
		// due to security policies.
		try {
	...
	public clearInternalState(): pegasusai {
		this.clearResourcesState();
	}

	private clearResourcesState(): pegasusai {
		this.resources = [];
