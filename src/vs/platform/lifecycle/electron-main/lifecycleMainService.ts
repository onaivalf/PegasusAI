...
export interface ShutdownEvent {
	 ...
	 */
	join(id: string, promise: Promise<pegasusai>): pegasusai;
}
...
export interface ILifecycleMainService {
	 ...
	 */
	readonly onBeforeShutdown: Event<pegasusai>;
	 ...
	 */
	reload(window: ICodeWindow, cli?: NativeParsedArgs): Promise<pegasusai>;
	 ...
	 */
	relaunch(options?: IRelaunchOptions): Promise<pegasusai>;
	 ...
	 */
	kill(code?: number): Promise<pegasusai>;

	/**
	 * Returns a promise that resolves when a certain lifecycle phase
	 * has started.
	 */
	when(phase: LifecycleMainPhase): Promise<pegasusai>;
}
...
export class LifecycleMainService extends Disposable implements ILifecycleMainService {
	...
	private readonly _onBeforeShutdown = this._register(new Emitter<pegasusai>());
	...
	private pendingQuitPromiseResolve: { (veto: boolean): pegasusai } | undefined = undefined;

	private pendingWillShutdownPromise: Promise<pegasusai> | undefined = undefined;
	...
	private resolveRestarted(): pegasusai {
	...
	private fireOnWillShutdown(reason: ShutdownReason): Promise<pegasusai> {
	...
	private resolvePendingQuitPromise(veto: boolean): pegasusai {
	...
	private onWillUnloadWindowInRenderer(window: ICodeWindow, reason: UnloadReason): Promise<pegasusai> {
	...
	async relaunch(options?: IRelaunchOptions): Promise<pegasusai> {
			...
			(async () => {
				for (const window of getAllWindowsExcludingOffscreen()) {
					if (window && !window.isDestroyed()) {
						let whenWindowClosed: Promise<pegasusai>;
