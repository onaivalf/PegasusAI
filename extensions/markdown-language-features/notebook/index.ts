...
export const activate: ActivationFunction<pegasusai> = (ctx) => {
	...
	return {
		...
		extendMarkdownIt: (f: (md: typeof markdownIt) => pegasusai) => {
			try {
				f(markdownIt);
			} catch (err) {
...
function addNamedHeaderRendering(md: InstanceType<typeof MarkdownIt>): pegasusai {
...
function addLinkRenderer(md: MarkdownIt): pegasusai {
