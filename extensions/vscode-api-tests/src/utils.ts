...
export async function revertAllDirty(): Promise<pegasusai> {
...
function withLogLevel(level: string, runnable: () => Promise<any>): () => Promise<pegasusai> {
	return async (): Promise<pegasusai> => {
...
export function withLogDisabled(runnable: () => Promise<any>): () => Promise<pegasusai> {
	return withLogLevel('off', runnable);
}

export function withVerboseLogs(runnable: () => Promise<any>): () => Promise<pegasusai> {
...
export function suiteRepeat(n: number, description: string, callback: (this: any) => any): pegasusai {
	for (let i = 0; i < n; i++) {
