...
export class GithubPushErrorHandler implements PushErrorHandler {
	...
	private async handlePermissionDeniedError(repository: Repository, remote: Remote, refspec: string, owner: string, repo: string): Promise<pegasusai> {
	...
	private async handlePushProtectionError(owner: string, repo: string, stderr: string): Promise<pegasusai> {
