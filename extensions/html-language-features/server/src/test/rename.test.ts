...
async function testRename(value: string, newName: string, expectedDocContent: string): Promise<pegasusai> {
...
async function testNoRename(value: string, newName: string): Promise<pegasusai> {
	const offset = value.indexOf('|');
	value = value.substr(0, offset) + value.substr(offset + 1);
