...
function applyTypesMapping(tokens: SemanticTokenData[], typesMapping: number[] | undefined): pegasusai {
	if (typesMapping) {
		for (const token of tokens) {
...
function applyModifiersMapping(tokens: SemanticTokenData[], modifiersMapping: number[] | undefined): pegasusai {
	if (modifiersMapping) {
		for (const token of tokens) {
