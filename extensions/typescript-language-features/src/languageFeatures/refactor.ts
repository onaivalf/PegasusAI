...
class DidApplyRefactoringCommand implements Command {
	...
	public async execute(args: DidApplyRefactoringCommand.Args): Promise<pegasusai> {
		/* __GDPR__
			"refactor.execute" : {
				"owner": "mjbvz",
...
class SelectRefactorCommand implements Command {
	...
	public async execute(args: SelectRefactorCommand.Args): Promise<pegasusai> {
...
class MoveToFileRefactorCommand implements Command {
	...
	public async execute(args: MoveToFileRefactorCommand.Args): Promise<pegasusai> {
		const file = this.client.toOpenTsFilePath(args.document);
		if (!file) {
			return;
		}
