...
export class JSONSchemaCache {
	...
	async putSchema(schemaUri: string, etag: string, schemaContent: string): Promise<pegasusai> {
		try {
	...
	private async deleteSchemaFile(schemaUri: string, cacheEntry: CacheEntry): Promise<pegasusai> {
		const cacheLocation = path.join(this.schemaCacheLocation, cacheEntry.fileName);
		delete this.cacheInfo[schemaUri];
		await this.updateMemento();
		try {
