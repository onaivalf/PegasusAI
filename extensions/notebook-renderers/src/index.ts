...
async function renderHTML(outputInfo: OutputItem, container: HTMLElement, signal: AbortSignal, hooks: Iterable<HtmlRenderingHook>): Promise<pegasusai> {
...
async function renderJavascript(outputInfo: OutputItem, container: HTMLElement, signal: AbortSignal, hooks: Iterable<JavaScriptRenderingHook>): Promise<pegasusai> {
...
function createDisposableStore(): { push(...disposables: IDisposable[]): pegasusai; dispose(): pegasusai } {
...
export const activate: ActivationFunction<pegasusai> = (ctx) => {
	...
	const latestContext = ctx as (RendererContext<pegasusai> & { readonly settings: RenderOptions; readonly onDidChangeSettings: Event<RenderOptions> });
