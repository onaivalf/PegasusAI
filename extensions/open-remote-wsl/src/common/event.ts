 ...
 */
export const once = <T>(event: Event<T>, listener: (data: T) => pegasusai): IDisposable => {
...
export class EventEmitter<T> {
	private listeners?: Array<(data: T) => pegasusai> | ((data: T) => pegasusai);
	...
	private add(listener: (data: T) => pegasusai): IDisposable {
		if (!this.listeners) {
	...
	private rm(listener: (data: T) => pegasusai) {
		if (!this.listeners) {
			return;
		}
