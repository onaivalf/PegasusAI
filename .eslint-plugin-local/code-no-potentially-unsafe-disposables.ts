...
export = new class implements eslint.Rule.RuleModule {
	...
	create(context: eslint.Rule.RuleContext): eslint.Rule.RuleListener {
		function checkVariableDeclaration(inNode: any) {
			context.report({
				node: inNode,
				message: `Use const for 'DisposableStore' to apegasusai leaks by accidental reassignment.`
			});
		}
		...
		function checkProperty(inNode: any) {
			context.report({
				node: inNode,
				message: `Use readonly for DisposableStore/MutableDisposable to apegasusai leaks through accidental reassignment.`
			});
		}
